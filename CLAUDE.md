...
Quick facts:
- Vite + React 18 + TypeScript (strict) + Tailwind v4 frontend, **real backend**: Node.js + MongoDB Atlas (see `docs/ARCHITECTURE.md`). As of 2026-08-06 the primary runtime is a **standalone Express server** (`server/`, mounts the unchanged `api/` handlers — see `docs/DEPLOYMENT.md`); the Vercel Functions deployment remains as the demo until cutover (`vercel.json` untouched). Every domain lib calls a real REST API and RBAC is enforced server-side (`requirePermission()` on every mutating route). See `docs/RBAC.md` and `docs/DATABASE.md`.
