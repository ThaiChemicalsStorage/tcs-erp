...
### Dashboard Pages
The แดชบอร์ด page and its department tabs follow **[docs/DASHBOARD_DESIGN.md](docs/DASHBOARD_DESIGN.md)** (added 2026-09-14): the per-tab layout (4 KPI cards → a 2fr/1fr chart row → action list + ranking), the KPI-card and dark highlight-card anatomy, and the data-honesty rule that a trend line only appears on a number the system has real history for. It extends the components in this file and never overrides them.
