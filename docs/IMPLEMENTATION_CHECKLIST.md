...
| Dashboard | **Completed against the full business spec + an independent Codex review + a UI/UX redesign**, verification **In Progress** | Full Executive BI rebuild 2026-07-10 (~20 KPIs, pipeline, filters, rankings, forecast, follow-ups, approvals, activity feed, charts), code-reviewed (10 real bugs found and fixed), a same-day completion pass against the detailed business spec, a third pass fixing an independent Codex review's findings, then a **fourth same-day pass** replacing the cluttered flat KPI grid with a tiered Primary/Secondary KPI hierarchy, replacing the Sales Pipeline funnel chart (overlapping labels, read as broken) with horizontal step cards + a separate off-ramp row, adding a Quotation Status Summary and a new Sales Activity Analytics section (Created/Edited counts by week/month/quarter/year), dropping 5 now-redundant charts, and adding shared `PageHeader`/`EmptyState`/`MetricInfoTooltip` components plus an optional first-time Driver.js guided tour. See CHANGELOG and [MODULES/Dashboard.md](./MODULES/Dashboard.md) for the itemized list. Not yet exercised against live data in a browser — see [PROJECT_STATUS.md](./PROJECT_STATUS.md) "Known Risks" for the specific, reproducible, three-times-confirmed environment limitation (MongoDB Atlas SRV DNS resolution blocked in the sandboxed session), not a defect; the redesign's client bundle was confirmed error-free via a Playwright check as a partial substitute. |
| Quotation | **Completed**, real server-side validation + authoritative audit trail | List/create/edit/duplicate, 9-status approval workflow, print/PDF, product-library picker, Job Type/Potential Opportunity/Follow-up Date (added 2026-07-10). **2026-07-10, Codex review fix**: `api/_lib/quoteValidation.ts` validates every field server-side (type/length/range/dates), `amount` is always server-derived (never client-writable), Job Type is required on create and membership-checked on any edit that changes it, quotation numbering is now atomic (`counters` collection, not scan-then-max+1). **2026-07-10, fifth pass**: every mutation (create/update/duplicate/workflow) now writes its own audit-log entry server-side (previously the client called the generic audit endpoint after the fact, which was forgeable); Reject/Customer-Reject/Cancel now require a non-empty comment server-side, not just in the UI. |
...
| Activity Analytics (period-grouped, multi-dimension) | **Partially completed 2026-07-10** | The new Sales Activity Analytics section (`SalesActivityAnalytics.tsx`) covers the period-grouped (week/month/quarter/year) Created-vs-Edited count part, filterable by salesperson/department. The Dashboard's separate Activity Timeline widget is still a flat recent-N feed from `audit_log`, not grouped or filterable — that specific gap remains. |
