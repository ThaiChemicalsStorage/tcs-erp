...
- [ ] **Verify the 2026-07-10 Executive Dashboard/Job Type pass against live data.** `tsc`/`lint`/`build` pass clean and the feature went through a full 10-angle code review that found and fixed 10 real bugs (see CHANGELOG), but no browser pass against real MongoDB data has happened: no local DB credential and no safe non-production test environment were available in that session. Needs the user to test locally (`vercel env pull` + `vercel dev`, or point a local `.env` at a dev/staging Mongo cluster if one gets created) or on a preview deploy, or explicit sign-off to test against production with cleanup afterward. See [PROJECT_STATUS.md](./PROJECT_STATUS.md) "In Progress."
...
- [ ] Revisit `DashboardPage` bundle size (~490KB gzipped ~130KB as of the 2026-07-10 rebuild, up from ~430KB, mostly `recharts`) if more chart-heavy modules get added.
