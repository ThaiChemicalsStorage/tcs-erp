...
- [ ] Lift `QuotationPage`'s `view`/`selectedId` state up to `App.tsx` so a notification click can deep-link straight to the specific quote's detail view — currently it only navigates to the quotation list module. **Partially related work landed 2026-07-10**: a narrower `quotationListFilter` (status/client, not a specific record) is now lifted for the Dashboard's pipeline/follow-up click-through — this item (full per-quote deep-link) is still open.
