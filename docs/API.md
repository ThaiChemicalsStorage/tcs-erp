...
| `POST /api/auth/forgot-password` | None | `{ identifier, note? }` | `200 { ok: true }` always | **2026-09-24.** Password recovery request (see MODULES/Auth.md). Same answer whether or not the account exists; 5 per IP per 15 min → `429`; notifies every active Super Admin once per pending request. |
...
| `GET /api/users/password-resets` | **Super Admin role only** (`isSuperAdmin`, 403 otherwise — even with `users:manage`) | **2026-09-24.** Pending requests (newest first) then the last 100 resolved/dismissed. |
| `POST /api/users/password-resets/:id/issue` | Super Admin only | Generates a temporary password, hashes it onto the user, sets `mustChangePassword`, revokes the user's sessions, resolves the request; returns `{ temporaryPassword, request }` — the only time the password exists outside the hash. `409` if the request is no longer pending, `400` if the account is inactive. The request is claimed atomically *before* the password changes (2026-09-24f) — two Super Admins clicking at once get one password and one `409`, never two passwords where the first no longer works. |
| `POST /api/users/password-resets/:id/dismiss` | Super Admin only | Closes a pending request without touching the account. |
...
| `GET /api/stock-movements/history?q=&kind=&sourceType=&productId=&departmentId=&from=&to=&skip=&limit=` | `stock:view` | **2026-09-23** — the หน้าประวัติสต๊อก feed. Every filter optional; `from`/`to` are Bangkok `YYYY-MM-DD`, inclusive; unknown `kind`/`sourceType` or a malformed date → 400. `q` matches product code/name, `sourceLabel`, reason, department/team **and** the source documents themselves (MR job code/job order/production order/customer, RR PO number/vendor, PR job code — capped at 500 matching documents). Page size default 50, max 500. Returns `{ movements, total, summary }`: each row adds `createdByName` and `link` (job/PO/vendor/code read from its source document at request time — nothing is denormalised onto the ledger); `total` and the per-kind `summary` (count + absolute value) cover the **whole filtered set**, not just the page. |
| `POST /api/stock-movements/import` | `stock:adjust` | **2026-09-23** — body `{ rows: [{ code, qty, unitCost? }], note? }`, max 5,000 rows. `qty` is the balance the item **should have**; the server posts the difference against the live balance, one movement per changed item (`receive` + `unitCost` when it rises and a cost is given — re-averages like a receipt; otherwise `adjust`; unchanged rows skipped). The whole file is validated first — any unknown/duplicate code or bad number → 400 and nothing is written. Every row of one call shares `sourceType: "stock_import"` and a batch label `SI-YYYYMM-NNNN` in `sourceId`/`sourceLabel`. Returns `{ batchLabel, changed, unchanged }`; one audit entry per call. The spreadsheet itself is parsed in the browser and never uploaded. |
...
| `POST /api/receiving-reports/:id/receipts` | `receivingReport:receive` | One round of receiving. A blank report needs `vendorName` first (`400`) — the payable must name a creditor. Validates `qty ≤ outstanding` per line, at least one positive line, a non-empty `invoiceNumber`, and that every referenced product still exists — all **before** the first stock write, so a rejected round writes nothing at all. **2026-09-24**: optional `priceType`, `discount` + `discountMode`, `creditDays` (default: the report's) — totals via `batchTotals()`; a custom biller with no `billerName` is `400`; the payable is posted under the biller's name, after discount, and each stock line's `unitCost` is the unit price × `costFactor` (pre-VAT, discount spread pro rata); the batch stores its terms, `grossAmount`, `discountAmt` and `dueDate`. Then: `applyStockMovement(kind:"receive", unitCost, sourceType:"receiving_report")` per line that has a `productId` (a hand-typed line posts a payable but no stock) → one `ap_entries` row → push the batch → auto-close when nothing is outstanding. Returns the updated document. |
...
| `GET /api/material-requisitions/store-sources` | `materialRequisition:view` | **2026-09-23.** Other departments' (`ownerDepartment` ≠ `store`) `Final` requisitions that still have something to issue, oldest first, with `outstandingLineCount`, capped at 500 **matches** (read with a cursor and filtered while reading — before 2026-09-24f it cut 500 then filtered, so once 500+ fully-issued requisitions existed new ones never showed) — the "อ้างอิงใบเบิก" choices of a store issue slip, and **since 2026-09-24 also the store page's "ใบเบิกจากแผนก" tab** (approved requisitions "sent" to Stores; matched to existing slips via the list summary's new `sourceRequisitionId`). |
| `POST /api/material-requisitions/:id/issues` | `stock:adjust` | **Material Requisition only. 2026-09-07 — replaces `POST /:id/issue`. **2026-09-24: no longer capped at the outstanding quantity** (owner: over-issue allowed) — only stock sufficiency is checked; a slip referencing a department requisition only checks that the source line still exists.** **2026-09-23:** on a store slip with `sourceRequisitionId`, each line's quantity is also capped by the source requisition's live outstanding (`400`), and the round is **mirrored onto the source requisition** (same batch id, lines mapped by `sourceLineId`, `stockMovementIds: []`, `storeSlipId`/`storeSlipNumber`) so the department sees what was issued; stock is deducted once, on the slip. `DELETE …/issues/:batchId` on the slip removes the mirror (refused if the source has returned more than would remain issued); on the department requisition a mirrored round cannot be cancelled (`400`). `Final` documents only — **except (2026-09-24) a store slip with `sourceRequisitionId`** (`storeSlipSkipsApproval()`), which issues straight from `Draft`/`PendingApproval`: the first round atomically flips it to `Final` before any stock is touched (filtered on the read `status` + `updatedAt`, `409` if an auto-save slipped in; reverted if the deduction fails before any movement), and `submit-approval` on such a slip is a `400`. Body is one issue round: `{ lines: [{ lineId, qty }], issuedDate?, issuedBy?, remark?, chargeDepartmentId?, chargeTeamId?, chargeWorkTypeCode?, chargeWorkTypeName? }`. Lines with `qty: 0` are skipped; `qty` above the line's outstanding quantity is a `400`. Appends to `issues[]`, deducts **that round's quantities only** (stock sufficiency for the whole round is checked before any movement is written), then recomputes each line's `withdrawal1Qty`/`withdrawal2Qty` from the rounds (round 1 → first column, rounds 2..N summed → second). Documents issued before this route existed are converted to synthetic rounds first, so their stored numbers survive and are not re-deducted. Returns `{ materialRequisition, stockByProduct }`. The old `POST /:id/issue` path is kept mounted and answers `400 "…กรุณารีเฟรชหน้าเว็บ…"` so a stale browser tab cannot corrupt the round list. |
...
| `POST /api/material-requisitions/:id/return` | **`stock:adjust` only since 2026-09-23** (owner's order: other departments may not return goods themselves; the department page no longer shows a return card — the store returns through a store receipt) | **Material Requisition only.** Body `{ lines: [{ id, returnQty }], returnedBy?, returnReceivedBy? }` — updates each line's `returnQty` plus the document-level returner/receiver signatures. Deliberately **not** gated by the `status === "Final"` lock the plain `PATCH` above enforces — leftover material is returned via the same original document, after issuance, same "follow-up fields survive Final" exemption Scope of Work's PO-chasing fields use (see [MODULES/ScopeOfWork.md](./MODULES/ScopeOfWork.md) "PO Chasing"). |
...
| `DELETE /api/store-receipts/:id` | `materialRequisition:delete` (owner or `:finalize` — 2026-09-24f) | Soft delete; 400 once posted; 403 for a non-owner without `:finalize` (same rule as deleting a requisition). |
| `GET /api/store-receipts/source-requisitions?receiptCode=` | `materialRequisition:view` | Final store requisitions with the paired code and something still out — not ownership-filtered (Store takes back goods anyone drew). Newest first, capped at 500 **matches** (filtered while reading, not cut before filtering — 2026-09-24f). |
...
| `POST /api/store-receipts/:id/post` | `stock:adjust` | Final + not yet posted. Validates every line first (return caps re-checked against the live requisition), then **claims the post atomically** (`postedAt: ""` → now; a concurrent second post gets 409, a failure part-way clears it again — 2026-09-24f). return → `return` movements at last-purchase cost + source `returnQty` += qty · receive → `receive` with `unitCost` (GC: row value 0, average untouched) · adjust → `adjust` by (target − live balance), unchanged skipped. `sourceType: "store_receipt"`. Once only. |
| `POST /api/store-receipts/:id/print` | `materialRequisition:print` | Audit entry + `{ unitCostByProduct }` (2026-09-23g) — unit cost per productId for the slip's หน่วยละ/รวม columns: the cost actually posted if the receipt is posted, otherwise the cost posting would use. `POST /api/material-requisitions/:id/print` returns the same field alongside `ok: true` (issued = posted issue cost, not yet issued = current average). |
