...
| Method & Path | Auth | Notes |
|---|---|---|
| `GET /api/scope-of-works?quotationId=` | `scopeOfWork:view` | Lists (summary shape only) the non-deleted Scope of Work records for one quotation, newest-updated first. `400` if `quotationId` is missing. |
...
| `POST /api/scope-of-works/:id/print` | `scopeOfWork:print` | Writes a `"Scope of Work Printed"` audit entry only — called by the client immediately before `window.print()`; never blocks printing on failure. |
