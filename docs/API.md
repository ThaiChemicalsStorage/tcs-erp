...
| Method & Path | Auth | Notes |
|---|---|---|
| `GET /api/company-profiles` | `companyProfiles:view` **or** `quotations:create` | Holders of `companyProfiles:view` get every profile, including archived/inactive ones (sorted default-first, then by Thai name) — the list page filters client-side, same "show archived" toggle convention as `ProductList.tsx`. Callers who only have `quotations:create` get a **server-filtered, active-and-not-deleted-only** list — this relaxed read access predates the (now-reverted) Quotation-form issuer selector and is kept as-is; **Company Profiles are no longer read anywhere in the Quotation create/edit flow** as of 2026-07-14, see the Quotations section below and [MODULES/CompanyProfiles.md](./MODULES/CompanyProfiles.md) "Correction (2026-07-14)." No pagination/server-side projection yet (flagged Medium by a 2026-07-13 Codex review, acceptable at today's real scale — see TODO.md). |
...
## Customers (`api/_lib/customersHandler.ts`, mounted at `/api/customers` — added 2026-07-14)

**Shares the `company-profiles` serverless function file** rather than getting its own — see
"Routing mechanics" above and [ARCHITECTURE.md](./ARCHITECTURE.md) "Serverless function count."

| Method & Path | Auth | Notes |
|---|---|---|
| `GET /api/customers` | `customers:view` **or** `quotations:create` | Same "manage vs. pick-for-a-quotation" carve-out as `GET /api/company-profiles`: `customers:view` holders get every customer including archived/inactive (sorted by `companyName`); a `quotations:create`-only caller (e.g. Sales User) gets a server-filtered active-and-not-deleted-only list — this is what feeds the Quotation form's Customer selector (`src/pages/quotation/CustomerSelector.tsx`). |
