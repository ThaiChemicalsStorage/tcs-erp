...
- ✅ **[2026-07-09] Production-readiness pass**: branding, real Dashboard, MongoDB schema prep, dead-code removal, Thai/English i18n infrastructure:
  ...
  - New `src/lib/i18n.tsx` (Thai/English translation context, `localStorage`-persisted, toggle in Settings → Profile) — covers only strings this pass touched (Dashboard, empty states, the toggle itself); the rest of the app's existing Thai UI is unchanged, tracked as a follow-up in [TODO.md](./TODO.md)
...
- Full app-wide Thai/English translation — the 2026-07-09 pass built the i18n infrastructure and toggle but only translated strings it directly touched (Dashboard, empty states); the rest of the app's Thai UI (sidebar nav, every other page) is still hardcoded Thai
...
- `i18n.tsx` only covers strings the 2026-07-09 pass touched — most of the app's Thai UI is still hardcoded, not translatable via the new language toggle.
