...
- ✅ **[2026-09-24] งานสโตร์ 8 ข้อ (รอบสอง).** ใบรับสินค้า: ประเภทราคา ไม่มี/แยก/รวม VAT · สรุปยอดแบบใบเสนอราคา · ส่วนลดท้ายบิล
  (หักหนี้และต้นทุนสต๊อก ไม่พิมพ์) · เครดิต/ครบกำหนด · ผู้ออกบิลกรอกเองได้ (เป็นเจ้าหนี้แทนผู้ขาย) · ใบเบิกจ่ายเกินที่ขอได้ · ใบเบิกที่อนุมัติแล้ว
  ขึ้นแท็บ "ใบเบิกจากแผนก" ในหน้าสโตร์พร้อมปุ่มทำใบจ่าย · ส่งออก Excel/PDF หน้าสต๊อก ประวัติสต๊อก การ์ดสต๊อก · **กู้รหัสผ่านผ่าน Super Admin**
  (หน้าคำขอใหม่ + บังคับตั้งรหัสใหม่) · 934 เทสต์ผ่าน · ยังไม่ได้กดทดสอบบนเบราว์เซอร์ (ดู TODO) · ขยายโมดูลเดิม ตัวเลข ~42% / ~98% ไม่ขยับ

- ✅ **[2026-09-23] งานสโตร์ 6 ข้อ.** ใบขอซื้อแยกรหัสฝ่าย PR/FD/ED/SD (รันเลขแยก) · หน้าประวัติสต๊อกแยก ค้นได้ว่าของไปงานไหน/มาจากไหน ·
  นำเข้ายอดสต๊อกจาก Excel · ใบรับสินค้าใบเปล่า + รหัสรับเข้า RR/RX/RI (เป็นประเภทหนี้ด้วย) · ใบเบิกของสโตร์ 15 รหัสจ่าย +
  ใบรับคืน/รับเข้าคลัง 15 รหัสรับ (คอลเลกชันใหม่ `store_receipts`) · สลับลำดับเมนูใบขอซื้อของสโตร์ · 898 เทสต์ผ่าน ·
  ยังไม่ได้กดทดสอบบนเบราว์เซอร์ (ดู TODO) · เป็นงานขยายโมดูลสโตร์/คลังที่มีอยู่ ตัวเลข ~42% / ~98% ไม่ขยับ

- ✅ **[2026-09-14] ลบทุกอย่างที่เกี่ยวกับ Vercel ออกจากรีโป.** เจ้าของสั่ง "ลบให้หมดทุกอย่างจริงๆที่เกี่ยวกับ
  vercel": `vercel.json` ถูกลบ, ถอด `@vercel/node` (handler ใช้ `ApiRequest`/`ApiResponse` จาก
  `api/_lib/httpTypes.ts` แทน หน้าตาเดิม ไม่มีพฤติกรรมเปลี่ยน), `server/env.ts` อ่านแค่ `.env`,
  ตัดเช็ค `VERCEL_ENV` ใน `api/_lib/mongodb.ts` และเอกสารทุกไฟล์เลิกพูดถึงเพดาน 12 function —
  เส้นทาง API มีที่เดียวคือ `API_ROUTES` ใน `server/app.ts` ดู CHANGELOG.md 2026-09-14 และ
  [SERVER_MIGRATION_PLAN.md](./SERVER_MIGRATION_PLAN.md) step H
- ✅ **[2026-09-11] คู่มือการใช้งานตามระบบทัน — 23 → 30 บท และถ่ายภาพหน้าจอใหม่ทั้งเล่ม 48 ภาพ.**
  `public/manual.html` ค้างอยู่ที่ฉบับ 28 ส.ค. ส่วนภาพส่วนใหญ่เป็นของ 7 ส.ค. · ทั้งเจ็ดโมดูลที่ขึ้นระบบ
  ระหว่างนั้นไม่เคยถูกเขียนถึงเลย (เอกสารรออนุมัติ · ทะเบียนภาษีซื้อ/เจ้าหนี้ · ทะเบียนผู้ขาย/ทะเบียนรหัส ·
  Cost Control · ใบรับสินค้า · ตัดของตามใบเบิก · เครื่องมือประจำทีม) และบทเดิม 12 บทเขียนผิดไปแล้ว
  (สต๊อกตัดตอนอนุมัติ · ไม่มีขั้นสโตร์เช็คของ · เลขเอกสารรูปแบบเก่า) · เลขบทถูกเลื่อนแบบ monotonic
  ลำดับ section จึงไม่ต้องสลับที่ · ภาพถ่ายจากสแตกจริงด้วย Playwright ที่ viewport เดิม 1536×782 ·
  **5 หน้าที่ฐานข้อมูล dev ยังว่างเขียนเป็นข้อความแทนการใส่ภาพเปล่า** ไม่มีการสร้างข้อมูลปลอม ·
  เรนเดอร์ PDF ใหม่ได้ 71 หน้า ตรวจภาพ ลิงก์ และลำดับบทในเบราว์เซอร์จริงแล้ว
  ดู CHANGELOG.md 2026-09-11f
- ✅ **[2026-09-10] กล่องงานเข้าของจัดซื้อกรองตามขั้นของใบ และเมนูกล่องงานไม่เปิดกว้างเกินหน้าที่.**
  กล่อง "ใบขอซื้อ (ทุกฝ่าย)" มีแถบกรอง ถึงคิวจัดซื้อ (ตั้งต้น) / ยังอยู่ที่สโตร์ / ทั้งหมด พร้อมตัวนับ ·
  เมนูกล่องงานของสโตร์และของจัดซื้อต้องมีสองสิทธิ์แล้ว ผู้ขอซื้อธรรมดาจึงไม่เห็นกล่องงานของอีกสองฝ่าย
  ไม่มีสิทธิ์ใหม่และไม่ต้องติ๊กอะไรเพิ่ม ดู CHANGELOG.md 2026-09-10e
- ✅ **[2026-09-10] ไล่ flow ใบขอซื้อทั้งสายด้วยผู้ใช้จริง 4 บทบาท และแก้บั๊กที่จัดซื้อไปต่อไม่ได้.**
  กล่องเลือกใบขอซื้อต้นทางตอนสร้างใบสั่งซื้อดึงเฉพาะฝ่ายโครงการกับฝ่ายผลิต ตกใบของฝ่ายที่ไม่มีเอกสาร
  ต้นทาง (`ownerDepartment: "general"`) ซึ่งเป็นทางเดียวที่สโตร์/เซอร์วิส/บัญชี/บุคคลเปิดใบขอซื้อได้ ·
  ใบพวกนั้นอนุมัติและผ่านสโตร์แล้วแต่ไม่เคยขึ้นให้จัดซื้อเลือกเลย · ขั้นอื่นของ flow ถูกต้องทั้งหมด
  ดู CHANGELOG.md 2026-09-10d และ [MODULES/Purchasing.md](./MODULES/Purchasing.md)
- ✅ **[2026-09-10] เมนู "โครงการ" ออกจากแถบข้าง และแถบข้างเรียงตามลำดับที่ประกาศไว้.**
  เจ้าของแจ้งว่าไม่ได้ใช้หน้าโครงการ และเมนูกลุ่มผลิตเรียงมั่ว · ตัวเรนเดอร์แถบข้างใช้ลำดับของ
  `navItems` (ลำดับที่โมดูลถูกสร้าง) แทนลำดับใน `NAV_GROUPS` ที่ตั้งใจไว้ตั้งแต่แรก · กลุ่มผลิตตอนนี้
  เป็น ใบสั่งผลิต → ใบเบิก → เทมเพลตใบเบิก → ใบขอซื้อ → ใบส่งมอบ · **หน้าโครงการยังไม่ได้ถูกลบทิ้ง**
  ยังเปิดได้จากปุ่มในใบเสนอราคา/Scope of Work และผลค้นหา ดู CHANGELOG.md 2026-09-10c
- ✅ **[2026-09-10] ใบเบิกเปิดเป็นใบเปล่าได้ ไม่ต้องอิงงานไหนเลย.**
  ปุ่ม "เปิดใบเปล่า" ข้างปุ่มสร้างใบเบิก ทั้งเมนูฝ่ายโครงการและฝ่ายผลิต · ใบที่ได้ไม่มีโครงการ ไม่มี
  รหัสงาน ไม่มีใบสั่งผลิต พิมพ์บรรทัดเองทั้งใบ สำหรับของที่เบิกไปใช้กับงานซ่อมบำรุงหรืองานภายใน ·
  แผนกของใบมาจากเมนูที่กดสร้าง ไม่ใช่ค่าที่สามแบบใบขอซื้อ ใบจึงไม่หายไปจากรายการของคนที่เพิ่งเปิด
  ดู CHANGELOG.md 2026-09-10b และ [MODULES/Project.md](./MODULES/Project.md)
- ✅ **[2026-09-10] หน้าตัดของของสโตร์ — คิวใบเบิกที่รอจ่าย รวมสองฝ่ายไว้ที่เดียว.**
  เมนู "ตัดของตามใบเบิก" ในกลุ่มคลังสินค้า แสดงใบที่อนุมัติแล้วและยังจ่ายไม่ครบของ**ทั้งฝ่ายโครงการ
  และฝ่ายผลิต** เรียงใบที่รอนานที่สุดขึ้นก่อน · กดแถวแล้ว**จ่ายจบได้ในหน้านั้น ไม่ต้องเปิดเข้าไปในใบ**
  จ่ายครบแล้วใบหลุดจากคิวเอง · ยิง `POST /:id/issues` ตัวเดียวกับการ์ดในใบเบิก ไม่ใช่ตัวเลขคนละชุด ·
  กล่องนี้เปิดด้วย `stock:adjust` และไม่กรองด้วยความเป็นเจ้าของใบ (เหตุผลเดียวกับกล่องเอกสารรออนุมัติ)
  ส่วนรายการใบเบิกปกติของสองฝ่ายยังกรองเหมือนเดิม · **ไม่มีสิทธิ์ใหม่ ไม่ต้องติ๊กอะไรเพิ่ม**
  ดู CHANGELOG.md 2026-09-10 และ [MODULES/Store.md](./MODULES/Store.md)
- ✅ **[2026-09-09] จัดการหมวดหมู่สินค้าได้จากเมนูของตัวเอง และสร้างหมวดใหม่ได้ตอนตั้งรหัสสินค้า.**
  เดิมตัวจัดการหมวดหมู่ซ่อนอยู่หลังปุ่มในหน้าสินค้า · ตอนนี้เป็นเมนู "หมวดหมู่สินค้า" ในกลุ่มคลังสินค้า
  โชว์จำนวนสินค้าต่อหมวด และกล่องอนุมัติคำขอเพิ่มสินค้าพิมพ์ชื่อหมวดใหม่ได้เลย โดยไม่ต้องออกจากงาน
  ที่ทำอยู่ · ชื่อซ้ำใช้หมวดเดิม ไม่สร้างซ้ำ

- ✅ **[2026-09-09] สินค้าที่สโตร์เพิ่งตั้งรหัสขึ้นในทุกเอกสารทันที.**
  ตัวเลือกสินค้าเคยกรองด้วยลิสต์ **ชื่อหมวด 4 ชื่อที่เขียนตายไว้ในโค้ด** สินค้าที่ลงหมวดอื่นจึงหายจาก
  ใบเบิก/ใบขอซื้อ/เทมเพลตใบเบิกอย่างถาวร (และหมวดคลังก็หายจากใบเสนอราคาเพราะกรองกลับทาง) ·
  การกรองย้ายมาเป็นช่องเลือกหมวดที่คนมองเห็นในตัวเลือกร่วมตัวเดียว หมวดคลังยังขึ้นก่อนแต่ไม่ซ่อนอะไร ·
  บรรทัดใบขอซื้อถูก **เติมรหัสกลับให้เอง** เมื่อสโตร์ตั้งรหัสเสร็จ ไม่ต้องลบแล้วเพิ่มใหม่ ·
  ขอรหัสสินค้าใหม่ได้จากในตัวเลือกของใบเบิกด้วย

- ✅ **[2026-09-09] ใบขอซื้อเดินผ่านสโตร์ก่อนถึงจัดซื้อ.**
  อนุมัติ → สโตร์เช็คของ → มีของ = จ่ายจากใบขอซื้อเลย (ตัดสต๊อกจริง จ่ายเป็นรอบ ยกเลิกรอบล่าสุดได้) /
  ไม่มี = ส่งต่อจัดซื้อ · ออกใบสั่งซื้อระหว่างรอสโตร์ไม่ได้ และบรรทัดที่จ่ายจากสต๊อกแล้วไม่ถูกลอกไป PO ·
  **ไม่ได้เพิ่มสถานะที่ 4** ให้เครื่องอนุมัติร่วม — เก็บเป็นฟิลด์ของใบขอซื้อเอง ใบเก่าวิ่งตรงไปจัดซื้อเหมือนเดิม

- ✅ **[2026-09-09] ฝ่ายจัดซื้อแก้ใบขอซื้อที่อนุมัติแล้วได้ทุกช่อง** ด้วยสิทธิ์ใหม่
  `purchaseRequest:editApproved` พร้อมประวัติการแก้บนเอกสาร audit log และแจ้งเตือนผู้สร้างใบ ·
  ระหว่าง "รออนุมัติ" ยังล็อกทุกคนตามเดิม

- ✅ **[2026-09-09] ของที่คืนเข้าคลังลงบัญชีด้วยราคาซื้อล่าสุด** (`Product.lastCost`) และโชว์ให้เห็น
  ก่อนกดบันทึกทั้งในใบเบิกและหน้าสต๊อก · มูลค่าสต๊อกรวมยังคิดแบบถัวเฉลี่ยเหมือนเดิม
...
- ✅ **[2026-09-04] นำเข้าสินค้าจากไฟล์ Excel ที่หน้าคลังสินค้า.**
  ...
  หัวไทยและอังกฤษ และหัวตารางไม่ต้องอยู่บรรทัดแรก · มีขั้นดูก่อนยืนยันหนึ่งจังหวะที่บอกว่าจะสร้างกี่
  รายการ ข้ามกี่รายการ จะสร้างหมวดหมู่ใหม่อะไร และแถวไหนตกเพราะอะไรพร้อมเลขแถว · **รหัสที่มีอยู่
  แล้วถูกข้าม ไม่เขียนทับ** และเทียบแบบไม่สนตัวพิมพ์ · หมวดหมู่จับคู่/สร้างจากชื่อ · **`stockQty` และ
  `avgCost` เป็น 0 เสมอ** นำเข้าตั้งยอดสต๊อกตั้งต้นไม่ได้ เพราะสต๊อกเปลี่ยนผ่าน `applyStockMovement()`
  ทางเดียว · มีปุ่มดาวน์โหลดไฟล์ตัวอย่างที่สร้างจากค่าคงที่ชุดเดียวกับตัวแกะ · เทสต์ใหม่ 28 ข้อ
  (ตัวแกะ 16 + เส้นทางจริงบนฐานข้อมูลในหน่วยความจำ 12) รวมทั้งชุด 631 ข้อผ่าน · **✅ กดจริงใน
  เบราว์เซอร์แล้ว** ด้วยไฟล์ที่ลอกรูปร่างไฟล์ส่งออกจริง ดู CHANGELOG.md 2026-09-04b และ
  [MODULES/Product.md](./MODULES/Product.md) "Excel import"
- ✅ **[2026-09-03] แผนกสโตร์ — ใบรับสินค้า + ตั้งหนี้ · ใบเบิกตัดตอนจ่ายจริง · เครื่องมือประจำทีม.**
  เจ้าของส่งรายการ 8 ข้อชุดเดียวและระบุว่าเป็นของ **แผนกสโตร์** · โมดูลใหม่ **ใบรับสินค้า (RR)**
  หนึ่งใบต่อหนึ่งใบสั่งซื้อ (บังคับด้วย unique partial index) รับได้หลายรอบในใบเดียว และการกดรับของ
  หนึ่งครั้งได้ทั้งสต๊อกพร้อมต้นทุน ทะเบียนเจ้าหนี้ และทะเบียนภาษีซื้อ — ไม่ต้องคีย์ซ้ำที่บัญชี ·
  **ใบเบิกกลับคำจาก 2026-09-02**: ตัดสต๊อกตอนสโตร์จ่ายของจริง (จ่ายบางส่วนได้) แทนตอนอนุมัติ
  พร้อมยอดค้างเบิกและการตัดของเข้าแผนก/ทีม/ประเภทงาน · **สต๊อกมีมูลค่าแล้ว** (ต้นทุนถัวเฉลี่ย
  เคลื่อนที่ + การ์ดสต๊อกพิมพ์ได้) · **เครื่องมือประจำทีม** อ่านจากบัญชีสต๊อกเดิม ไม่มี collection
  ของตัวเอง · **เลขเอกสารภายในทุกใบ** เป็น `{PREFIX}-{YYYYMM}-{NNNN}` รันใหม่ทุกเดือน ·
  ใบส่งมอบสินค้าแนบไฟล์ได้ · สิทธิ์ใหม่ 9 ตัวพร้อม migration `store-ap-permissions-2026-09-03` ·
  เทสต์ใหม่ 47 ข้อ รวมทั้งชุด 594 ข้อผ่าน · **✅ กดจริงในเบราว์เซอร์แล้ว 2026-09-03c** บนสแตกแยก
  พอร์ต + ฐานข้อมูลชั่วคราว เจอและแก้บั๊กชั้นหน้าจอสองข้อ (หน้าสต๊อกและการ์ดจ่ายเครื่องมือโชว์ยอดเก่า)
  · **✅ ตรวจใบพิมพ์เป็น PDF จริงและสลับภาษา EN แล้ว 2026-09-04** เจอและแก้อีกสองบั๊กที่มองไม่เห็น
  ใน DOM (ทะเบียนภาษีซื้อพิมพ์แล้วหายสามคอลัมน์เงิน · แถบตั้งบนหัวจดหมายล้นขอบทำให้ขอบขวาของหน้าถูกตัด)
  · **ยังเหลือ**: ใบพิมพ์ RR ยังเป็นฟอร์มชั่วคราวรอฟอร์มจริงจากเจ้าของ ดู [TODO.md](./TODO.md)
  ดู [MODULES/Store.md](./MODULES/Store.md)
- ✅ **[2026-08-31i] Cost Control ไปพร้อมกับ Scope of Work ตอนส่งให้คนอื่น.**
  เจ้าของสั่งว่า *"Scope of work เวลาที่จะส่งไปให้คนอื่นอะมันจะมาพร้อมกับ Cost control ด้วย"* และเลือก
  ให้ทำแบบ**ส่งในระบบ** · เพิ่ม FK `costControl.scopeOfWorkId` ข้าง ๆ `jobOrder` เดิม (ข้อความยังอยู่
  เพราะเป็นสิ่งที่คนพิมพ์และไฟล์ Excel พามา) · **งานหลักไม่ใช่ FK แต่เป็นทางมองเห็นทางที่สามของโมดูล** —
  ก่อนหน้านี้ Cost Control เห็นได้แค่ "เป็นคนสร้าง" หรือ `:viewAll` ผูกเฉย ๆ จึงไม่เปลี่ยนอะไรให้คน
  ที่เจ้าของเรียกว่า "คนอื่น" เลย · ตอนนี้ผู้รับเอกสารของ Scope เห็นใบต้นทุนที่ผูกอยู่ทั้งในรายการและ
  Global Search **ดูและพิมพ์ได้ แก้ไม่ได้** (`assertNotScopeRecipientOnly()` กันแม้บทบาทจะถือ
  `:finalize`) · ปุ่ม "สร้าง/เปิด Cost Control" บนแถบเครื่องมือของ Scope (ไม่มีด่านสถานะ) และช่อง
  "ผูกกับ Scope of Work" ที่หัวใบ Cost Control สำหรับใบที่เกิดจากการโยนไฟล์ · ปิดรายการที่ `TODO.md`
  ค้างไว้ตั้งแต่ 2026-08-28e พร้อมคำตอบเรื่อง `-R1` (ลิงก์ไม่ตามไปหาฉบับแก้ไข ตรงกับใบส่งมอบ/โครงการ)
  · เทสต์ใหม่ 10 ข้อ (`tests/api/costControlScopeLink.test.ts`) · ⚠️ **เจ้าของต้องเปิดสิทธิ์
  `costControl:view` ให้บทบาทที่ควรเห็นเอง** — งานนี้ไม่แจกสิทธิ์ให้ใคร
- ✅ **[2026-08-31g] สามช่องที่แคบกว่างานจริง: เงื่อนไขชำระเงิน · เลขใบเสนอราคา/PO · ราคาใน Cost Control.**
  งานแก้รูปทรงของข้อมูล ไม่ใช่โมดูลใหม่ · **(1)** "เงื่อนไขการชำระเงิน" ในใบเสนอราคาพิมพ์เองได้แล้ว
  (`Combobox` แทน `<select>` 4 ตัวเลือก) — ฝั่งเซิร์ฟเวอร์รับข้อความอิสระมาตลอด ข้อจำกัดอยู่ที่ UI
  อย่างเดียว จึงไม่ต้องแก้ schema/API/validation/สิทธิ์เลย และเก็บบั๊กเดิมที่เทมเพลตหลายบรรทัดถูก
  `<select>` กลืนไปด้วย · **(2)** Scope of Work หนึ่งใบใส่เลขใบเสนอราคาและเลข PO ได้หลายเลข
  (`additionalQuotationNumbers`/`additionalPoNumbers` + ปุ่ม "+ เพิ่ม") เก็บแบบ "เลขหลัก + เลขเพิ่มเติม"
  เพื่อไม่ให้ KPI "ยังไม่มี PO" และปุ่มดึงข้อมูลจากใบเสนอราคาพังเงียบ ๆ · ครบทั้งใบพิมพ์ หน้ารายการ
  ค้นหา สรุปการแก้ไข และใบวางบิล AR · **(3)** Cost Control ไม่ดึงราคาจากไฟล์ Excel อีกต่อไป
  (`stripImportedPrices()` ตัดที่ชั้น dialog เพื่อรักษาการตรวจ "แกะรายการมาไม่ครบ" ไว้) — และชั่วโมงถัดมา
  **[2026-08-31h] ถอดบล็อกกำไรของ Cost Control ออกทั้งโมดูล** ตามที่เจ้าของสั่ง ("เอาออก"): ค่าดำเนินการ/
  Bubble/Entertainment/ราคาขาย/กำไร/คิดเป็น% หายไปจากหน้าเอกสาร ใบพิมพ์ และหน้ารายการ — และเมื่อ
  เจ้าของทักว่ายังเหลือยอดต้นทุนรวมอยู่ ก็ถอดออกอีกชั้น **ตอนนี้ไม่มีการรวมยอดที่ไหนเลย** · ใบพิมพ์จึงต่างจากฟอร์ม FM-SL-06 ตรงจุดนี้จุดเดียวโดยตั้งใจ · ข้อมูลเก่าใน MongoDB ไม่ถูกลบ ·
  เทสต์ 518 ข้อผ่านทั้งหมด (เพิ่มใหม่ 23 · มีไฟล์ใหม่ tests/api/scopeOfWorkNumbers.test.ts) · See CHANGELOG.md 2026-08-31g.

- ✅ **[2026-08-28e] Cost Control (แผนก BD) — โยนไฟล์ Excel ของงานแล้วได้เอกสารที่แก้/พิมพ์ได้.**
  ปิดช่องว่างที่จดไว้ 5 ที่ตั้งแต่ 2026-08-20 ว่า "Cost Control สร้างไม่ได้เพราะยังไม่มีใครนิยาม" —
  คำตอบคือไฟล์ Excel ที่บริษัทกรอกมืออยู่แล้ว โมดูลนี้ถอดแบบจากไฟล์จริงทั้งโครงข้อมูลและใบพิมพ์
  สร้างได้ทั้งจากการโยนไฟล์ (เบราว์เซอร์แกะเอง ผ่านหน้า preview ที่แก้ได้เสมอ) และเปิดใบเปล่า ·
  ชนิดแถวอ่านจากสีพื้นหลังของเซลล์ · ไม่เก็บยอดรวมลงฐานข้อมูลเลย · **ใบพิมพ์เป็นฟอร์มจริง
  FM-SL-06 Rev.02** ต่างจากใบสั่งซื้อที่ยังเป็นโครง · กลุ่มเมนูที่ 10 "BD" · 7 สิทธิ์ · 33 เทสต์ใหม่
  (รวมโปรเจกต์ 395) · **เครื่องจริงต้องติ๊กสิทธิ์เอง** · See CHANGELOG.md 2026-08-28e.
...
- **Automated test coverage is partial** (was "none" until 2026-07-29): a first real vitest suite (55 tests, `tests/`, `npm test`) now covers quotation money math (incl. client/server formula parity), default-role RBAC grants + permission edge cases, the quotation workflow state machine + per-action authorization, ownership rules, revision-chain parsing/dedup, Scope of Work validation, and a full `/api/auth/login` integration test (bcrypt/JWT/rate limiting against an in-memory MongoDB) — and CI runs it on every push/PR (`.github/workflows/ci.yml`, notify-only: it doesn't block a deploy). Still uncovered: HTTP-level guards on every route other than `/api/auth/*`, products CRUD helpers, all UI components. See [TODO.md](./TODO.md).
...
- Company logo/stamp/profile-picture/signature images are stored as base64 data URLs (now inside MongoDB documents rather than `localStorage`), still capped at 1MB each client-side before upload — fine at current scale, but doesn't scale to a real object-storage (e.g. S3-compatible) approach; revisit if image volume/size grows.
