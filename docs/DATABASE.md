...
- `revenueByMonth`: groups won quotes by `$substr(issueDate, 0, 7)`, zero-filled over the last 12 calendar months. Respects the salesperson filter (fixed 2026-07-10 — the original version ignored it, an inconsistency caught by code review), deliberately not the date-range filter (a trend chart collapsed to one selected day defeats its purpose).
- `monthlyClosingRate` (added 2026-07-10): win rate (`won / (won + lost)`) per calendar month, trailing 12 months, same zero-fill treatment and same salesperson-filter/not-date-filter behavior as `revenueByMonth`. `winRate` is `null` (not `0`) for a month with no won/lost deals — a real 0% month (deals that all lost) must still render, not be hidden behind an empty state.
