...
| `users` | MongoDB `ObjectId` | server-only `UserFields` (see below) | Includes `passwordHash` — never sent to the client. `toPublicUser()` (`api/_lib/collections.ts`) strips it before any response. `mustChangePassword` (2026-09-24) = signed in with a Super-Admin-issued temporary password; cleared when the user sets their own. |
...
| `password_reset_requests` | **Live (2026-09-24)** — one row per password-recovery request (`src/lib/passwordResets.ts`): user snapshot, typed identifier, note, `status` pending/resolved/dismissed, `requestCount`, who resolved it. **No password is ever stored here.** | Yes | `{ userId: 1, status: 1 }` (lazy, `passwordResetHandler.ts`) |
| `password_reset_attempts` | **Live (2026-09-24)** — `{ ip, createdAt: Date }` per forgot-password call, for the 5/IP/15-min limit; separate from `login_attempts` so it does not count against login lockouts. | Yes | TTL `{ createdAt: 1 }` (900 s), `{ ip: 1, createdAt: 1 }` (lazy) |
