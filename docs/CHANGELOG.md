...
## 2026-08-31c (absolute latest) — ใบขอซื้อคายช่องที่ไม่ใช่ของตัวเองออก ใบสั่งซื้อได้ส่วนลดกับรายละเอียดที่ขาด

ขั้นที่ 3–4 ของงานจัดซื้อที่ค้างจากวันศุกร์ ทั้งสามข้อมาจากคำสั่งชุดเดียวกันเมื่อ 2026-08-28

### ใบขอซื้อ: ถอดผู้จำหน่าย / โทร. / เครดิต / ขนส่งโดย

เจ้าของสั่งไว้ตรง ๆ ว่า *"ใบขอซื้อไม่ต้องมีผู้จำหน่าย เครดิต ขนส่งโดย"* และใบจริงที่กรอกแล้ว
(`ED6908038.pdf`) ก็ยืนยัน — **ช่องพวกนั้นมีอยู่บนกระดาษแต่เว้นว่างไว้ทั้งหมด** เพราะคนขอซื้อไม่ใช่
คนกรอก ฝ่ายจัดซื้อกรอกทีหลังตอนออกใบสั่งซื้อ

`vendorPhone` ที่เพิ่งเพิ่มเข้าไปเองเมื่อ 2026-08-31 (รอบทาบฟอร์ม FM-PU-05) ถูกถอดคืนด้วย —
มันอยู่ในกลุ่มเดียวกับ "ผู้จำหน่าย" ที่เจ้าของบอกว่าไม่ต้องมี ตอนนั้นทำเพราะเห็นว่ากระดาษมีช่อง
โดยไม่ได้เปิด TODO ดูก่อนว่าเจ้าของสั่งตรงข้ามไว้แล้ว

**การถอดไม่ใช่แค่ลบช่องบนหน้าจอ** — สามช่องนั้นคือสิ่งที่ `purchaseOrderHandler.handleCreate()`
ก๊อปจากใบขอซื้อไปใบสั่งซื้อพอดี และมีเทสต์ตรึงไว้ (`purchasing.test.ts:161`) ตอนนี้ใบสั่งซื้อ
เริ่มด้วยผู้ขายว่างเสมอ แล้วฝ่ายจัดซื้อเลือกเองจาก**ทะเบียนผู้ขาย** ที่เพิ่งสร้างเมื่อเช้า ซึ่งให้ข้อมูล
ครบกว่าที่ใบขอซื้อเคยส่งต่อได้ (ผู้ติดต่อ/โทร/เลขภาษี/ที่อยู่)

`searchDocuments.ts` เคยใช้ `vendorName` เป็นคอลัมน์ "party" ของผลค้นหาใบขอซื้อ — เปลี่ยนเป็น
**ผู้ขอซื้อ** (`requestedBy`) ซึ่งเป็นสิ่งที่คนหาใบขอซื้อนึกออกจริง (รหัสงานอยู่คอลัมน์ `lineage` แล้ว)
และเอา `vendorName`/`shippingMethod` ออกจากช่องที่ค้นได้

ค่าที่เอกสารเก่าเคยบันทึกไว้**ยังอยู่ในฐานข้อมูล** ไม่ได้ลบ แค่ไม่มีใครอ่านอีกแล้ว — ถ้าภายหลังกลับลำ
ข้อมูลยังกู้ได้

### ใบสั่งซื้อ: ส่วนลดรายบรรทัดและท้ายใบ ทั้ง % และจำนวนเงิน

`PurchaseOrderLine.discount`/`discountMode` และคู่เดียวกันบนหัวเอกสาร — รูปร่างเดียวกับ
`QuoteLine`/`Quote` เป๊ะ ๆ ทั้งคู่ optional ไม่มีค่า = ไม่มีส่วนลด เอกสารเก่าอ่านออกมาเหมือนเดิม

**เรื่องสำคัญที่คอมเมนต์เดิมในไฟล์เตือนไว้ถูกแล้ว**: `purchaseOrderSubtotal()` เคยเขียนกำกับตัวเองว่า
จงใจไม่ใช้ `quoteMath.ts` เพราะ *"ใบสั่งซื้อยังไม่รู้ว่าต้องใช้ส่วนลดแบบไหน"* ตอนนี้รู้แล้ว จึงยืม
**เฉพาะตัวคิดส่วนลด** (`resolveDiscountAmount`/`lineDiscountAmount`/`lineSubtotal`) มาใช้ —
แต่ **ยังใช้ `computeTotals()` ไม่ได้** เพราะมันฮาร์ดโค้ด `VAT_RATE = 7` ขณะที่ใบสั่งซื้อมี `vatRate`
ที่แก้เองได้ต่อใบ (ซื้อจากผู้ขายที่ไม่จด VAT ก็มี) จึงมี `purchaseOrderTotals()` ของตัวเอง

`tests/purchaseOrderTotals.test.ts` (9 ข้อ) ตรึงเรื่องนี้ไว้โดยเฉพาะ: ใบที่ `vatRate: 10` ต้องได้
ภาษี 100 ไม่ใช่ 70 และต้อง**ไม่ตรง**กับสิ่งที่ `computeTotals()` จะให้ ถ้าใครเผลอสลับไปเรียกตัวนั้น
เพราะเห็นว่า "สูตรเหมือนกัน" เทสต์จะดัง

ผลพลอยได้: สูตรยอดรวมเคยถูกเขียนซ้ำสองที่ (หน้าแก้ไขกับใบพิมพ์) ตอนนี้ทั้งคู่เรียก `purchaseOrderTotals()`

บนใบพิมพ์ ส่วนลดพิมพ์**ตามที่กรอก** (`10%` หรือ `500`) ไม่ใช่ยอดที่คิดแล้ว — คนอ่านใบต้องเห็นเงื่อนไข
ที่ตกลงกัน ไม่ใช่แค่ผลลัพธ์ · แถวส่วนลดท้ายใบซ่อนเมื่อไม่มีส่วนลด

### ใบสั่งซื้อ: รายละเอียดต่อบรรทัดที่ดึงมาจากใบขอซื้อ

*"ใบสั่งซื้อให้มีรายละเอียดด้วยที่ดึงมาจากใบขอซื้อ"* — `subDetails` ก๊อปอยู่แล้ว ที่หายไปเงียบ ๆ คือ
`neededByDate` / `departmentCode` / `costCode` **ต่อบรรทัด** เพราะใบสั่งซื้อไม่มีที่เก็บ ตอนนี้มีแล้ว
ทั้งสามช่อง ไม่ต้อง migrate เพราะ `toClient()` normalize ตอนอ่านอยู่แล้ว

หมายเหตุ: `costCode` ยังเป็น**ฟิลด์ตาย**บนใบขอซื้อ (เก็บและ sanitize แต่ไม่มีช่องกรอก) จะได้ใช้จริง
ตอนทำทะเบียนรหัสในขั้นถัดไป

### การตรวจสอบ

`tsc` ทั้งสองชุด · `eslint` · เทสต์ 446 ข้อผ่าน (`purchaseOrderTotals` ใหม่ 9 · `purchasing` +4)

กดจริงในเบราว์เซอร์ **โดยบล็อกการเขียนไว้ก่อนเปิดหน้าแรก** (บทเรียนจากรอบเช้า): ใส่ส่วนลด 10%
บรรทัดแรก ยอดบรรทัดจาก 100.00 → 90.00 และยอดรวมลดลง 10 พอดี · ใส่ส่วนลดท้ายใบ 100 บาท
แล้วกดสลับเป็น ฿ — ภาษีเปลี่ยนจาก 97,684.30 เป็น 97,677.30 คือ **7% ของยอดหลังหักส่วนลดแล้ว**
ยืนยันว่าลำดับ ส่วนลด → ภาษี ถูกต้อง

---

## 2026-08-31b — ทะเบียนผู้ขาย, ช่องพิมพ์ที่มีตัวเลือก, และสิทธิ์ที่หายไปเงียบ ๆ สองโมดูล

เจ้าของขอดู TODO ของฝ่ายจัดซื้อที่วางแผนไว้ตั้งแต่วันศุกร์ 2026-08-28 แล้วยังไม่ได้ทำ วันนั้นสั่งมา
เป็นชุดใหญ่แล้วเลือกลำดับว่า *"Cost Control ก่อน"* — Cost Control เสร็จวันเดียวกัน ที่เหลือค้างไว้
รอบนี้เก็บสองข้อแรกตามลำดับที่วางไว้

### 🔴 สิ่งที่เจอตอนสำรวจ: เมนู "จัดซื้อ" กับ "BD" มองไม่เห็นมาตั้งแต่วันสร้าง

เช็คตาราง `roles` บนฐานข้อมูล dev จริงแล้วพบว่า **ทุก role มี `purchaseOrder:*` = 0 และ
`costControl:*` = 0** ทั้งสองโมดูลใส่สิทธิ์ไว้ใน `defaultRoles` ซึ่งมีผลกับ**การติดตั้งใหม่เท่านั้น**
`syncDefaultRoles()` ไม่เคยแก้ array สิทธิ์ของ role ที่มีอยู่แล้ว — กลุ่มเมนูจะซ่อนตัวเองเมื่อไม่มีสิทธิ์
ดูสักรายการ ผลคือ **สองกลุ่มเมนูที่เพิ่งสร้างเมื่อ 2026-08-28 ไม่เคยขึ้นให้ใครเห็นเลย** ยกเว้น
Super Admin ที่ได้ทุกสิทธิ์อัตโนมัติ จึงไม่มีใครสังเกต

เป็นบั๊กคลาสเดียวกับที่ `productionOrder` เจอเมื่อ 2026-08-20 และที่ `project:*` ยังเจออยู่ทุกวันนี้

เจ้าของ**เปลี่ยนการตัดสินใจจากที่เลือกไว้เมื่อ 2026-08-25** (เดิมว่าจะไปติ๊กเองไม่ต้องเขียน migration)
รายการ `purchasing-registers-permissions-2026-08-31` ใน `api/_lib/rbacSeed.ts` จึงแจกทั้งสามชุดพร้อมกัน
— `vendor:*` ของโมดูลใหม่ บวก `purchaseOrder:*` และ `costControl:*` ที่ค้างอยู่ — ให้ Administrator
**ไม่รวม `project:*` 28 ตัว** เพราะเจ้าของยังไม่ได้เปลี่ยนใจเรื่องนั้น (`TODO.md` เขียนกำกับไว้ว่าห้ามเพิ่มเอง)

ยืนยันแล้วว่า migration รันเองตอน dev server restart และเมนู "จัดซื้อ"/"BD" กลับมาเห็นได้จริง

### `Combobox` — ช่องที่พิมพ์เองได้ แต่พิมพ์ไม่กี่ตัวก็ขึ้นให้เลือก

เจ้าของขอไว้ว่า *"เวลาพิมพ์ชื่อลูกค้าของใบสั่งซื้อหรืออะไรต่างๆ ก็ตาม ให้พิมพ์แบบคีย์เวิร์ดในนั้นก็ขึ้นมา
เหมือนค้นหา แค่สามารถพิมพ์ตรงนั้นได้ปกติ"*

ทั้งแอปไม่มีคอมโพเนนต์แบบนี้เลย มี `role="combobox"` อยู่สองที่ และตัวที่ใกล้ที่สุด
(`CustomerSelector.tsx`) **ไม่ยอมให้พิมพ์อิสระ** — ข้อความที่พิมพ์เป็นแค่คำค้นในสเตทของตัวเอง
พอ blur แล้วหายไปเฉย ๆ ไม่เคยถูกส่งกลับให้ผู้เรียก

`src/components/Combobox.tsx` ประกอบจากสองที่: โครง/สไตล์/การปิดตอน blur จาก `CustomerSelector`
\+ คีย์บอร์ดและ ARIA จาก `GlobalSearch.tsx` (`aria-activedescendant`, ลูกศร, Enter, `scrollIntoView`)
สิ่งที่ต่างคือ **`value`/`onChange` เป็นข้อความอิสระจริง** ทุกตัวอักษรถูกส่งขึ้นทันที ดรอปดาวน์เป็น
ทางลัดเท่านั้น ปิดหรือ blur ไม่เคยทำให้สิ่งที่พิมพ์หาย

สองเรื่องที่ไม่ชัดจนกว่าจะลงมือ:

- **ดรอปดาวน์ต้อง portal ไป `document.body` แล้ววางด้วยพิกัด fixed** — ที่ใช้จริงหลายจุดเป็นช่องใน
  ตารางรายการซึ่งอยู่ใน `<div className="overflow-x-auto">` ดรอปดาวน์แบบ absolute โดนตัดหายทันที
  (`createPortal` มีที่ใช้อยู่แล้วใน `pages/dashboard/ApprovalDashboard.tsx`)
- **การบีบ `activeIndex` ต้องทำตอนเรนเดอร์ ไม่ใช่ใน `useEffect`** — eslint จับได้ถูกแล้วว่า setState
  ใน effect ทำให้เกิดการเรนเดอร์ซ้อน

### ทะเบียนผู้ขาย

`api/_lib/vendorsHandler.ts` + `vendorValidation.ts` + `src/pages/vendors/VendorsPage.tsx`
ลอกจาก `customersHandler`/`customerValidation`/`CustomersPage` ทั้งดุ้น (แม่แบบข้อมูลหลักที่ครบที่สุด
ในแอปนี้) แล้วเติมการเช็ครหัสซ้ำแบบ `departmentsHandler.ts` เข้าไป อยู่ในกลุ่มเมนู **จัดซื้อ**

**รหัสผู้ขายว่างได้ แต่ถ้ากรอกแล้วห้ามซ้ำ** สองชั้น: regex ไม่สนตัวพิมพ์ (ให้ error อ่านรู้เรื่อง) +
unique partial index (กันสองคนกดพร้อมกัน) `validateVendorDraft` แปลงเป็นตัวพิมพ์ใหญ่ก่อนเสมอ
ทั้งสองชั้นจึงเห็นค่าเดียวกันจริง — ต่างจาก `departmentsHandler.ts` ที่ regex ไม่สนตัวพิมพ์แต่ index สน
คือสองชั้นไม่ตรงกัน

**ใบสั่งซื้อยังเก็บ `vendorName` เป็นข้อความเหมือนเดิม** ทะเบียนเป็นทางลัด ไม่ใช่กำแพง — เอกสารเก่า
อ่านได้เหมือนเดิม ไม่ต้อง migrate และยังพิมพ์ชื่อผู้ขายที่ยังไม่ขึ้นทะเบียนได้ เลือกจากทะเบียนแล้วระบบ
เติมผู้ติดต่อ/โทร/เลขภาษี/ที่อยู่ให้ (ทับของเดิมโดยตั้งใจ — การกดเลือกคือการบอกว่าเอารายนี้ทั้งราย)

`GET /api/vendors` เปิดให้คนที่มี `purchaseOrder:view`/`purchaseRequest:view` อ่านได้ด้วย ไม่ใช่แค่
คนที่ดูแลทะเบียน ไม่งั้นดรอปดาวน์บนใบสั่งซื้อจะว่างเปล่าสำหรับคนที่ใช้งานจริง — การ์ฟเอาต์แบบเดียวกับ
ที่ลูกค้าทำไว้ให้คนออกใบเสนอราคา

### การตรวจสอบ

`tsc` ทั้งสองชุด · `eslint` · เทสต์ 433 ข้อผ่าน (`purchasing` +8, `rbacMigrations` +5 — ชุดหลัง
พิสูจน์ว่า migration คืนสิทธิ์ที่หายไปจริงบนฐานข้อมูลที่จำลองสภาพเครื่องจริง)

กดจริงในเบราว์เซอร์: เมนู "จัดซื้อ"/"BD" กลับมาเห็น · สร้างผู้ขายแล้วรหัสถูกเก็บเป็นตัวพิมพ์ใหญ่ ·
สร้างซ้ำด้วยตัวพิมพ์เล็กขึ้น *"รหัสผู้ขายนี้มีผู้ใช้งานแล้ว"* ในฟอร์ม ไม่ใช่หน้าขาว · บนใบสั่งซื้อพิมพ์
"ทดสอบ" แล้วขึ้น `TC-001 — บริษัท ทดสอบคอมโบ จำกัด` · ลูกศร+Enter เลือกแล้วเติมผู้ติดต่อ/เบอร์โทรให้ ·
พิมพ์ชื่อที่ไม่มีในทะเบียนแล้ว blur ข้อความยังอยู่

⚠️ **ระหว่างตรวจ ลืมบล็อกการเขียน** การพิมพ์ทดสอบจึงถูกบันทึกอัตโนมัติลง `PO-2569-0002` จริง
คืนค่า `vendorName`/`vendorContact`/`vendorPhone` กลับเป็น `""` แล้ว (ค่าที่ทุกใบสั่งซื้อใบอื่นและ
ใบขอซื้อต้นทางมีเหมือนกัน) ฟิลด์อื่นที่ผู้ใช้กรอกไว้ไม่ถูกแตะ — บทเรียนนี้อยู่ในบันทึกความจำแล้ว

---

## 2026-08-31 — ทาบใบพิมพ์ของฝ่ายผลิต 4 ใบกับฟอร์มกระดาษตัวจริง

เจ้าของวางไฟล์ตัวอย่างจริงไว้ใน `reference/company/` แล้วบอกสั้น ๆ ว่า *"เอาไว้ใช้เป็นตัวอย่าง
ช่วยปรับแก้ให้ที"* และ *"จะให้เอาไปปรับใช้กับเว็บในส่วนของแผนกผลิตนะ"* — สี่ไฟล์นั้นคือครั้งแรกที่
โค้ดได้เห็นฟอร์มจริงของเอกสารที่ก่อนหน้านี้สร้างจากการอนุมานล้วน ๆ:

| ไฟล์ | คือ | ปิดช่องว่างอะไร |
|---|---|---|
| `FM-PJ-01 ใบสั่งงาน Rev1 (1).pdf` + `(3).xlsx` | ฟอร์มเปล่า + กรอกแล้ว 3 ใบ | TODO "ทาบการจัดหัวข้อเช็คลิสต์ใบสั่งงานกับฟอร์ม FM-PJ-01 ตัวจริง" |
| `ED6908038.pdf` | ใบขอซื้อ FM-PU-05 Rev.02 ที่กรอก+เซ็นจริง | `PurchaseRequestPrintDocument.tsx` ที่ระบุตัวเองว่า "first-pass, not yet pixel-calibrated" |
| `ใบส่งมอบสินค้าและบริการ PQ202607-190-SC-SK …pdf` | **ผลลัพธ์ที่แอปพิมพ์เอง ไม่ใช่ฟอร์มกระดาษ** | บั๊กหน้าที่สองของ FM-SL-05 |
| `ใบสั่งผลิต(Production Order).pdf` | FM-PD-02 ที่กรอกจริง (ภาพสแกน) | `MODULES/Production.md` "print layout has not been compared against the physical form" |

`reference/` ถูก gitignore ไว้ ไฟล์เหล่านี้จึง commit ไม่ได้ ผังของแต่ละฟอร์มถูกถอดความลง doc
comment ของไฟล์ที่เกี่ยวข้องแทน ตามแบบที่ `productionOrder.ts` ทำไว้อยู่แล้ว — **นั่นคือบันทึกถาวร
อย่าคาดหวังว่าจะกลับไปเปิด PDF อ่านซ้ำได้**

### 1. ใบสั่งงาน: เช็คลิสต์ที่เคย "อนุมานเอง" ถูกยุบกลับตามกระดาษ

2026-08-27 เคยแตก 23 ตัวเลือกออกเป็น 6 หัวข้อ (แบบและการคำนวณ / วัสดุและงานผลิต / …) พร้อมเขียน
กำกับตัวเองไว้ตรง ๆ ว่าเป็น *"การอนุมานจากความหมาย ไม่ได้อ่านจากฟอร์มกระดาษจริง"* ฟอร์มจริงบอกว่า
**ไม่มีหัวข้อย่อยแม้แต่หัวข้อเดียว** เป็นรายการเรียงยาวสองคอลัมน์ ซ้าย 11 ขวา 12 จึงยุบกลับเป็น
กลุ่มเดียวไม่มีชื่อ พร้อม `SCOPE_COLUMN_SPLIT` บอกจุดตัดคอลัมน์ให้ใบพิมพ์ใช้

`key` ของทุกตัวเลือกไม่เปลี่ยนแม้แต่ตัวเดียว `withJobOrderChecklistGroups()` จึงอ่านเอกสารได้ทั้ง
สามรูปแบบที่เคยลงฐานข้อมูลจริง (กลุ่มเดียวแบบเดิม / 6 หัวข้อ / กลุ่มเดียวไม่มีหัวข้อ) โดยค่าที่ติ๊กไว้
ไม่หาย `tests/jobOrderChecklist.test.ts` ขยายจาก 9 เป็น 16 ข้อ คุมทั้งสองทิศทาง

บรรทัดงานสีสามบรรทัดบนกระดาษมีช่องกรอก **สองช่อง** (`PRIMER COAT : ____ / ____ MICRON`) ซึ่ง
`ChecklistOption` เดิมมีที่เก็บช่องเดียว จึงเพิ่ม `value2` และเพิ่ม `unit`/`unit2` สำหรับหน่วยที่พิมพ์
อยู่บนฟอร์มอยู่แล้ว (BAR / TON / MICRON) — สามฟิลด์นี้ optional ทั้งหมด Scope of Work ไม่ได้ตั้งค่า
จึงไม่กระทบ และ `unit`/`unit2` ถูกสร้างใหม่จากโค้ดทุกครั้งที่อ่าน ไม่เคยอ่านกลับจากฐานข้อมูล

### 2. ใบสั่งงาน: `รหัสงาน` เคยพิมพ์ผิดช่อง

`JobOrderPrintDocument.tsx` พิมพ์ `j.id` (เช่น `JO-2569-0006`) ลงช่อง "รหัสงาน" ตัวอย่างที่กรอกแล้ว
ทั้งสามใบเขียนเลข PQ ลงช่องนั้น (`PQ202305-087-VT-SK`) และ `JobOrder.jobCode` ก็เก็บค่านั้นอยู่แล้ว
— เป็นบั๊ก ไม่ใช่เรื่องหน้าตา แก้เป็น `j.jobCode`

### 3. ใบส่งมอบสินค้า: หน้าที่สองที่อ่านไม่ออกว่าเป็นของใคร

ไฟล์ที่เจ้าของพิมพ์ออกมาเองมี 4 หน้า งวดแรก (12 รายการ + 35 สเปค) ล้นเป็นสองหน้า และ**หน้าที่สอง
ไม่มีหัวจดหมาย ไม่มีบล็อก เรียน/เลขที่/วันที่/WORK ORDER และไม่มีหัวคอลัมน์เลย** `<thead>` ที่ควรจะ
ถูกพิมพ์ซ้ำไม่ได้ซ้ำ ซึ่ง `UI_GUIDELINES.md` §Print/PDF เตือนไว้อยู่แล้วว่า thead ที่สูงเกินไป Chromium
จะเลิกซ้ำเงียบ ๆ

แก้โดย**ประกอบหน้าเองในโค้ด** (`paginateItems()`) แทนการฝากความหวังไว้กับตัวตัดหน้าของเบราว์เซอร์:
หัวหน้ากระดาษถูกแยกเป็น `PageHead` แล้วเรนเดอร์ซ้ำทุกหน้า, รายการหนึ่งกับสเปคของมันไม่ถูกแยกคนละหน้า,
เลขลำดับเดินต่อข้ามหน้า, และ Remark กับช่องเซ็นอยู่เฉพาะหน้าสุดท้ายของงวด

ผลพลอยได้: `SINGLE_PAGE_ROW_TARGET` เคยคิดแถวเติมต่อ**งวด** ทำให้งวดที่ล้นหลายหน้าได้แถวเติมผิด
ตอนนี้เป็น `PAGE_ROW_CAPACITY` คิดต่อ**หน้า** ซึ่งเป็นหน่วยที่ถูกต้อง

### 4. ใบขอซื้อ: ช่องที่ฟอร์มมีแต่ระบบไม่เคยมีที่เก็บ

`ED6908038.pdf` มีช่องที่ระบบเก็บไม่ได้เลย 5 ช่อง เพิ่มเข้า `PurchaseRequest` ทั้งหมด (optional
บนดิสก์ `toClient()` เติม `""` ตอนอ่าน ไม่ได้ทำ migration): `issueDate` · `vendorPhone` ·
`deliveryContact` · `deliveryPhone` · `headerRemark`

หัวเรื่องมี `(ฝ่ายโครงการ)` กำกับ ซึ่งอ่านจาก `ownerDepartment` ได้ตรง ๆ เพราะฝ่ายผลิตกับฝ่ายโครงการ
ใช้ฟอร์มเดียวกันอยู่แล้ว

**คอลัมน์ที่ 7 เปลี่ยนจาก `ราคาประเมิน` เป็น `ให้ซื้อ` ที่เว้นว่าง** ตามกระดาษ ซึ่งเป็นช่องให้ฝ่ายจัดซื้อ
เขียนเอง ไม่ใช่ราคาของผู้ขอ — `estimatedCost` ยังกรอกและเก็บได้เหมือนเดิม แค่ไม่ถูกพิมพ์ ถ้าภายหลัง
ยืนยันว่าฝ่ายจัดซื้อใช้ราคาบนใบพิมพ์จริง ให้เพิ่มเป็นคอลัมน์ที่ 8 อย่าไปทับ "ให้ซื้อ"

`พิมพ์ครั้งที่ N` บนใบตัวอย่างมาจากซอฟต์แวร์ Express เดิม ระบบนี้เขียน audit log ตอนพิมพ์แต่ไม่เคย
อ่านจำนวนครั้งกลับมาฝั่งหน้าจอ จึงพิมพ์บรรทัดนั้นโดย**ละตัวเลขไว้** ไม่เดา

### 5. บรรทัดต่อที่มีจำนวนของตัวเอง (ใบสั่งงาน + ใบสั่งผลิต)

ฟอร์มจริงทั้งสองใบมีแถวที่ **ไม่มีเลขลำดับ แต่มีจำนวน/หน่วยของตัวเอง**:

- FM-PJ-01 `1 Flexible Joint` (ไม่มีจำนวน) แล้ว `Ø 650 | 15 | PCS`, `Ø 600 mm. | 2 | PCS` …
- FM-PD-02 `3 หน้าแปลน 20A | 2 ตัว` แล้ว `หน้าแปลน 50A | 3 ตัว`, `หน้าแปลน 100A | 1 ตัว`

`subDetails: string[]` เป็นข้อความล้วน จำนวนของแถวพวกนี้จึงหายไปทั้งหมด เพิ่ม `isContinuation?:
boolean` ที่ระดับ *line* แทนการเปลี่ยนชนิดของ `subDetails` — ใช้ตรรกะ "ข้ามตอนไล่เลข" ตัวเดียวกับ
`isSectionHeader` ที่ใบสั่งผลิตมีอยู่แล้ว ต่างกันแค่**ยังมี qty/unit ได้** เป็น boolean ที่ default
`false` เอกสารเก่าทุกใบจึงอ่านออกมาเหมือนเดิม ไม่ต้อง migrate ถ้าหน้าจอส่งมาทั้ง `isSectionHeader`
และ `isContinuation` **หัวข้อชนะ** เพราะเป็นชนิดที่ล้างค่ามากกว่า

### 6. เรื่องหน้าตาที่เจอตอนพิมพ์จริง

ทั้งสี่ใบถูกพิมพ์ออกมาเป็น PDF จริงแล้วเอาไปเทียบกับภาพฟอร์มกระดาษทีละส่วน สิ่งที่เจอตอนนั้น:

- **พื้นหลังของแอปติดไปบนกระดาษ** ทุกใบที่แตะรอบนี้ต้องประกาศ `background: "#fff"` เอง ไม่งั้นสีพื้น
  ของธีมไหลลงไปเต็มหน้าเมื่อผู้ใช้เปิด "Background graphics"
- **ขอบขวาของใบสั่งผลิตถูกตัด** ทั้งเส้นตารางและเลขที่ใบสั่งผลิต — เพิ่ม `EDGE_GUARD = "2px"`
  บทเรียนเดียวกับ commit `f1509da` ของ Cost Control
- **ช่องกรอกที่ยังว่างบนใบสั่งงานพิมพ์ออกมาเป็น `: BAR` และ `PRIMER COAT / MICRON` ลอย ๆ** —
  บนกระดาษช่องพวกนั้นเป็น*เส้นให้เขียน* ที่มีอยู่เสมอ จึงเรนเดอร์เส้นไว้ตลอดไม่ว่าจะกรอกแล้วหรือยัง
- **บล็อกขอบเขตงานถูกตัดกลางรายการ** เมื่อใบยาวเกินหน้า ใส่ `breakInside: "avoid"` ให้ยกไปทั้งก้อน

### สิ่งที่ไม่ได้ทำ และเหตุผล

- **ไม่ได้ย้าย `estimatedCost` ไปคอลัมน์อื่น** — รอให้ฝ่ายจัดซื้อยืนยันก่อนว่าใช้ราคาบนใบพิมพ์หรือไม่
- **ไม่ได้แตะแถวว่างที่เคยโผล่กลางเอกสารในไฟล์ที่ผู้ใช้ส่งมา** โดยตรง — ทำซ้ำบนโค้ดปัจจุบันไม่ได้
  (ตัวเลขไม่ตรงกับสูตรเดิม น่าจะเป็นเวอร์ชันที่ deploy อยู่ตามหลัง) แต่การคิดแถวเติมต่อหน้าในข้อ 3
  ทำให้จำนวนแถวเติมเป็นค่าที่คาดเดาได้แล้วไม่ว่าเดิมจะเพี้ยนเพราะอะไร
- **ไม่ได้แตะฟอนต์/ขนาดตัวอักษรให้เข้ากับ design system** — `DESIGN.md` ระบุไว้เองว่าเอกสารพิมพ์ที่
  คัดจากฟอร์มกระดาษจริงไม่อยู่ใต้ design system และกระดาษคือผู้ตัดสิน

### การตรวจสอบ

`tsc -b` · `tsc --noEmit -p tsconfig.api.json` · `eslint` ผ่านหมด · เทสต์ 418 ข้อผ่าน
(`jobOrderChecklist` 9 → 16, `projectAtomicity` 44 → 47 เพิ่มการคุม `isContinuation` ทั้งสองใบ
และช่องใหม่ของใบขอซื้อ) · ใบพิมพ์ทั้งสี่ถูกพิมพ์เป็น PDF จริงจาก dev stack แล้วเรนเดอร์เป็นภาพเทียบ
กับฟอร์มกระดาษทีละหน้า

---

## 2026-08-28g — Cost Control: อ่านชีตจากเนื้อใน เลือกได้หลายชีต และดึงบล็อกสรุปมาด้วย

เจ้าของส่งไฟล์งานที่สองมา (`PQ202511-267-LI-SK - น้ำมันพืชไทย …xlsx`) แล้วโยนเข้าระบบไม่ขึ้นเลย
พร้อมอธิบายว่าเข้าใจผิดกันตั้งแต่รอบแรก: *"ไอ่ตัวโยนงานไฟล์ SC อะไม่ใช่ … มันเป็นรหัสงาน"* —
**`SC` ในชื่อไฟล์คือรหัสประเภทงาน ไม่ใช่ชื่อชีต** (ไฟล์ใหม่เป็น `LI`) และสิ่งที่ต้องการคือ
*"เอาข้อมูลในไฟล์ Excel มาทำ Cost control ในเว็บ ถ้ามี sheet หลายอันก็ทำให้เลือกว่าจะเอาอันไหน
หรือเอาข้อมูลหลายอันไปใช้"*

### 1. รู้จักชีตจากเนื้อใน ไม่ใช่จากชื่อ

รอบแรกตัดสินชีตจากชื่อ (`COST CONTROL-…` / `SC`) ซึ่งพอดีกับไฟล์ตัวอย่างไฟล์เดียวเท่านั้น ไฟล์งาน
จริงไฟล์ที่สองมี 4 ชีตชื่อ `Manhole 5 mm.` / `Rev.01` / `ลองๆ` / `3mm.` — **ทั้งสี่เป็นใบ Cost**
**Control เต็มรูปแบบ** แต่ตัวแกะอ่านไม่เจอสักใบ ตอนนี้ `classifySheetRows()` ดูจากแถวหัวตารางจริง:
`A`=ลำดับที่ + `B`=รายละเอียด คือใบ Cost Control · `B`=ITEM + `D`=DESCRIPTION คือใบประเมินราคา ·
ชื่อชีตเหลือเป็นตัวช่วยตัดสินท้ายสุดเมื่อเนื้อในอ่านไม่ออก

### 2. เลือกได้หลายชีต

`<select>` ชีตเดียวกลายเป็นรายการติ๊ก บอกชนิดและ**จำนวนบรรทัดที่แกะได้ของแต่ละชีต** เลือกหลายชีต
ได้ ระบบต่อเป็นใบเดียวโดย**คั่นแต่ละชุดด้วยหัวกลุ่มชื่อชีต** (ลบทิ้งได้ในหน้า preview) · หัวใบเอาค่าแรก
ที่ไม่ว่าง · **ราคาขายไม่บวกกัน** เพราะเป็นของทั้งงาน ถ้าชีตที่เลือกระบุไว้ไม่ตรงกันจะเตือน

ค่าตั้งต้นเลือกชีตเดียวเสมอ และเลือก**อันซ้ายสุดที่แกะได้จริง** ไม่ใช่อันที่บรรทัดเยอะสุด — ตอนแรก
เขียนให้เลือกอันบรรทัดเยอะสุด แล้วมันไปเลือกชีตชื่อ `ลองๆ` ซึ่งเป็นชีตทดลองของคนทำ

### 3. ดึงบล็อกสรุปท้ายชีตมาด้วย

เดิมอ่านมาแต่รายการ คนต้องพิมพ์ราคาขายเองทุกครั้ง ตอนนี้อ่านข้อ 2-5 (ค่าดำเนินการ + %, Bubble + %,
Entertainment, ราคาขาย) มาใส่ให้ · ตัดสินจาก**เลขข้อ** ไม่ใช่ข้อความ เพราะถ้อยคำต่างกันระหว่างไฟล์
(`Bubble cost` กับ `Bubble Cost`) · `POST /api/cost-controls` รับหกช่องนี้แล้ว (เดิมบังคับเป็น null)

ยอดข้อ 1 (ราคาต้นทุน) **ไม่เก็บ** — ระบบคำนวณจาก lines เสมอตามเดิม แต่ใช้เทียบ: ถ้าต่างจากผลรวม
ที่แกะได้โดยไม่มีบรรทัดไหนปัดเศษเลย แปลว่า**แกะรายการมาไม่ครบ** จึงเตือน

### 4. วันที่แบบ พ.ศ.

ไฟล์ใหม่เขียนวันที่ว่า `14/11/68` ซึ่ง `parseSheetDate()` อ่านไม่ออก (รับแต่ปี ค.ศ. สี่หลัก) ตอนนี้
ปีสองหลักอ่านเป็น พ.ศ. และปี ≥ 2400 ลบ 543 — ยืนยันถูกจากเลขงานเอง: `PQ202511-267` = พ.ย. 2025

### ตรวจกับไฟล์จริงทั้งสองไฟล์

| ชีต | บรรทัด | ราคาต้นทุนที่รวมได้ | ไฟล์ระบุ |
|---|---|---|---|
| `Manhole 5 mm.` (น้ำมันพืชไทย) | 14 | 53,050.00 | 53,050.00 ✓ |
| `Rev.01` | 17 | 54,460.00 | 54,460.00 ✓ |
| `COST CONTROL-SC` (ไฟล์เดิม) | 52 | 8,558,521.55 | 8,558,497.46 (ต่าง 24.09 เตือนเหมือนเดิม) |

สร้างจริงจากชีต `Manhole 5 mm.` แล้วได้ **กำไร 9,950.00 และ 14.21%** ตรงกับที่พิมพ์ไว้ในไฟล์เป๊ะ

เทสต์เพิ่ม 17 ข้อใน `tests/costControlImport.test.ts` (23 → 40) ครอบคลุมการรู้จักชีตจากเนื้อใน,
วันที่ พ.ศ., บล็อกสรุป, ศูนย์แบบบัญชี (`฿-   .0`) และการรวมชีต · รวมทั้งโปรเจกต์ 411 ข้อ ผ่านหมด

---
## 2026-08-28f — ใบพิมพ์ Cost Control: เส้นขอบขวาหายตอนพิมพ์ A4 + เอาเส้นคั่นหลัง ฿ ออก

เจ้าของทดลองพิมพ์จริงแล้วแจ้ง 2 เรื่องติดกัน — *"ตอนกดปริ้นเป็น A4 ขอบมันมาไม่ครบ"* และ
*"จะมีขีดหลัง ฿ ทำไม เอาออกไปสิ"* ทั้งคู่แก้ที่ `src/pages/costControl/CostControlPrintDocument.tsx`

### 1. เส้นขอบขวาของตารางหายทั้งเส้น — ไม่ใช่ภาพลวงตาจากภาพหน้าจอ

รอบที่สร้างโมดูลเคยสงสัยเรื่องนี้แล้วสรุปว่าเป็นขอบของภาพสกรีนช็อต เพราะวัดจาก DOM แล้วทุกอย่างพอดี
รอบนี้พิสูจน์จาก **ไฟล์ PDF ที่ผ่านกระบวนการพิมพ์จริง** (`page.pdf()` แล้วแตก content stream ออกมา
อ่านพิกัดของสี่เหลี่ยมที่วาด) ได้คำตอบชัด:

| | ก่อนแก้ | หลังแก้ |
|---|---|---|
| เส้นแนวตั้งขวาสุด | วาดที่ x = **703 → 704** | วาดที่ x = **701 → 702** |
| พื้นที่พิมพ์ A4 (186mm) | 0 → **703** | 0 → 703 |

เส้นถูกวาด **นอกหน้ากระดาษทั้งเส้น** จึงหายไปหมด ไม่ใช่แค่จาง สาเหตุคือตารางกว้าง 100% พอดีเป๊ะกับ
พื้นที่พิมพ์ และ `border-collapse: collapse` วาดเส้นขอบนอกสุด **ถัดจาก** ขอบขวาของตาราง ไม่ใช่ข้างใน
(เส้นซ้ายไม่มีปัญหาเพราะวาดที่ x = 0 → 1 ซึ่งอยู่ในหน้าพอดี — ความไม่สมมาตรนี้เองที่ทำให้สังเกตยาก)
แก้ด้วย `EDGE_GUARD` คือเว้นขอบขวาของบล็อกใบพิมพ์ไว้ 2px ตารางทุกตัวจึงแคบลงเท่านั้นและเส้นขอบ
ตกอยู่ในหน้าพอดี — ตรวจแล้วทั้ง 2 หน้าของเอกสารจริง

### 2. ฿ กับตัวเลขรวมเป็นช่องเดียว

เดิมแยก `฿` เป็นคอลัมน์ของตัวเองต่อจากคอลัมน์ต้นทุน (`colSpan={2}` ที่หัวตาราง) จึงมีเส้นคั่นระหว่าง
`฿` กับตัวเลข ซึ่งฟอร์มจริงไม่มี — ในไฟล์ Excel มันเป็น **เซลล์เดียวที่ใช้รูปแบบบัญชี** ต่างหาก
ตอนนี้เหลือ `<td>` เดียว ข้างในเป็น flex ดัน `฿` ชิดซ้ายและตัวเลขชิดขวา ผลพลอยได้คือตัว `฿` ไม่โดน
ขอบเซลล์เฉือนอีก (เดิมช่องนั้นตั้ง `paddingRight: 0` ไว้จนตัวอักษรล้นออกไปทับเส้น)

### 3. กำไรพิมพ์เป็น `3011478.45` ไม่มีคอมมา

เจอระหว่างตรวจหน้าเดียวกัน — บรรทัดกำไรใช้ `toFixed(2)` ตรง ๆ ขณะที่ทุกตัวเลขอื่นในใบใช้ `fmt()`
เปลี่ยนมาใช้ `fmt()` ให้เหมือนกันทั้งใบ (`3,011,478.45`)

ตรวจครบ: `tsc` ทั้งสอง config · `lint` (เหลือ warning เดิม 3 ข้อ) · `test` 395 ข้อ · `build` ผ่านหมด

---
## 2026-08-28e — Cost Control (แผนก BD): โยนไฟล์ Excel ของงานแล้วได้เอกสารที่แก้/พิมพ์ได้

เจ้าของถามว่า *"ถ้าโยนไฟล์ Excel เข้าไปแล้วสร้างใบขึ้นมาได้ไหม"* แล้วระบุว่าเอกสารที่ต้องการคือ
**Cost Control ของแผนก BD** พร้อมส่งไฟล์จริงมาให้ทั้ง .xlsx และ .pdf

### ปิดช่องว่างที่ค้างมาตั้งแต่ 20 ส.ค.

Cost Control ถูกจดไว้ว่าเป็นตัวติดขัด **5 ที่** (`TODO.md` 2 ข้อ, `MODULES/Project.md`,
`MODULES/Production.md`, `SESSION_LOG.md`) ทุกที่เขียนตรงกันว่า *"ต้องให้เจ้าของนิยามก่อนว่าเป็น
ข้อมูลแบบไหน"* — คำตอบคือ **ไฟล์ Excel ที่บริษัทกรอกมืออยู่แล้ว** โมดูลนี้จึงถอดแบบจากไฟล์จริง
ทั้งโครงข้อมูลและใบพิมพ์ ไม่ได้ออกแบบเอง

### สร้างได้ 2 ทาง

**โยนไฟล์ Excel** (ทางหลัก) หรือ **เปิดใบเปล่ากรอกเอง** (เจ้าของสั่งเพิ่มระหว่างทำ:
*"cost control ไม่ต้องโยนไฟล์ก็สร้างเองได้ด้วยดิ"*) · เลขที่ `CC-{พ.ศ.}-{NNNN}` ·
ใช้เครื่องอนุมัติกลางร่วมกับเอกสารอื่น เพราะฟอร์มจริงมีช่อง Submitted by / Approved by

**เบราว์เซอร์เป็นคนอ่านไฟล์ ไม่ได้อัปโหลดไฟล์ดิบขึ้นเซิร์ฟเวอร์** — ส่งขึ้นไปเฉพาะแถวที่คนตรวจแล้ว
เป็น JSON ธรรมดา จึงไม่ต้องมี multipart ไม่ต้อง base64 ไม่ชนเพดานขนาด body และ preview ไม่ต้องรอ network

### ทำไมต้องมีหน้า preview เสมอ

การแปลงจากใบประเมินราคา (ชีต `SC`) เป็น Cost Control **ไม่ใช่ 1:1** ตรวจกับไฟล์จริงเจอครบสามแบบ:
บล็อกที่ยอดตรงเป๊ะ · บล็อกที่คน**แตกเป็นหลายบรรทัดพร้อมแก้ราคา** (FRP HOOD ยอด 180,000 กลายเป็น
สองบรรทัดรวม 248,000) · บล็อกที่คน**ยุบหลายแถวเป็นบรรทัดเดียว** (LADDER 14,400 + PLATFORM 10,000
→ 24,400) ตัวแกะจึงให้แค่จุดตั้งต้น แล้วบังคับให้คนตรวจ/แก้ก่อนกดสร้าง — ข้อสรุปเดียวกับที่
`templateWorkbookParser.ts` เคยเขียนเตือนไว้ตั้งแต่กรกฎาคม

### ชนิดแถวอ่านจาก "สี" ไม่ใช่จากตัวหนังสือ

ในไฟล์จริง หัวกลุ่ม (`งาน Dust Collector`) กับบรรทัดบรรยายใต้รายการ (`VERTICAL PUMP`,
`-34,800 CMH`) **หน้าตาเหมือนกันเป๊ะ** — เป็นแถวที่มีแต่ข้อความไม่มีตัวเลขทั้งคู่ คนแยกออกเพราะ
**สีพื้นหลัง** ตัวแกะจึงอ่าน `fgColor` ของเซลล์ด้วย (`F2DBDB` หัวกลุ่ม · `92D050` รายการ ·
`FFFF00` ไฮไลต์) ค่าทั้งสามอ่านจากไฟล์ ไม่ได้กะจากภาพ · ไฟล์ที่ไม่ได้ทาสีจะตกเป็น "รายละเอียดย่อย"
ไม่ใช่ "หัวกลุ่ม" — **เดาผิดทางนั้นทำให้ใบพิมพ์ขึ้นพื้นชมพูเกือบทั้งหน้า** (เจอตอนตรวจในเบราว์เซอร์)
และมีช่องเลือกชนิดแถวในหน้าแก้ไขให้แก้เองได้

### ไม่เก็บยอดรวมลงฐานข้อมูลสักตัว

`costControlTotals()` คำนวณต้นทุน/กำไร/% จาก `lines` ทุกครั้งที่อ่าน (กฎเดียวกับ `Quote.amount`
และ `purchaseOrderSubtotal()`) · **`คิดเป็น%` หารด้วยราคาขาย ไม่ใช่ต้นทุน** — ตรวจกับไฟล์จริงแล้ว
23.17% ได้จากการหารด้วยราคาขายเท่านั้น

**ส่วนต่าง 24 บาทที่เลือกจะบอก ไม่ใช่ซ่อน**: นำเข้าไฟล์แล้วได้ต้นทุน 8,558,521.55 ขณะที่ไฟล์เขียน
8,558,497.46 เพราะไฟล์**แสดงจำนวนแบบปัดเศษ** (Packing Media แสดง 10.62 ค่าจริง 10.618583)
แต่คำนวณจากค่าเต็ม ทางเลือกมีสามทาง: เก็บยอดของไฟล์ไว้ด้วย (มีความจริงสองชุด) · ปัดเศษเงียบ ๆ ·
หรือคำนวณตรงไปตรงมาแล้วบอกผู้ใช้ — เลือกทางที่สาม ตอนนำเข้าจะนับบรรทัดที่ไม่ตรงและเตือนก่อนสร้าง

### ใบพิมพ์เป็นฟอร์มจริง ไม่ใช่ placeholder

เจ้าของสั่งว่า *"รูปแบบ pdf ต้องออกมาตรงตามเหมือนใน pdf ที่ส่งให้ไปเลย"* — `CostControlPrintDocument.tsx`
จึงถอดจาก PDF จริงทีละส่วน: **หัวจดหมายของตัวเอง** (โลโก้ซ้าย ข้อความขวา ไม่ใช้ `PrintLetterhead`
ที่มีแถบทองแนวตั้ง) · ช่องหัวเรื่องมีเส้นขอบ · กล่องข้อมูลงาน 2 แถว · ตาราง 9 คอลัมน์ที่ลงสีเฉพาะ
ช่องรายละเอียด · `฿` แยกช่องของตัวเอง · ศูนย์พิมพ์เป็น `-` · บล็อกสรุปพร้อมไฮไลต์เหลือง ·
และรหัสฟอร์ม **FM-SL-06 Rev.02 : 11/09/67** มุมขวาล่าง

> หมายเหตุสำหรับคนแก้ต่อ: ตารางรายการใช้ **auto table layout** โดยตั้งใจ — เวอร์ชันแรกตั้ง
> `table-layout: fixed` พร้อม % ตายตัว แล้วตัวเลข nowrap ล้นช่องจนตัวท้ายหายตอนพิมพ์

### กลุ่มเมนูที่ 10

**BD** — มีรายการเดียวคือ Cost Control ตามกฎใน `DESIGN.md` ที่เขียนไว้เมื่อ 2026-08-28c ว่าแผนกที่มี
กระบวนการของตัวเองได้กลุ่มของตัวเอง · 7 สิทธิ์ `costControl:*` · หมวดค้นหา `CC-`

### การตรวจสอบ

เทสต์ใหม่ 33 ข้อ: `tests/costControlImport.test.ts` (23 — fixture **สังเคราะห์ในเทสต์เอง** เพราะไฟล์จริง
อยู่ใต้ `reference/` ที่ gitignore ไว้ ครอบคอลัมน์ `|` ปลอม, บล็อกที่จบด้วย `*** SAFETY***` และที่จบ
ด้วยแถวคั่นเปล่า, `฿`/ลูกน้ำ/`-`, และการตกกลับเป็น sub เมื่อไม่มีสี) และ `tests/api/costControl.test.ts`
(10 — เลขที่, ยอดที่คำนวณสดทุกครั้ง, ล็อกสถานะ, autoSave ได้ 409, rewrite, เลขซ้ำ 409, สิทธิ์ 403)
รวมทั้งโปรเจกต์ 395 เทสต์ · typecheck ทั้งสอง config · lint · build ผ่านหมด

ตรวจในเบราว์เซอร์ด้วย**ไฟล์จริงของงาน**: โยนไฟล์ → ระบบเลือกชีต `COST CONTROL-SC` ให้เอง → ยอด
ทั้งสี่บล็อกตรงกับไฟล์ → เตือนเรื่องส่วนต่าง 24.09 บาท → สร้างเป็น `CC-2569-0001` → กรอกราคาขาย
13,000,000 ได้กำไร 3,011,478.45 และ **23.17%** ตรงกับใบจริง → พิมพ์ออกมาเทียบกับ PDF ต้นฉบับ

**การให้สิทธิ์บนเครื่องจริง:** 7 สิทธิ์อยู่ใน `defaultRoles` (มีผลกับการติดตั้งใหม่เท่านั้น)
**เครื่องจริงต้องเข้าไปติ๊กเองที่หน้าบทบาทและสิทธิ์**

---

## 2026-08-28d — ถอดใบตรวจรับสินค้าและใบรับวางบิลออก (สร้างเช้า ลบบ่าย)

เจ้าของสั่งสั้น ๆ: *"ลบ 2 อันนี้ออกไปด้วยไม่ได้ใช้"* — ทั้งสองใบเพิ่งสร้างเมื่อเช้าวันเดียวกัน
(commit `dc0334c`) และไม่เคยมีใครกรอกข้อมูลจริงเลย

ทำตามแบบอย่างการถอดโมดูลที่มีอยู่แล้ว ([MODULES/CompanyProfiles.md](./MODULES/CompanyProfiles.md)):
ลบ 8 ไฟล์ (client lib ×2, handler ×2, หน้าจอ ×4) · `collections.ts` เสีย accessor/type/index ไป 9 ตัว ·
`quotes.ts` เสีย dispatch 2 สาขา · `server/app.ts` 2 route · `vercel.json` 4 rewrite ·
`App.tsx` 11 จุด · สิทธิ์ 14 ตัวออกจากทั้ง 5 ตารางและจาก `defaultRoles` ·
ค้นหาเสีย 2 หมวดกับ fast path `GR-`/`BR-` (Global Search 19 → 17 หมวด) · i18n 238 คีย์สองภาษา ·
คู่มือเสียบท 17-18 แล้วเลื่อน 19-25 → 17-23 · เทสต์ 364 → 361

### สองอย่างที่จงใจไม่แตะ

1. **collection `goods_receipts` และ `bill_receipts` ใน MongoDB** — กฎเดิมของรีโปคือการลบโค้ด
   ไม่เคยลบ collection การจะดรอปจริงต้องเป็นการตัดสินใจแยกที่สำรองข้อมูลไว้ก่อน
2. **ประกาศใน What's New ของวันที่ 2026-08-28 ที่บอกผู้ใช้ว่ามี 2 ใบนี้** — มันเป็นบันทึกลงวันที่
   ไม่ใช่คำอธิบายระบบปัจจุบัน การไปแก้ย้อนหลังคือการเขียนประวัติศาสตร์ใหม่ จึงเพิ่มรายการใหม่บอกว่าถอดออกแทน

### ของแถมที่ต้องลบตามไปด้วย

**`PurchaseOrderPickerDialog.tsx`** — ถูกใช้โดยสองหน้าที่ลบไปเท่านั้น พอลบแล้วกลายเป็นไฟล์ตาย
และ **ไม่มีเครื่องมือไหนจับได้เลย**: `noUnusedLocals` มองไม่เห็นไฟล์ที่ไม่มีใคร import และ ESLint ก็ไม่จับ
ส่วน `StockMovementSourceType` เสียค่า `"goods_receipt"` ไปทั้งสองที่ที่ประกาศ union นี้ซ้ำกัน
(`api/_lib/collections.ts` กับ `src/lib/stock.ts`) — ไม่เคยมีใครเขียนค่านี้ ข้อมูลเก่าจึงไม่มีทางมีค่านี้

ตรวจครบ: typecheck ทั้งสอง config · lint · 361 เทสต์ · build ผ่านทั้งหมด

---

## 2026-08-28c — โมดูลจัดซื้อ: ใบสั่งซื้อ · ใบตรวจรับสินค้า · ใบรับวางบิล + เปิดใบขอซื้อให้ทุกฝ่าย
...
ว่า "เพิ่ม Rewrite" เฉย ๆ ซึ่งครอบคลุมเอกสารที่ฝ่ายผลิตใช้ทุกใบ รอบนี้จึงเติมสองใบที่เหลือ

หลังรอบนี้ **เอกสารที่แก้ไขซ้ำได้มีครบทุกใบแล้ว**: ใบเสนอราคา · Scope of Work · ใบส่งมอบ (ของเดิม) ·
ใบสั่งผลิต · ใบเบิก (เฟส 2) · **ใบสั่งงาน · ใบขอซื้อ (รอบนี้)**

ทั้งสองใบใช้รูปแบบเดียวกับที่วางไว้แล้ว: `_id` คือเลขที่เอกสาร จึงต่อท้าย `-R{n}` ผ่านตัวนับ atomic
ต่อสายเอกสาร, ล้างช่องเซ็นและสถานะ, `revisionNote` ไม่สืบทอด, และ**พิมพ์หมายเหตุการแก้ไขลงบนใบจริง**

### จุดที่ใบสั่งงานต่างจากใบอื่น และเป็นจุดที่พังเงียบได้

**ใบสั่งงานผูกได้หลายรายการ** (ตั้งแต่ commit ก่อนหน้า) การย้ายลิงก์ตอน Rewrite จึงต้องใช้ตัวช่วย
พหูพจน์ `findProjectItemIdsByLink()` + `linkProjectItemsToSubDocument()` — ถ้าเผลอใช้ตัวเอกพจน์แบบ
ใบเบิก/ใบขอซื้อ จะย้ายให้แค่รายการแรก **ที่เหลือค้างชี้ฉบับเก่าเงียบ ๆ โดยไม่มี error ที่ไหนเลย**
มีเทสต์ปักไว้ และพิสูจน์แล้วว่าจับได้จริงด้วยการเปลี่ยนไปย้ายแค่ตัวแรกชั่วคราว

**ไฟล์แนบไม่สืบทอด** — สำเนาจะชี้ไฟล์ก้อนเดียวกันกับฉบับเดิม พอลบจากฉบับหนึ่ง อีกฉบับจะเหลือลิงก์เสีย
เหตุผลเดียวกับที่ Scope of Work ไม่สืบทอดไฟล์แนบตอน Rewrite (เขียนไว้ใน `scopeOfWorkHandler.ts`)
ส่วนเช็คลิสต์ขอบเขตงาน**สืบทอดมาทั้งหมด** เพราะเป็นเนื้อหาของเอกสาร ไม่ใช่ข้อมูลการดำเนินงาน
และ `id` ของแต่ละบรรทัดถูกสร้างใหม่ ไม่ใช้ซ้ำกับฉบับเดิม

### สิ่งที่ใบขอซื้อสืบทอดต่างจากใบเบิก

ใบเบิกล้างยอดเบิก/ยอดคืนทิ้งตอน Rewrite (ฉบับใหม่เริ่มเหมือนใบที่ยังไม่ได้เบิก) แต่**ใบขอซื้อสืบทอด
รายการและราคาประเมินมาทั้งหมด** เพราะฉบับแก้ไขของใบขอซื้อมักเป็นการแก้ผู้ขาย/เงื่อนไข/จำนวน
ไม่ใช่การเริ่มขอซื้อใหม่ตั้งแต่ต้น

### สิทธิ์

ทั้งสอง route เช็ค `canEdit(ctx, doc)` รายเอกสารตั้งแต่ต้น ไม่ใช่แค่ `:create` ระดับโมดูล — เป็นช่องโหว่
ที่ code review เพิ่งจับได้ในสอง route ก่อนหน้า (`2026-08-27f`) จึงไม่ทำซ้ำอีก

### ตรวจสอบ

`tsc` (ทั้งสอง config) / `lint` (0 error) / `build` / `npm test` **337 ข้อผ่านทั้งหมด** (เพิ่ม 4 ข้อ)

เทสต์ใหม่คุม: ย้ายลิงก์ครบทุกรายการของใบสั่งงาน, คัดลอกรายการ+เช็คลิสต์และให้ `id` ใหม่, ไฟล์แนบไม่สืบทอด,
ย้ายลิงก์ของใบขอซื้อ, และฉบับเดิมยังเป็น Final ไม่ถูกแตะต้อง

**ยังไม่ได้กดจริงในเบราว์เซอร์** — สะสมกับรายการที่ค้างอยู่แล้วใน TODO.md

---

## 2026-08-27f — แก้ตามผลรีวิวโค้ดของรอบ 2026-08-27 (a–e)

**ตามเก็บหลังรีวิว: `$in` → `$all` ในตัว filter ของ `linkProjectItemsToSubDocument()`**

รีวิวจับถูกว่าด่าน 404 หายไป และใส่ `"items.id": { $in: itemIds }` กลับมา — ถูกต้องแต่ยังไม่พอ
สมัยที่ผูกทีละรายการ `$in` กับ `$all` ให้ผลเหมือนกัน แต่พอผูกได้หลายรายการแล้วต่างกันมาก:
`$in` ผ่านเมื่อ**มีสักรายการเดียว**ตรง ถ้ารายการหนึ่งถูกลบไประหว่างทาง มันจะปล่อยผ่านแล้วผูกให้แค่ตัวที่เหลือ
— กลายเป็นเอกสารที่ผูกครึ่ง ๆ กลาง ๆ ซึ่งเป็นสิ่งที่คอมเมนต์ CRITICAL บอกว่าห้ามเกิด `$all` บังคับว่าทุก id
ที่ขอมาต้องมีอยู่จริง มีเทสต์คุมไว้ และพิสูจน์แล้วว่าเทสต์จับได้จริงด้วยการกลับไปใช้ `$in` ชั่วคราว


รีวิวโค้ดรอบ recall สูงของทุก commit ที่เพิ่มมาเมื่อ 2026-08-27 เจอ 15 ข้อ แก้ไปแล้ว 13 ข้อ
(อีก 2 ข้อค้างไว้ใน [TODO.md](./TODO.md) เพราะการแก้จะเปลี่ยนพฤติกรรมที่ตั้งใจไว้)

### พังจริงกับข้อมูลที่มีอยู่ก่อนแล้ว (สองข้อแรกคือของจริง)

- **หน้าแก้ไขใบขอซื้อพังทั้งหน้ากับใบที่สร้างก่อน 2026-08-27** — `PurchaseRequestLine.subDetails`
  ถูกประกาศเป็นฟิลด์บังคับ แต่ `toClient()` ไม่ได้เติมให้ตอนอ่าน บรรทัดเก่าจึงไม่มีฟิลด์นี้จริง ๆ แล้ว
  `line.subDetails.length` โยน TypeError (TypeScript จับไม่ได้ เพราะ type บอกว่ามีแน่นอน)
  แก้ที่ต้นทาง: `toClient()` เติม `subDetails: l.subDetails ?? []` ทุกบรรทัด + ใส่ `?? []` กันไว้ในหน้าจอ
  แบบเดียวกับที่ใบสั่งงานทำอยู่แล้ว
- **สินค้าที่สโตร์อนุมัติจากคำขอเพิ่มสินค้าทำหน้าใบเสนอราคาพัง** — `handleApprove()` เขียน
  `specifications: []` ลง `Product` ทั้งที่ `Product.specifications` เป็น **string** (cast `as never`
  กลบไว้) พอเลือกสินค้านั้นในตารางใบเสนอราคา `product.specifications.trim()` ก็ระเบิด แก้เป็น `""`

### สิทธิ์และความถูกต้องของข้อมูล

- **`POST /:id/rewrite` ของใบเบิกและใบสั่งผลิตไม่มีด่านรายเอกสาร** — เช็คแค่สิทธิ์ `:create` ต่างจาก
  ทุก route ที่แก้ข้อมูลในสองโมดูลนี้ซึ่งเช็ค `canEdit(ctx, doc)` ด้วย ใครที่สร้างเอกสารได้จึงแตกฉบับ
  แก้ไขจากใบของคนอื่นได้ (และของใบเบิก ยัง**ย้ายลิงก์ ProjectItem** ตามไปด้วย) เพิ่ม `canEdit` แล้ว
- **`linkProjectItemsToSubDocument()` ทำด่าน 404 หายไปตอนแปลงเป็นหลายรายการ** — filter ถูกลดเหลือ
  `{ _id }` ล้วน ๆ การอัปเดตจึง "สำเร็จ" แม้ไม่มีรายการไหนตรงเลย แล้วเอกสารลูกที่เพิ่ง insert ไปกลาย
  เป็นเอกสารกำพร้าเงียบ ๆ ซึ่งตรงข้ามกับคอมเมนต์ CRITICAL ของฟังก์ชันนั้นเอง คืน
  `"items.id": { $in: itemIds }` เข้าไปใน filter
- **แก้เลขที่/วันที่ในใบส่งมอบที่อนุมัติแล้ว แล้วเปลี่ยนหน้า = ข้อมูลหายเงียบ ๆ** — `useDirtyTracker`
  เปิดเฉพาะสถานะร่าง ทั้งที่สองช่องนี้เพิ่งถูกปลดให้แก้ได้ทุกสถานะ เปิดให้ครอบทุกสถานะ และให้การ์ด
  "ยังไม่ได้บันทึก" เรียก `saveNumbers()` แทน `save()` เมื่อพ้นร่างแล้ว
- **ลบไฟล์แนบ: ลบตัวไฟล์ก่อน `$pull`** — ถ้า `$pull` ล้ม เอกสารจะเหลือไฟล์แนบที่กดแล้วได้ 404 ตลอดไป
  สลับลำดับ อย่างแย่สุดเหลือแค่ blob กำพร้าที่ไม่มีใครอ้างถึง

### หน้าจอ

- **การ์ดไฟล์แนบในใบสั่งงานถูกวางซ้อนอยู่ในการ์ด "รายละเอียดอื่นๆ (Out of Scope)"** —
  `DocumentAttachmentsCard` เรนเดอร์ `bg-card border rounded-xl` ของตัวเองอยู่แล้ว จึงได้กรอบซ้อนกรอบ
  และอ่านเหมือนว่าไฟล์แนบเป็นส่วนหนึ่งของ Out of Scope ย้ายออกมาเป็นการ์ดของตัวเอง
- **ลบไฟล์แนบไม่สำเร็จแล้วเงียบสนิท** — `void onDelete(...)` เปล่า ๆ ไม่มีข้อความ ไม่มีอะไรเปลี่ยน
  แถมทิ้ง unhandled rejection ไว้ที่ console ตอนนี้จับ error เหมือนตอนอัปโหลด (คีย์ `attachments.removeFailed`)
- **กล่องเลือกรายการของใบสั่งผลิตเป็นทางตัน** เมื่องานนั้นไม่มีรายการเลย (หรือมีแต่หัวข้อคั่น) — แถบยืนยัน
  ถูกซ่อน กดอะไรต่อไม่ได้นอกจากย้อนกลับ ตอนนี้ยังเปิดใบเปล่าได้
- **ปุ่มลบคำขอเพิ่มสินค้าไม่ถามยืนยัน** และขึ้นให้ทุกคนเห็นแม้กับคำขอของคนอื่นที่กดแล้วได้ 403 เสมอ —
  เพิ่ม `ConfirmDialog` และซ่อนปุ่มเมื่อไม่ใช่เจ้าของและไม่มีสิทธิ์ review
- **หัวจดหมายใบพิมพ์ใส่ `pr-8` ซ้ำสองชั้น** — บล็อกเลขที่/วันที่มุมขวาถูกดันเข้ามาอีก 2rem โดยไม่มีเหตุผล
- **การบันทึกอัตโนมัติของใบสั่งผลิตเด้ง error ระหว่างพิมพ์เลขที่ใหม่** — ล้างช่องเพื่อพิมพ์ทับ = ส่ง `""`
  ขึ้นไปแล้วได้ 400 ทันที ฝั่งหน้าจอคงเลขเดิมไว้ระหว่างที่ช่องยังว่าง (ด่าน "ห้ามว่าง" ฝั่งเซิร์ฟเวอร์ยังอยู่
  เหมือนเดิม — มีเทสต์คุมไว้)

### ความสะอาดของโค้ด

- ข้อความไทยฮาร์ดโค้ดใน `ChecklistGroupCard.tsx` (บรรทัดย่อยของเช็คลิสต์) ผ่าน `t()` แล้ว
  (`checklist.subDetailPlaceholder` / `.addSubDetail` / `.removeSubDetail`) — กติกาใน `docs/CLAUDE.md`
  ระบุว่า *"New user-facing strings should always go through `t()`/`translate()`"*
- ลบ `resolveDepartmentLabels()` ใน `departmentNotify.ts` ทิ้ง — export ไว้แต่ไม่มีใครเรียกเลย

### ยังไม่แก้ (อยู่ใน TODO.md)

- `departmentNotify.ts` จับคู่ผู้รับจาก**ชื่อ**แผนกที่ฮาร์ดโค้ดไว้เป็นลิสต์ แทนที่จะหาจาก `code`
  (`WAREHOUSE`/`PURCHASING`) ในตาราง `departments` — แอดมินเปลี่ยนชื่อแผนกเมื่อไหร่ แจ้งเตือนหยุดเงียบ ๆ
  ทันที และ `handleSendToDepartments()` ของใบส่งมอบก็ยังค้นผู้รับด้วยโค้ดชุดเดิมของตัวเองอยู่ ไม่ได้เรียก
  helper ตัวใหม่
- `size: A4 portrait` ที่เติมเข้าไปในใบพิมพ์ 5 ใบไม่มีผลอะไร — `src/styles/index.css` ประกาศ
  `@media print { @page { size: A4 portrait; margin: 12mm } }` ระดับ global อยู่ก่อนแล้ว

---

## 2026-08-27e — ติ๊กเลือกรายการได้หลายอัน: ใบสั่งงานและใบสั่งผลิต

ตามที่เจ้าของสั่งเพิ่มหลังเห็นหน้าจอจริง: *"อยากให้พวกนี้มันติ๊กเลือกได้ เพราะแต่ละอันไม่เหมือนกัน
ฟีลคล้ายๆใบส่งมอบงาน"* — กล่องเลือกรายการเดิมให้เลือกได้ **ทีละรายการเท่านั้น** กดปุ๊บสร้างเอกสารทันที

เดิมข้อ J1 ในบันทึกการประชุมเขียนว่า *"ใบสั่งงานสามารถติ๊กเลือกได้ว่าจะเอาตัวไหน แบบหลายหัวข้อและ
สามารถเพิ่มเติมรายละเอียดย่อยได้"* — รอบก่อนตีความว่าเป็น**เช็คลิสต์ขอบเขตงาน** (และเจ้าของยืนยันตอนนั้น)
ซึ่งก็ทำไปแล้วในเฟส 2 รอบนี้คือ**อีกครึ่งของประโยคเดียวกัน**: ตัว "รายการ" ที่จะออกเอกสาร

### ใบสั่งงาน — หนึ่งใบครอบคลุมได้หลายรายการ

`ProjectItem` แต่ละตัวมี `jobOrderId` เป็นของตัวเอง **หลายรายการจึงชี้ไปที่ใบสั่งงานเดียวกันได้อยู่แล้ว
โดยไม่ต้องรื้อโครงสร้าง** — สิ่งที่ต้องแก้จริงคือตัวช่วยใน `projectHandler.ts` ที่เขียนไว้ว่า "หนึ่งเอกสาร
ต่อหนึ่งรายการ" จึงคืนค่าและรับค่าเป็นรายการเดียว:

| เดิม | เพิ่ม |
|---|---|
| `loadPendingProjectItemOrThrow()` | `loadPendingProjectItemsOrThrow()` |
| `linkProjectItemToSubDocument()` | `linkProjectItemsToSubDocument()` |
| `findProjectItemIdByLink()` | `findProjectItemIdsByLink()` |
| `markProjectItemFulfilled()` | `markProjectItemsFulfilled()` |
| `unlinkProjectItem()` | `unlinkProjectItems()` |

ตัวเดิมยังอยู่ครบและกลายเป็น wrapper บาง ๆ ของตัวใหม่ — **ใบเบิกและใบขอซื้อจึงไม่ถูกแตะเลย** ทั้งสองใบ
ยังเป็นหนึ่งใบต่อหนึ่งรายการตามเดิม

**เขียนหลายรายการด้วย `updateOne` + `arrayFilters` ครั้งเดียว ไม่ใช่วนลูปอัปเดตทีละตัว** — การวนลูป
เปิดช่องให้ผูกได้บางตัวแล้วพลาดตัวที่เหลือ กลายเป็นเอกสารที่ผูกครึ่ง ๆ กลาง ๆ ซึ่งไม่มีอะไรฟ้อง และ
`loadPendingProjectItemsOrThrow()` ตรวจ**ทุกรายการให้ครบก่อน**แล้วค่อยคืนค่า ไม่ใช่ตรวจไปสร้างไป

**รายการที่ติ๊กถูกคัดลอกมาเป็นรายการดำเนินงานในเอกสารให้เลย** พร้อมสเปกที่ลงไปเป็นบรรทัดย่อย —
`JobOrderLine` จึงได้ฟิลด์ `subDetails` เพิ่ม (รูปแบบเดียวกับ `ProductionOrderLine`/`PurchaseRequestLine`
ที่ทำไปในเฟส 2) **ถ้าไม่มีฟิลด์นี้ สเปกของทุกรายการจะหายเงียบ ๆ ตอนคัดลอก** พร้อมช่องกรอกในหน้าจอ
และการพิมพ์ใต้คำอธิบายเหมือนอีกสองใบ

### ใบสั่งผลิต — เลือกงานแล้วติ๊กว่าใบนี้ครอบคลุมรายการไหน

เฟส 2 ทำให้ใบสั่งผลิตดึง **ทุก** รายการจาก Scope of Work มา ซึ่งขัดกับความจริงที่บันทึกไว้เองใน
`src/lib/productionOrder.ts` ว่า *"หนึ่งงานอาจสั่งผลิตหลายรายการแยกใบกัน"* — กล่องเลือกงานจึงได้ขั้นที่สอง
ให้ติ๊กเลือกรายการ (prop `pickItems`) ไม่ส่ง `itemIds` มา = เอาทั้งหมดเหมือนเดิม ผู้เรียกเก่าจึงไม่พัง

**ปุ่ม "อัปเดตข้อมูลจาก Scope of Work" ต้องเคารพรายการที่ใบนี้ครอบคลุมด้วย** ไม่งั้นกดครั้งเดียว
รายการที่ผู้ใช้ตั้งใจไม่เอาจะไหลกลับเข้ามาทั้งหมด จับคู่ด้วย**ชื่อรายการ** เพราะใบสั่งผลิตไม่ได้เก็บ id ของ
`ScopeOfWorkItem` ไว้ — และ id พวกนั้นถูกสร้างใหม่ทุกครั้งที่ Scope refresh จากใบเสนอราคา เก็บไว้ก็ไม่
น่าเชื่อถืออยู่ดี (ดู TODO.md เรื่อง `ScopeOfWorkItem` id stability)

### กล่องเลือกรายการ

`ProjectItemSourcePickerDialog` ได้ prop `multiSelect` — เปิดแล้วแถวกลายเป็นเช็คบ็อกซ์ พร้อมแถบท้ายกล่อง
บอกจำนวนที่เลือกและปุ่มยืนยัน `onSelect` **คืนเป็นลิสต์เสมอ** แม้ในโหมดเลือกเดี่ยว ผู้เรียกจึงเขียนทางเดียวกันได้
การติ๊กถูกล้างทุกครั้งที่สลับโครงการ ไม่งั้น id ของโครงการเก่าจะติดไปกับการสร้างเอกสารของโครงการใหม่

### ตรวจสอบ

`tsc` (ทั้งสอง config) / `lint` (0 error) / `build` / `npm test` **332 ข้อผ่านทั้งหมด** (เพิ่ม 5 ข้อ)

เทสต์ใหม่คุมจุดที่พังเงียบได้: ผูกครบทุกรายการ, **ปิดงานครบทุกรายการตอนอนุมัติ**, **ปลดลิงก์ครบตอนลบ**,
ปฏิเสธทั้งคำขอเมื่อมีรายการใดถูกจองไปแล้ว (และตัวที่ว่างต้องไม่ถูกแตะ), และรับ body แบบเดิมได้อยู่

**พลาดครั้งหนึ่งระหว่างเขียนเทสต์ และแก้แล้ว**: fixture ของไฟล์นี้มีรายการเดียว เทสต์ชุดแรกจึง "ผ่าน" ทั้งที่
ทดสอบด้วยรายการเดียว — ซึ่งจะผ่านเหมือนกันแม้โค้ดรองรับแค่ตัวแรก เพิ่มรายการที่สองเข้า fixture แล้ว
พิสูจน์ซ้ำด้วยการทำให้ `markProjectItemsFulfilled` ปิดงานแค่ตัวแรก → เทสต์ฟ้องตามคาด แล้วคืนค่า

**ยังไม่ได้กดจริงในเบราว์เซอร์** — รอบนี้ผ่านแค่ tsc/lint/build/test ต้องไล่กดกล่องติ๊กทั้งสองใบด้วยตา

---

## 2026-08-27d — ผลการประชุมฝ่ายผลิต + ฝ่ายโครงการ — เฟส 3 (ส่งต่อหลังอนุมัติ + คำขอเพิ่มสินค้า)

เฟสสุดท้ายของ 18 ข้อจากที่ประชุม ต่อจาก [2026-08-27c](#2026-08-27c)

### แจ้งเตือนส่งต่อหลังอนุมัติ (J7 + J8)

*"เมื่อผู้จัดการอนุมัติเสร็จจะส่งให้ Stores ของใบเบิก"* และ *"ใบขอซื้อ ให้ผู้จัดการอนุมัติแล้วส่งไปที่จัดซื้อ"*

**เอกสารกลุ่มโครงการ/ผลิตทั้ง 4 ใบไม่เคยมีการแจ้งเตือนเลยแม้แต่ตัวเดียว** — ตัวช่วยอนุมัติร่วม
(`api/_lib/documentApproval.ts`) เขียนแค่ audit log และ `NotificationType` ก็ไม่มีสมาชิกของโมดูลเหล่านี้
งานรอบนี้จึงเป็นการต่อเส้นทางแจ้งเตือนให้ทั้งชุด ไม่ใช่แค่เพิ่มการยิงหนึ่งบรรทัด:

- `NotificationType` ได้สมาชิกใหม่ และ `Notification` ได้ฟิลด์ deep-link `relatedMaterialRequisitionId`/
  `relatedPurchaseRequestId`/`relatedProductRequestId` — **ถ้าไม่มีฟิลด์พวกนี้ กระดิ่งจะได้แจ้งเตือนที่กดแล้ว
  ไม่ไปไหน** ซึ่ง `deliveryOrderHandler.ts` เตือนไว้ตรง ๆ ว่าเป็นกับดักที่เคยเจอมาแล้ว
- ต้องลงทะเบียนอีก 3 ที่ที่ลืมง่าย: `api/_lib/systemSeed.ts` (เป็น exhaustive map — ไม่ใส่แล้ว compile ไม่ผ่าน
  ซึ่งดี), ไอคอนใน `NotificationBell.tsx`, และเส้นทางกดจากกระดิ่งใน `App.tsx`
- ยิงจาก hook `onApproved` ของ `ApprovalConfig` ที่มีอยู่แล้ว ไม่ต้องแตะตัวช่วยอนุมัติร่วม

**การหาผู้รับถูกแยกเป็นของกลาง** `api/_lib/departmentNotify.ts` — เดิมตรรกะนี้เขียนแทรกอยู่ที่เดียวใน
`deliveryOrderHandler.ts` ยกออกมาแทนที่จะก๊อป เพราะจุดเปราะอยู่ที่**การจับคู่** ไม่ใช่การ insert

⚠️ **จุดเปราะนั้นคือ `User.department` เป็น "ข้อความ" ไม่ใช่ id** และข้อมูลจริงยังไม่ตรงกัน (TODO.md บันทึกไว้
ตั้งแต่ก่อนหน้านี้) ฟังก์ชันจึงรับ**ชื่อได้หลายแบบต่อหนึ่งแผนก** — ทั้งชื่อไทยเต็ม ชื่อสั้น และค่าเก่าภาษาอังกฤษ
(`ฝ่ายคลังสินค้า`/`คลังสินค้า`/`สโตร์`/`Store`/`Stores`) เก็บไว้ที่เดียวจะได้ไล่แก้ง่ายวันที่ข้อมูลถูกจัดให้ตรง

และ **คืนจำนวนผู้รับจริงกลับมาเสมอ** ถ้าเป็น 0 จะ `console.warn` บอกว่าไม่มีใครได้รับและเพราะอะไร —
ความล้มเหลวแบบเงียบที่แย่ที่สุดของฟีเจอร์นี้คือกดอนุมัติแล้วคิดว่าส่งต่อเรียบร้อย ทั้งที่ไม่มีใครรู้เรื่อง
การแจ้งเตือนเป็น best-effort โดยตั้งใจ (`try/catch`) — **การอนุมัติต้องไม่ล้มเพราะแจ้งเตือนส่งไม่ออก**

### ระบบคำขอเพิ่มสินค้า (J9) + ปุ่มขอรหัสในใบขอซื้อ (J5)

*"เพิ่มหน้าแผนกอื่นสามารถขอเพิ่มสินค้าได้แต่ไม่สามารถตั้งรหัสได้ เมื่อสโตร์กดอนุมัติให้แจ้งเตือนผู้ขอเพิ่ม
สินค้าว่าสินค้าได้รับการตั้งรหัสสินค้าแล้ว"*

ระบบ**ไม่มีอะไรใกล้เคียงเลยก่อนหน้านี้** — `Product` ไม่มีสถานะ ไม่มี workflow และ `code` เป็นช่องที่ใครก็ตาม
ที่มีสิทธิ์ `products:create` พิมพ์เองได้ ทางออกเดิมของหน้างานคือพิมพ์ชื่อสินค้าลงใบขอซื้อแบบอิสระ (บรรทัด
ที่ไม่มี `productId`) ซึ่งไม่เคยกลายเป็นสินค้าในคลังเลย

โมดูลใหม่ครบชุด: คอลเลกชัน `product_requests`, `api/_lib/productRequestHandler.ts`,
`src/lib/productRequest.ts`, หน้า `src/pages/productRequest/ProductRequestPage.tsx`, สิทธิ์ใหม่ 4 ตัว

**หัวใจของฟีเจอร์คือ "ใครตั้งรหัสได้" และมันถูกบังคับที่เซิร์ฟเวอร์ ไม่ใช่แค่ซ่อนช่องบนหน้าจอ**:
`handleCreate()` และ `handleUpdate()` **ไม่อ่านฟิลด์ `code` จาก body เลยแม้แต่บรรทัดเดียว** รหัสเข้ามาได้
ทางเดียวคือ `handleApprove()` ซึ่งต้องมีสิทธิ์ `productRequest:review` มีเทสต์ยิงค่า `code`/`assignedProductCode`/
`status: "Approved"` เข้าไปตรง ๆ เพื่อพิสูจน์ว่าถูกเมิน — และพิสูจน์แล้วว่าเทสต์จับได้จริงด้วยการทำให้พังชั่วคราว

**สร้าง `Product` ก่อนอัปเดตสถานะคำขอ** เพื่อว่าถ้ารหัสซ้ำ (409) คำขอจะยังเป็น Pending ให้แก้รหัสแล้วกดใหม่ได้
ไม่ใช่กลายเป็น Approved ทั้งที่ไม่มีสินค้าจริงในคลัง — มีเทสต์คุมทั้งเคสชนและเคสแก้แล้วกดใหม่

**(J5)** บรรทัดในใบขอซื้อที่ไม่มี `productId` ได้ปุ่ม "ขอรหัสสินค้า" ที่สร้างคำขอโดยเติมชื่อ/หน่วย/บรรทัดย่อย
จากบรรทัดนั้นให้ และผูก `sourcePurchaseRequestId` ไว้ ปุ่มไม่ขึ้นกับสถานะเอกสาร เพราะการขอรหัสไม่ได้แก้
เนื้อหาใบขอซื้อ **ตั้งใจไม่แก้บรรทัดในใบขอซื้อให้อัตโนมัติเมื่อสโตร์ตั้งรหัสเสร็จ** — ใบขอซื้ออาจอนุมัติ/ล็อก
ไปแล้ว และการไปแก้เนื้อหาเอกสารที่อนุมัติแล้วเงียบ ๆ แย่กว่าการให้คนกดเลือกสินค้าจากคลังเองอีกครั้ง

### 🔸 ต้องติ๊กสิทธิ์เองบน production

สิทธิ์ใหม่ 4 ตัว (`productRequest:view/viewAll/create/review`) ใส่ไว้ใน `defaultRoles` แล้ว ซึ่งมีผลกับ
**การติดตั้งใหม่เท่านั้น** ฐานข้อมูลที่ติดตั้งไปแล้วจะไม่ได้รับ — **ไม่ได้เขียน `RBAC_MIGRATIONS` ให้ ตามที่
เจ้าของสั่งไว้เมื่อ 2026-08-25** ว่าจะไปติ๊กเองในหน้า "บทบาทและสิทธิ์" สิทธิ์ทั้ง 4 อยู่ในการ์ดกลุ่ม
**"คลังสินค้า"** และต้องติ๊กบน**เครื่อง production** ไม่ใช่แค่ฐานข้อมูล dev

การมอบสิทธิ์ที่ตรงกับเจตนา: `productRequest:create` ให้**ทุกแผนกที่ต้องขอของ**, `productRequest:review`
ให้**สโตร์เท่านั้น** — เพราะ `:review` คือสิ่งที่ทำให้ตั้งรหัสได้

### ตรวจสอบ

`tsc` (ทั้งสอง config) / `lint` (0 error) / `build` / `npm test` **327 ข้อผ่านทั้งหมด** (เพิ่ม 5 ข้อ)

**กดจริงในเบราว์เซอร์แล้ว** พร้อมตรวจฐานข้อมูลโดยตรงเพราะแจ้งเตือนส่งถึง "คนอื่น" จึงดูผ่านกระดิ่งของตัวเองไม่ได้:
- อนุมัติใบเบิกผ่าน UI จริง → เกิดแจ้งเตือน **1 รายการ ถึงผู้ใช้แผนกคลังสินค้าถูกคน** พร้อมหัวข้อไทย
  คำอธิบายที่ระบุผู้อนุมัติและรหัสงาน และฟิลด์ `relatedMaterialRequisitionId` สำหรับกดเปิดเอกสาร
- ฟอร์มขอเพิ่มสินค้า **ไม่มีช่องรหัสสินค้าเลย** ส่วนกล่องอนุมัติของสโตร์**มีช่องรหัสอยู่ที่เดียว**
- ส่งคำขอ → สโตร์ได้แจ้งเตือน → อนุมัติพร้อมตั้งรหัส → **สินค้าเกิดขึ้นจริงในคลัง** (stock 0) → **ผู้ขอได้รับ
  แจ้งเตือนพร้อมรหัสที่ได้** (ทดสอบข้ามผู้ใช้จริง เพราะถ้าผู้ขอกับผู้อนุมัติเป็นคนเดียวกัน ระบบจะไม่แจ้งตัวเอง)
- ข้อมูลทดสอบทั้งหมดถูกลบและคืนค่าแล้ว: คำขอ 2 ใบ, สินค้า 2 รายการ, แจ้งเตือน 4 รายการ, audit 4 รายการ,
  แผนกของผู้ใช้ทดสอบ และสถานะใบเบิกกลับเป็นฉบับร่างตามเดิม — ตรวจซ้ำหลังเก็บกวาดแล้ว

**สภาพข้อมูลจริงยืนยันปัญหาที่เตือนไว้**: ระบบมีผู้ใช้ 2 คน อยู่แผนก `Technic` และ `ฝ่ายผลิต` —
**ไม่มีใครอยู่ฝ่ายคลังสินค้าหรือฝ่ายจัดซื้อเลย** แปลว่าถ้าไม่ตั้งแผนกให้พนักงานก่อน การส่งต่อทั้งหมดนี้จะยิงไปที่
ศูนย์คน (ระบบจะ warn ใน log แต่ผู้ใช้จะไม่เห็น) — ต้องแก้ที่ข้อมูลพนักงาน ไม่ใช่ที่โค้ด

---

## 2026-08-27c — ผลการประชุมฝ่ายผลิต + ฝ่ายโครงการ — เฟส 2 (Rewrite / รายละเอียดย่อย / แนบไฟล์ / หน้าพิมพ์)

ต่อจากเฟส 1 ([2026-08-27a](#2026-08-27a), ตรวจในเบราว์เซอร์แล้วที่ [2026-08-27b](#2026-08-27b))

### Rewrite ของใบสั่งผลิตและใบเบิก + หมายเหตุการแก้ไขที่พิมพ์ออกมาได้ (P4 + P8)

เดิม Rewrite มีแค่ใบเสนอราคา/Scope of Work/ใบส่งมอบ และ `revisionNote` **ไม่เคยถูกพิมพ์ที่ไหนเลย** —
เป็นข้อมูลภายในล้วน ๆ ฝ่ายผลิตขอทั้งตัว Rewrite และขอให้ *"สามารถดูในใบปริ้นได้"* ทั้งสองใบจึงได้ทั้งคู่

ทั้งสองใบใช้ `_id` เป็นเลขที่เอกสาร จึงต่อท้ายด้วย `-R{n}` แบบเดียวกับที่ใบเสนอราคาทำกับ `_id` ของตัวเอง
โดยจองเลขผ่านตัวนับ atomic ต่อสายเอกสาร (`{doc}_revision_{root}`) ช่องเซ็นทุกช่องถูกล้าง และ `revisionNote`
**ไม่สืบทอด**มาจากฉบับก่อน ตรงกับพฤติกรรมของ Scope of Work

**จุดที่ต่างกันสองใบ และเป็นจุดที่พังเงียบได้:**

- **ใบสั่งผลิต** มีเลขสองชุด — `_id` กับ `documentNumber` (เลขที่พิมพ์บนฟอร์ม ซึ่งผู้ใช้แก้เองได้ตั้งแต่เฟส 1)
  แต่ละตัวต่อ `-R{n}` จาก**รากของตัวเอง** ถ้าคิดรากของ `documentNumber` จาก `_id` เลขที่ผู้ใช้ตั้งเองจะถูกทิ้ง
  มีเทสต์ปักไว้: ตั้งเลขเป็น `เลขของฉัน-99` แล้ว Rewrite ต้องได้ `เลขของฉัน-99-R1` ไม่ใช่ `SC-…-R1`
- **ใบเบิก** ผูกกับ `ProjectItem` — ฉบับใหม่มี `_id` ใหม่ **ถ้าไม่ย้ายลิงก์มาชี้ฉบับใหม่ หน้าโครงการจะยังชี้
  ฉบับเก่าตลอดไป** แล้วผู้ใช้กดจากโครงการเข้าไปเจอใบที่เลิกใช้แล้ว ไม่มี error ที่ไหนเลย — พิสูจน์แล้วว่า
  เทสต์จับได้จริงด้วยการทำให้พังชั่วคราว (`expected 'MR-2569-0008' to be 'MR-2569-0008-R1'`) แล้วคืนค่ากลับ
  ใบของฝ่ายผลิตไม่มี `projectId` จึงข้ามขั้นตอนนี้ไปเอง ยอดเบิก/ยอดคืนก็ไม่สืบทอด — ฉบับใหม่เริ่มเหมือนใบที่ยังไม่ได้เบิก

### ใบสั่งผลิตดึงรายการจาก Scope of Work (P6)

เดิม `handleCreate()` คัดลอกมาแค่รหัสงานกับชื่อลูกค้า แล้วตั้ง `lines: []` — ผู้ใช้ต้องพิมพ์รายการเองทั้งหมด
ตอนนี้คัดลอก `scope.items` มาเป็นรายการให้เลย โดย `specifications` (สเปกของงาน) กลายเป็น `subDetails`
ซึ่งเป็นสิ่งเดียวกันในเชิงความหมาย แค่คนละโครงสร้าง — เป็นทิศทางกลับของ `mapLineToScopeItem()`

เพิ่มปุ่ม "อัปเดตข้อมูลจาก Scope of Work" (ฉบับร่างเท่านั้น) แบบเดียวกับที่ใบส่งมอบและโครงการมี **มีกล่อง
ยืนยันก่อนเสมอ** เพราะมันเขียนทับรายการทั้งชุด รวมถึงสิ่งที่ผู้ใช้พิมพ์เพิ่มเอง และเขียน audit กำกับ

### เช็คลิสต์ขอบเขตงานแบ่งเป็นหลายหัวข้อ + รายละเอียดย่อยใต้ข้อที่ติ๊ก (J1)

เดิมเป็น**กลุ่มเดียว 23 ตัวเลือกเรียงยาว** ตอนนี้แบ่งเป็น 6 หัวข้อ (แบบและการคำนวณ / วัสดุและงานผลิต /
การตรวจสอบและทดสอบ / งานสี / ขนส่งและงานหน้างาน / อื่น ๆ) และแต่ละข้อที่**ติ๊กแล้ว**ใส่บรรทัดรายละเอียดย่อยได้

⚠️ **การจัดกลุ่มเป็นการอนุมานจากความหมายของแต่ละหัวข้อ ไม่ได้อ่านจากฟอร์มกระดาษจริง** (`reference/` ถูก
gitignore) ต้องเอาไปทาบกับ FM-PJ-01 ตัวจริงอีกที — จดไว้ใน TODO.md แล้ว

**ความเข้ากันได้ย้อนหลังคือหัวใจของข้อนี้**: ไม่ได้ migrate ฐานข้อมูล แต่จัดกลุ่มใหม่**ตอนอ่าน**ทุกครั้ง
(`withJobOrderChecklistGroups()`) จับคู่ด้วย `ChecklistOption.key` ซึ่ง**คงเดิมทุกตัว** ถ้าฟังก์ชันนี้พลาด
ใบสั่งงานที่ติ๊กไว้แล้วจะกลายเป็นว่างเปล่าเงียบ ๆ จึงมีเทสต์ 9 ข้อคุมไว้ (`tests/jobOrderChecklist.test.ts`)
รวมถึงกรณีตัวเลือกที่เลิกใช้แล้วต้องไม่หายไปเฉย ๆ และการเรียกซ้ำต้องได้ผลเดิม

`ChecklistOption.details` เป็น optional และเป็น**การเพิ่มล้วน ๆ** — Scope of Work ไม่ได้ตั้งค่านี้ การ์ดจึงไม่
เรนเดอร์อะไรเพิ่มให้ แบบเดียวกับตอนเพิ่ม `value` เมื่อ 2026-08-18 ช่องกรอกอยู่**นอก** `<label>` โดยตั้งใจ
ไม่งั้นคลิกในช่องจะไปสลับเช็คบ็อกซ์ และเซิร์ฟเวอร์**ล้างรายละเอียดย่อยเมื่อปลดติ๊ก** เพื่อไม่ให้ใบพิมพ์มี
รายละเอียดของงานที่ไม่ได้อยู่ในขอบเขต

### รายละเอียดย่อยในใบขอซื้อ (J4)

`PurchaseRequestLine.subDetails: string[]` ลอกจาก `ProductionOrderLine` ทั้งชุด (ชนิดข้อมูล ตัวกรองที่ตัด
บรรทัดว่างทิ้ง หน้าจอ และใบพิมพ์) เพราะเป็นแบบที่ง่ายที่สุดและมีเทสต์คุมพฤติกรรมอยู่แล้ว ในตารางของใบขอซื้อ
บรรทัดย่อยเป็น**แถวของตัวเอง**ใต้รายการหลัก และซ่อนทั้งแถวเมื่อเอกสารล็อกแล้วและไม่มีบรรทัดย่อย

### แนบไฟล์ในใบสั่งงาน (J2) — สร้างเป็นของกลาง ไม่ใช่ก๊อปชุดที่ 3

ตอนนั้นระบบมีการแนบไฟล์อยู่แล้ว **2 ชุดที่ไม่เกี่ยวกันเลย** (Scope of Work และ Accounting) และชุดของ
Scope of Work ผูกกับตัวเองทุกชั้น — ค่าคงที่ชื่อ `MAX_ATTACHMENTS_PER_SCOPE`, คอลเลกชันมีฟิลด์
`scopeOfWorkId`, route อยู่ใต้ `/api/scope-of-works/`, UI ฝังอยู่ใน `DocumentRecipientsPicker.tsx`
พร้อมข้อความไทยฮาร์ดโค้ด **ก๊อปเป็นชุดที่ 3 จะทำให้ตรรกะความปลอดภัยที่ยากจะทำให้ถูกกระจายอยู่ 3 ที่**
แล้วแก้ตามกันไม่ครบในวันที่เจอช่องโหว่ จึงแยกเป็นของกลางแทน:

- `api/_lib/documentAttachments.ts` — รับ config (`load`/`canEdit`/`writeAudit`/`respond`) โครงเดียวกับที่
  `documentApproval.ts` ทำสำเร็จมาแล้ว ยกตรรกะจาก `scopeOfWorkHandler.ts` มาครบพร้อมเหตุผลเดิมทุกข้อ:
  เช็คขนาดก่อน decode, ตรวจ charset ของ base64 (ตัว decode ของ Node **ข้าม**อักขระผิดเงียบ ๆ ไฟล์เสียหาย
  จะถูกเก็บแบบขาดหาย), `$push` ที่เช็คเพดานซ้ำในตัว filter กัน race, ล้างไฟล์ทิ้งเมื่อแพ้การแย่งช่องสุดท้าย,
  และไม่ส่ง HTML/SVG กลับเป็น inline (route นี้ไม่ต้องล็อกอินและอยู่บน origin เดียวกับแอป)
- คอลเลกชัน `document_attachment_files` — เหมือน `scope_attachment_files` ทุกอย่าง ต่างแค่ใช้คู่
  `docType` + `docId` แทน `scopeOfWorkId`
- `src/components/DocumentAttachmentsCard.tsx` — แกะออกมาจาก `DocumentRecipientsPicker.tsx` **พร้อมแปลเป็น `t()`**
- `src/lib/documentAttachments.ts` — รวม `fileToBase64()` ที่เคยเขียนแทรกไว้ใน `ScopeOfWorkDocument.tsx`
  (แปลงทีละก้อนเพราะ `String.fromCharCode(...bytes)` กับไฟล์ 2 MB จะ stack ล้น)

**Scope of Work ยังไม่ถูกย้ายมาใช้ตัวนี้** — โมดูลใช้งานหนัก เสี่ยงเกินคุณค่าในรอบเดียวกัน ค้างไว้ใน TODO.md
รูปร่าง `DocumentAttachment` จึงเหมือน `ScopeOfWorkAttachment` ทุกฟิลด์ เพื่อให้ย้ายทีหลังได้โดยไม่ต้องแปลงข้อมูล

### หน้าพิมพ์ใบเบิก-คืนวัสดุ (P5)

ได้หัวจดหมายของบริษัทแล้ว ผ่าน `src/components/PrintLetterhead.tsx` ตัวใหม่ และรหัสฟอร์ม FM-ST-04
ย้ายเข้า `<tfoot>` จึงซ้ำทุกหน้าเหมือนที่ใบสั่งผลิตทำในเฟส 1 **ตัวฟอร์มด้านล่างไม่แตะ** เพราะคอลัมน์ตามฟอร์ม
จริงคือสิ่งที่พนักงานสโตร์อ่าน สิ่งที่ขอให้เปลี่ยนคือหัวเอกสาร

🔸 **แผนตอนแรกคิดผิด แล้วแก้แผนกลางทาง**: แผนเขียนว่าใบเสนอราคากับใบส่งมอบ "ก๊อปหัวจดหมายกันอยู่ 2 ที่"
ให้ยุบเป็นตัวเดียว พอเปิดดูจริงกลับพบว่าเป็น**คนละดีไซน์** — ใบเสนอราคาใช้ Tailwind + สีแบรนด์ + แถบตั้ง
"QUOTATION" ส่วนใบส่งมอบใช้ inline style + ค่าคงที่ `LETTERHEAD` ที่ฮาร์ดโค้ด (ชื่ออังกฤษ, ที่อยู่สองบรรทัด,
โลโก้ 88px) การยุบสองอันนั้นเป็นอันเดียวจะ**เปลี่ยนหน้าตาเอกสารที่ใช้งานจริงอยู่ ซึ่งไม่มีใครขอ**
คอมโพเนนต์ใหม่จึงถอดแบบมาจากใบเสนอราคาตัวเดียว แล้วใช้กับใบเบิกก่อน — ใบเสนอราคาและใบส่งมอบ**ไม่ถูกแตะเลย**

ขนาดตัวอักษรในคอมโพเนนต์นี้เป็น px ตรง ๆ (`text-[9px]`/`text-[10px]`/`text-[13px]`) ซึ่งหลุดจากสเกลของ
`DESIGN.md` — **จงใจ** เพราะคัดลอกมาจาก `PrintDocument.tsx` แบบตรงตัวเพื่อให้หน้าตาตรงกัน และงานพิมพ์วัดด้วย
หน่วยจริงบนกระดาษ ไม่ใช่สเกลของหน้าจอ (ใบพิมพ์ทุกใบในแอปนี้เป็นแบบนี้เหมือนกันหมด) ไม่ได้ใส่ ignore ไว้

### ตรวจสอบ

`tsc` (ทั้งสอง config) / `lint` (0 error) / `build` / `npm test` **322 ข้อผ่านทั้งหมด** (เพิ่ม 17 ข้อ)
`tests/serverImportGraph.test.ts` ยังเขียว — สำคัญเพราะ `src/lib/jobOrder.ts` อยู่ในกราฟของเซิร์ฟเวอร์
และรอบนี้เพิ่ม import เข้าไป

**กดจริงในเบราว์เซอร์แล้ว** กับ stack ที่รันอยู่:
- ใบสั่งงาน**เก่า**ที่บันทึกไว้ตอนยังเป็นกลุ่มเดียว อ่านออกมาเป็น **6 หัวข้อ รวม 23 ตัวเลือกครบ** และมี `details` ให้
- ติ๊ก 1 ข้อ → ปุ่ม "เพิ่มรายละเอียดย่อย" โผล่ **1 ปุ่ม เฉพาะใต้ข้อที่ติ๊ก** → พิมพ์แล้วบันทึก → เซิร์ฟเวอร์เก็บจริง
  และข้อที่ไม่ได้ติ๊กยังมี `details: []`
- แนบไฟล์ PDF ชื่อไทย → อัปโหลดจริง 41 ไบต์ → ดาวน์โหลดผ่าน capability URL ได้เนื้อไฟล์ตรง พร้อม `nosniff`
  และ `filename*=UTF-8''…` ที่เข้ารหัสชื่อไทยถูกต้อง → **ใส่ key ผิดได้ 404**
- ลบไฟล์แนบ ปลดติ๊ก และล้างรายละเอียดย่อยคืนหมดแล้ว ตรวจซ้ำว่ากลับสู่สภาพเดิม

**บั๊กที่เทสต์จับได้ก่อนถึงมือผู้ใช้**: route ดาวน์โหลดไฟล์แนบเขียน `parts.length === 5` ทั้งที่ path จริงมี
4 ส่วน — อัปโหลดกับลบทำงานปกติ แต่**ทุกการดาวน์โหลดจะ 404** ซึ่งจะดูเหมือนไฟล์หายไปทั้งที่ยังอยู่ในฐานข้อมูล

---

## 2026-08-27b — เฟส 1: ผลการกดจริงในเบราว์เซอร์ (ปิดข้อค้าง "ยังไม่ได้ตรวจ")

ต่อจาก [2026-08-27a](#2026-08-27a) ซึ่งจบลงโดยยังกดจริงไม่ได้เพราะเซสชันเบราว์เซอร์หมดอายุ เจ้าของล็อกอิน
ให้แล้ว จึงไล่ตรวจครบทั้งเฟสกับ stack จริงที่รันอยู่ (Vite :3000 + Express :3001, ฐานข้อมูล dev ในเครื่อง)
**ใช้การคลิกจริงทั้งหมด** — และการคลิกจริงก็พิสูจน์คุณค่าตัวเองสองครั้ง: โอเวอร์เลย์ของทัวร์แนะนำ (driver.js)
บังปุ่มอยู่ Playwright จึงปฏิเสธการคลิกพร้อมบอกว่าอะไรบังอยู่ ซึ่ง `dispatchEvent` จะข้ามไปเงียบ ๆ

### หน้าพิมพ์ทุกหน้า (P2) — พิสูจน์ด้วย PDF จริง ไม่ใช่การอนุมาน

นี่คือข้อที่ 2026-08-27a เขียนไว้เองว่า *"\`tsc\`/\`lint\`/\`build\`/\`test\` มองไม่เห็นเลยแม้แต่นิดเดียว"* จึงตรวจแบบ
ไม่ให้เถียงได้: ใส่รายการ 41 บรรทัดให้ใบสั่งผลิตยาวเกินหนึ่งหน้า → กดปุ่มพิมพ์จริง (สตับ `window.print`
ไว้ก่อน เพื่อไม่ให้กล่องพิมพ์ของเบราว์เซอร์บล็อก ผลข้างเคียงที่ต้องการคือ `afterprint` ไม่ยิง DOM ของใบพิมพ์
จึงค้างอยู่ให้เรนเดอร์ได้) → สั่ง `page.pdf({ format: 'A4', preferCSSPageSize: true })` ซึ่งเรนเดอร์ด้วย print CSS จริง

ผลลัพธ์: PDF ออกมา **2 หน้า** แล้วแตก content stream ของแต่ละหน้าออกมาถอดรหัส glyph ดู พบว่า
**ข้อความสุดท้ายที่ถูกวาดในทั้งสองหน้าคือ `FM-PD-02 Rev.00 : 01/11/64`** ตรงตามที่ฝ่ายผลิตขอ
(`preferCSSPageSize` ให้ผลเป็น A4 ด้วย จึงยืนยันข้อ J6 ไปในตัว)

### ที่เหลือของเฟส 1

| ข้อ | ผลการกดจริง |
|---|---|
| **P3** ปลดด่าน Final | กล่องเลือกงานแสดง Scope of Work ครบ 6 ใบ **เลือกได้ทุกใบ ไม่มีป้าย "ยังไม่อนุมัติ"** และสร้างใบสั่งผลิตจากงานที่**เป็นร่าง**ได้จริง (เดิม 400) |
| **P1** แก้เลขที่ | แก้เป็น `SC-ทดสอบแก้เลข-01` บันทึกลงเซิร์ฟเวอร์จริง ส่วน `id` ยังเป็น `SC-2026-08-006` ไม่ขยับ · **สร้างใบถัดไปได้ `SC-2026-08-007` — ตัวนับเดินต่อ ไม่ได้ไปต่อจากเลขที่พิมพ์เอง** · เลขซ้ำขึ้น 409 พร้อมข้อความไทย `เลขที่ใบสั่งผลิต "…" ถูกใช้กับใบอื่นแล้ว กรุณาใช้เลขอื่น` และเซิร์ฟเวอร์ไม่บันทึกให้ · หน้ารายการแสดงเลขที่แก้แล้วในคอลัมน์ |
| **P1** ข้อมูลเก่า | ใบสั่งผลิตเดิม 3 ใบที่ไม่มีฟิลด์นี้ **แสดงเลขที่ถูกต้อง** ผ่าน fallback → `_id` ยืนยันว่า normalize-on-read + backfill ทำงานกับข้อมูลจริง ไม่ใช่แค่ในเทสต์ |
| **J10** ใบส่งมอบที่อนุมัติแล้ว | สถานะ Final: ช่องเลขที่/วันที่ **เปิดให้แก้** · ช่องติ๊กรายการ **ปิดครบทั้ง 3 งวด (16 ช่องต่องวด)** · ช่อง Remark ปิด · ปุ่ม "บันทึกเลขที่/วันที่" โผล่แทน "บันทึกร่าง" · บันทึกจริงสำเร็จ **พร้อม audit entry** `Delivery Order Installment Numbers Updated` |
| **J3** จาก/ถึงหน่วยงาน | `toSite` เป็น `<select>` มีครบ 9 แผนกจริง + ตัวเลือกว่าง · ใบสั่งงานใหม่ได้ `fromSite` เติมอัตโนมัติจากแผนกผู้สร้าง |

> หมายเหตุ: 9 ช่องติ๊กที่ยัง**เปิด**อยู่บนใบส่งมอบที่ Final คือช่อง "ส่งถึงแผนก" ซึ่งเป็นคนละฟีเจอร์
> (ของเดิมตั้งแต่ 2026-08-20) และตั้งใจให้แก้ได้หลังอนุมัติอยู่แล้ว ไม่ใช่ "ติ๊ก" ที่ที่ประชุมพูดถึง

### สิ่งที่เจอระหว่างตรวจ (ไม่ได้แก้ในรอบนี้)

**ชิปบันทึกอัตโนมัติขึ้นแดงระหว่างพิมพ์เลขที่ที่ซ้ำ** — เพราะ `documentNumber` อยู่ใน `toUpdateFields`
การบันทึกอัตโนมัติจึงยิง `PATCH ?autoSave=1` ทุกครั้งที่หยุดพิมพ์ แล้วโดน 409 ตามจริง ชิปจึงเปลี่ยนเป็น
"บันทึกอัตโนมัติไม่สำเร็จ กรุณากดบันทึกเอง" **พฤติกรรมนี้ถูกต้องตามตรรกะ** (การบันทึกล้มเหลวจริง ๆ)
แต่ระหว่างที่ผู้ใช้ยังพิมพ์เลขไม่เสร็จก็อาจดูน่าตกใจเกินเหตุ จดไว้ใน TODO.md ให้ตัดสินใจ ไม่แก้เอง
เพราะการไปยกเว้น 409 ให้ auto-save เงียบ ๆ จะกลบข้อผิดพลาดจริงประเภทอื่นไปด้วย

**ยืนยันปัญหาข้อมูลที่เตือนไว้ล่วงหน้าแล้วว่าเป็นจริง** — ผู้ใช้ที่ล็อกอินอยู่มี `department: "Technic"`
ขณะที่ตาราง `departments` มี `"ฝ่ายเทคนิค"` ใบสั่งงานใหม่จึงได้ `fromSite: "Technic"` ซึ่งไม่ตรงกับแผนกไหนเลย
ระบบทำถูกแล้ว (เก็บค่าที่มีจริงของผู้ใช้) แต่ต้องไปแก้ที่ข้อมูลพนักงาน ไม่ใช่ที่โค้ด

### ข้อมูลทดสอบ

สร้างใบสั่งผลิต 2 ใบ ใบสั่งงาน 1 ใบ และแก้เลขที่ใบส่งมอบ 1 งวด — **ลบ/คืนค่าครบทุกอย่างแล้ว** ตรวจซ้ำ
หลังเก็บกวาด: ใบสั่งผลิตเหลือ 3 ใบเดิม, ใบสั่งงานเหลือ 3 ใบเดิม, งวดใบส่งมอบกลับเป็นค่าว่าง และรายการ
ในโครงการกลับเป็น `pending` ตามเดิม

---

## 2026-08-27a — ผลการประชุมฝ่ายผลิต + ฝ่ายโครงการ — เฟส 1 (หน้าพิมพ์ / เลขที่เอกสาร / การปลดด่าน)

เจ้าของนำผลการประชุมกับ**ฝ่ายผลิต**และ**ฝ่ายโครงการ**มา รวม 18 ข้อ — เป็น feedback รอบแรกจากคนที่ต้อง
ใช้เอกสารเหล่านี้ทำงานจริง แบ่งทำเป็นเฟสตามที่เจ้าของสั่ง นี่คือเฟส 1 (ของด่วน)

### เลขที่ใบสั่งผลิตแก้เองได้ แต่ตัวนับยังเดินปกติ

`SC-{ค.ศ.}-{เดือน}-{ลำดับ}` **คือ `_id` ของ MongoDB เอง** จึงแก้ไม่ได้ และใบเบิก/ใบขอซื้อของฝ่ายผลิต
ก็อ้างถึงมันผ่าน `productionOrderId` — จึงเพิ่มฟิลด์ **`documentNumber`** แยกต่างหาก แทนการไปแก้ `_id`
ตอนสร้างมันเท่ากับเลขที่ระบบรันให้ จากนั้นแก้ได้อิสระตอนที่ยังเป็นร่าง (ตัวนับรายเดือนไม่ถูกแตะเลย)

ลอกสูตรความไม่ซ้ำของ Scope of Work มาทั้งชุด: sanitize ไม่ให้ว่าง + pre-check ที่ให้ข้อความไทย + unique
index กัน race มีจุดที่ต่างจาก Scope of Work อยู่หนึ่ง: เอกสารเก่าไม่มีฟิลด์นี้เลย ถ้าสร้าง unique index ทันที
ทุกใบจะเป็น null เหมือนกันหมดแล้วชน (E11000) — `ensureProductionOrderNumberIndex()` จึง backfill ให้เท่ากับ
`_id` ก่อนเสมอ ด้วย aggregation-pipeline update ซึ่งตรงกับค่าที่ `toClient()` แสดงอยู่แล้ว จึงไม่มีเอกสารใบไหน
เห็นเลขเปลี่ยน

### ฟุตเตอร์ FM-PD-02 ขึ้นทุกหน้า ไม่ใช่หน้าสุดท้ายหน้าเดียว

ข้อความ `FM-PD-02 Rev.00 : 01/11/64` **มีอยู่ก่อนแล้ว** แต่เป็น `<p>` ตัวสุดท้ายของสายเอกสาร จึงพิมพ์ออกมา
หน้าเดียว — ย้ายเข้า `<tfoot>` ของตารางนอกสุด ซึ่งเบราว์เซอร์ทำซ้ำให้ทุกหน้าเอง เป็นทริกเดียวกับที่ใบเสนอราคา
ใช้กับ `<thead>` สำหรับหัวจดหมายมาตั้งแต่ต้น (`PrintDocument.tsx:101-181`)

### ขนาดกระดาษ A4 ประกาศชัดทุกใบ

A4 portrait เป็นค่าเริ่มต้นทั้งแอปมาตั้งแต่ 2026-07-15 แล้ว (`src/styles/index.css`) และไม่มีใบไหนเป็น Letter
หรือ landscape เลย — งานรอบนี้จึงเป็นเรื่องความชัดเจนล้วน ๆ: ใบที่ตั้ง `@page { margin: 0 }` (เพื่อปิดหัว-ท้าย
ของเบราว์เซอร์) ตอนนี้ประกาศ `size: A4 portrait` ควบคู่ไปด้วย และใบสั่งผลิตที่เคยเขียน `@page` ไว้นอก
`@media print` และใช้ `margin: 10mm` อยู่ใบเดียวในแอป ก็แก้ให้ตรงกับชาวบ้าน (12mm)

**กระดาษต่อเนื่อง NCR ของบัญชีไม่แตะ** — เป็นขนาดที่ผู้ใช้คาลิเบรตเองต่อเครื่องโดยตั้งใจ

### ปลดด่าน Final ทั้งสองชั้น

ตามคำขอในที่ประชุม (*"ใบสั่งผลิตกับใบเบิกไม่ต้องรอ Final ก็สร้างได้"*) เจ้าของยืนยันให้ปลดทั้งสองชั้น:
Scope of Work → ใบสั่งผลิต (`productionOrderHandler.ts`) และ ใบสั่งผลิต → ใบเบิก/ใบขอซื้อ
(`materialRequisitionHandler.ts`, `purchaseRequestHandler.ts`) ฝั่งหน้าจอ `ScopeOfWorkSourcePickerDialog`
ได้ prop ใหม่ `requireFinalScope` (ค่าเริ่มต้น `true`) เฉพาะใบสั่งผลิตส่งมาเป็น `false`

**ด่านของ "สร้างโครงการ" (`projectHandler.ts`) ไม่ถูกแตะ** — เป็นคนละเรื่องกับที่ที่ประชุมขอ

เหตุผลเดิมที่ใส่ด่านไว้เมื่อ 2026-08-20 (งานที่ยังแก้ได้อาจเปลี่ยนรายการทีหลัง แล้วเอกสารลูกจะกำพร้า เพราะ
`ScopeOfWorkItem.id` ถูกสร้างใหม่ทุกครั้งที่ refresh) **ยังจริงอยู่** เพียงแต่เป็นการแลกที่ฝ่ายผลิตยอมรับเพื่อให้
เริ่มงานได้ก่อน เทสต์สองข้อที่เคยยืนยัน 400 จึงถูก**กลับด้านโดยตั้งใจ** พร้อมคอมเมนต์กำกับ เพื่อไม่ให้อ่าน
เหมือนด่านหายไปเอง

### เลขที่/วันที่ใบส่งมอบ แก้ได้แม้อนุมัติแล้ว — แต่ติ๊กรายการยังล็อกเหมือนเดิม

route ใหม่ `POST /api/delivery-orders/:id/installment-numbers` เขียนเฉพาะ `documentNumber`/`issueDate`
รายงวด และไม่ติดล็อก Final — แนวเดียวกับ `POST /material-requisitions/:id/return` และช่องเลข PO ของ Scope
of Work ที่ยกเว้นการล็อกด้วยเหตุผลเดียวกัน (ข้อมูลที่เติมทีหลังตามธรรมชาติของงาน) เขียน audit ทุกครั้ง และ
**ไม่ต่อกับการบันทึกอัตโนมัติ** ตามข้อตกลงเดิมใน TODO.md ว่าการแก้เอกสารที่อนุมัติแล้วควรมี audit กำกับเสมอ

ช่องติ๊กเลือกรายการ (`itemIds`) **ยังล็อกตามเดิม** — ตรงกับที่เจ้าของยืนยันว่า *"ไม่สามารถติ๊กได้เหมือนเดิม"*

### จาก/ถึงหน่วยงานในใบสั่งงาน

`fromSite` เติมอัตโนมัติจาก `ctx.user.department` ตอนสร้าง (แก้ทับทีหลังได้) ส่วน `toSite` เปลี่ยนจากช่องพิมพ์
อิสระเป็น `<select>` จากตาราง `departments` จริง เก็บเป็น**ชื่อ** ไม่ใช่ id เพราะใบพิมพ์ต้องแสดงชื่อ พร้อม
option สำรอง "ค่าเดิม" แบบเดียวกับหน้าจัดการผู้ใช้ จึงไม่ทำค่าที่พิมพ์ไว้แล้วหายเงียบ

⚠️ **ข้อมูลจริงยังไม่พร้อม** — TODO.md บันทึกไว้แล้วว่า user `admin` มี `department: "Technic"` ซึ่งไม่ตรงกับ
แถวไหนในตาราง `departments` เลย ช่อง "จากหน่วยงาน" จะว่างสำหรับคนกลุ่มนี้ เป็นปัญหาข้อมูล ไม่ใช่โค้ด

### ตรวจสอบ

`tsc` (ทั้งสอง config) / `lint` (0 error) / `build` / `npm test` **305 ข้อผ่านทั้งหมด** — เทสต์เพิ่ม 1 ข้อ
(แก้เลขที่ได้ / ตัวนับเดินต่อ / เลขซ้ำ 409 / เลขว่าง 400) และกลับด้านเทสต์เดิม 2 ข้อที่เคยยืนยันด่าน Final

**ยังไม่ได้กดจริงในเบราว์เซอร์** — เซสชันของ Playwright หมดอายุ ต้องให้เจ้าของล็อกอินใหม่หนึ่งครั้ง
โดยเฉพาะ**หน้าพิมพ์ต้องดูด้วยตา** (ฟุตเตอร์ทุกหน้า ต้องทดสอบกับใบที่ยาวเกิน 1 หน้า) — ดู TODO.md

---

## 2026-08-26c — Service checklist photos: the phone opened the camera and nothing else
...
**What the import graph actually looked like.** The TODO described the problem as "`statusIcon` is
JSX inside `src/lib/quotes.tsx`, which `api/` imports". Walking every `import` from `api/` into
`src/` showed it was narrower than that: **exactly one runtime import remained** —
`api/_lib/arHandler.ts`'s `import { bahtText } from "../../src/lib/quotes.js"`. Everything else
touching that module (`api/dashboard/index.ts`, `api/_lib/collections.ts`,
`src/lib/validation/quotationValidation.ts`) uses `import type`, which TypeScript erases, so it
costs the server nothing. One import, not a web of them.

**The fix, done at the root rather than the minimum.** Moving `bahtText` alone would have removed
the runtime risk, but left the JSX sitting in a file the server is one careless import away from
needing again. So both halves moved out and the file became plain TypeScript:

- `statusIcon` → **`src/pages/quotation/statusIcons.tsx`** (its two call sites, `QuoteDocument.tsx`
  and `QuoteList.tsx`, already live in that folder). It was the only JSX in the file, and the
  `lucide-react` import existed solely to feed it — both gone.
- `bahtText` (+ its private `convertDigitGroup`/digit tables) → **`src/lib/bahtText.ts`**, a module
  with *no imports at all*. `arHandler.ts` now pulls in ~60 lines of arithmetic instead of the whole
  quotation module (which also drags `apiClient.ts` in behind it). `src/lib/quotes.ts` re-exports
  `bahtText`, so all three frontend call sites are untouched.
- **`src/lib/quotes.tsx` → `src/lib/quotes.ts`.** `git mv`, so history follows.

`statusStyle`/`statusLabelKey` deliberately stayed put — plain strings, no React dependency.

**The guard that stops it coming back.** New `tests/serverImportGraph.test.ts` walks the real
runtime import graph starting from every file in `api/`, following only imports that survive
compilation (`import type` / `export type` / an all-`type`-specifier braced clause are skipped), and
fails if any reachable file is a `.tsx` or imports `react`/`react-dom`/`lucide-react`. It also
asserts the graph reaches `src/` at all, so the test can't quietly become vacuous. **Confirmed it
actually fires**: temporarily re-adding `import { statusIcon }` to `arHandler.ts` turned it red and
named the offending file; removing it turned it green again.

Deliberately *not* done: scanning file contents for JSX. In a `.ts` file TypeScript generics
(`Promise<Quote>`, `Record<QuoteStatus, X>`) are indistinguishable from JSX by regex — the first
draft of this test failed on twelve innocent files. TypeScript already rejects real JSX syntax in a
`.ts` file, so the extension check is exact on its own.

Also refreshed the three server-side comments in `quoteWorkflow.ts` / `quoteValidation.ts` /
`quoteAmounts.ts` that justified their design with "quotes.tsx also defines JSX" — no longer true,
and a stale reason is worse than none. `quoteWorkflow.ts`'s duplication now states its *actual*
remaining reason (importing the frontend module would drag `apiClient.ts` into a Node function) and
notes that merging the copies is now a real option.

Side effect worth noting: `npm run lint` warnings dropped **35 → 3**. Most were
`react-refresh/only-export-components` firing on `quotes.tsx` for exporting non-components — a rule
that doesn't apply to a `.ts` file. Those warnings were never real; they were noise generated by the
same misfiling this change corrected.

Verified: `tsc --noEmit` clean on both projects, `npm run lint` 0 errors, `npm run build` clean,
`npm test` **273/273** (28 files, +3 new). In a real browser after a dev-server restart: status
icons render on every row of the quotation list and in the document header; the print document's
Thai baht-words line is still correct (`฿1,140,016.41` → "(หนึ่งล้านหนึ่งแสนสี่หมื่นสิบหกบาทสี่สิบเอ็ดสตางค์)");
and `GET /api/ar-documents` returns 200, which forces the server to actually load `arHandler.ts`
and resolve the new `bahtText` import at runtime rather than only at typecheck.

---

## 2026-08-25 — Auto-save for every document editor, and discounts you can type in baht
...
> RE create-entry-point entry below; content otherwise unchanged.
...
## 2026-08-20j — Delivery Order can be routed to departments; the two department lists are found to be disconnected
...
## 2026-08-18p — Accounting/Stock i18n gap closed: full English translation for every on-screen page
