...
- **`SecondaryKpiSummary.tsx`**: everything else real and useful (Won/Lost/Non-Active Jobs, Average Deal Size, Average Closing Time, Pending Approvals, Overdue Follow-ups, Total Customers, Total Products) as small, dense mini-cards (`MiniCard`, icon + value + label in one row) under an uppercase section label — deliberately not the same visual weight as the primary tier. **2026-07-13, second pass**: the section header is now a click-to-toggle `<button>` (chevron rotates) that collapses/expands the whole mini-card grid — default expanded (unchanged desktop appearance), addresses a Codex "Dashboard is too dense below desktop" finding by giving users a way to hide it without removing it from anyone's default view. `MiniCard`'s value/title now carry `title=` tooltips (both were `truncate` with no full-value fallback).
