...
**This is currently the app's only styled scrollbar, on purpose.** Every other scroll region sits on white/paper-blue, where the OS scrollbar is unremarkable rather than glaring. If a future dark-surface scroll region appears, reuse `.sidebar-scroll` (or fork it with the same structure) rather than inventing a second scrollbar treatment.
