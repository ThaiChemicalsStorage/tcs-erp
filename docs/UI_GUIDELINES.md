...
- **2026-07-21, quotation page readability pass — superseded/extended the rule above**: direct user request ("อ่านง่ายขึ้น" — make it easier to read) bumped the *entire* Create/Edit Quotation page one tier further: every `text-[10px]`/`text-[11px]` eyebrow/column-header/meta label became `text-xs` (12px), and every remaining `text-xs` (12px) body/input/button/badge became `text-sm` (14px), across `QuoteDocument.tsx`, `LineItemsEditor.tsx`, `CustomerSelector.tsx`, and `InterestButtons.tsx`. Pre-existing `text-sm` content (client-name input, section titles, quote-ID badge) and the largest tiers (`text-base`/`text-lg`/`text-xl` — grand total, the "QUOTATION" document title) were left untouched, so they're now visually closer in size to the bumped body text; this is an accepted tradeoff of a mechanical two-tier bump, not a follow-up bug. **This page-specific pass does not change the general "start at `text-xs`" floor above for new labels elsewhere in the app** — it only applies to these 4 quotation-page files. If another page needs the same treatment, ask first rather than assuming it should follow automatically (this was a targeted, user-scoped change, confirmed via `AskUserQuestion` on the exact degree — "ปรับขึ้นพอประมาณ": 10-11px→12px, 12px→14px — before implementing across ~90 classNames).
...
**Browser-generated headers/footers (website URL, print date, page title) — not app-controllable.** 2026-07-16: investigated a report that a website URL appears at the bottom-left of printed Quotations/Scope of Works. Confirmed by code inspection this is **not app-rendered** — `PrintDocument.tsx`/`ScopeOfWorkPrintDocument.tsx` never render a URL anywhere (Quotation's `CompanyHeaderInfo.website` is always hardcoded to `""` in `QuoteDocument.tsx` and is never even read by the print component), and `src/styles/index.css`'s only `@media print` block contains no footer/URL content. The URL is Chrome/Edge's own "Headers and footers" print option (page URL + date + page title/number), which the browser injects into the print output itself — **`@page` CSS margins do not control this and cannot disable it**; there is no DOM/CSS API a web page can use to turn off a browser's print headers/footers. Since the app cannot suppress it, `QuoteDocument.tsx` and `ScopeOfWorkDocument.tsx` each show a small `MetricInfoTooltip.tsx` info icon next to their Print button (inside the `print:hidden` toolbar, so the hint itself never appears in the printed output) reading "หากไม่ต้องการให้ URL เว็บไซต์และวันที่พิมพ์ปรากฏบนเอกสาร ให้ปิดตัวเลือก 'Headers and footers' ... ในหน้าตั้งค่าการพิมพ์ของเบราว์เซอร์" — telling the user to disable it themselves in the print dialog before printing or saving as PDF. Both "Print" and "Export PDF" are the same `window.print()` call (see Architecture — there is no server-generated PDF anywhere in this app), so this one instruction covers both paths.
