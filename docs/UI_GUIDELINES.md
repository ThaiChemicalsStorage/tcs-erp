...
### Dashboard Overview (KPIs + status/performance panels)
The Dashboard's top-of-page "answer in 5 seconds" overview is 4 components, each owning a slice of `DashboardKpis` (`GET /api/dashboard`) in a shape that fits its content — **not** one big card grid:
- **`ExecutiveSummaryCards.tsx`** — exactly 4 cards, no more: Total Quotations, Total Quotation Value, Closed Sales, Expected Sales. `text-2xl font-mono` value, compact `p-4` card, `MetricInfoTooltip` only where the metric's calculation genuinely isn't obvious (currently just Expected Sales). **This is the only place on the Dashboard that should look like a "KPI card" — everything else uses a table, compact grid, or tile row.**
- **`QuotationStatusSummary.tsx`** — Won/Lost/Active/Non-Active as one donut + table panel (Status/Count/Value/Percentage columns), not 4 separate cards.
- **`SalesPerformancePanel.tsx`** — Win/Lose/Conversion Rate, Average Deal Size, Average Approval/Closing Time as a compact label/value grid inside one `ChartCard`, no per-metric card chrome at all.
- **`ActivityFollowUpSummary.tsx`** — Pending Approvals, Overdue Follow-ups, Expired Quotations, New Customers as a compact 4-tile row inside one `ChartCard`; only wire a tile to `onClick` navigation when a real filter exists for it (Pending Approvals → the quotation list filtered to that status) — don't fake a filter for date-derived metrics that don't have one.

**History**: 2026-07-10 (UI/UX redesign) split the original flat ~20-card grid into a two-tier hierarchy (`PrimaryKpiCards.tsx` 6 hero cards + `SecondaryKpiSummary.tsx` 9 mini-cards), which also silently stopped rendering 7 KPI fields the API still computed. **2026-07-13, first revert**: merged back into one flat `KpiGrid.tsx` (22 uniform cards) after the tiered hero-card treatment read as "template-like." **2026-07-13, second revert (current state)**: even the flat 22-card grid still read as cluttered with no hierarchy — `KpiGrid.tsx` deleted, replaced by the 4 components above. `totalCustomers`/`totalLeads`/`totalProducts`/`repeatCustomers` no longer get individual dashboard tiles (visible instead via `CustomerAnalytics.tsx`/the Products page) — still computed server-side, just not part of this overview. **Don't reintroduce a flat many-card KPI grid or a tiered hero-card hierarchy** — if a new top-level metric is added, put it in whichever of the 4 components above matches its theme (or propose a 5th compact panel), never a 5th/6th large card.
