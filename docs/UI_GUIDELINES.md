...
### Dashboard KPI Grid
`src/pages/dashboard/KpiGrid.tsx` — a single flat grid, uniform card size, `grid-cols-2 xl:grid-cols-4`, driven by the full `DashboardKpis` shape from `GET /api/dashboard` (22 fields — Total/Won/Lost/Active/Non-Active/Expired Quotations, Total Quotation Value, Closed Sales, Expected Sales, Average Deal Size, Win/Lose/Conversion Rate, Average Approval/Closing Time, Pending Approvals, Overdue Follow-ups, New/Repeat/Total Customers, Total Leads, Total Products). Each card: icon in a tinted square, `text-2xl font-mono` value, label — a `MetricInfoTooltip` only on the handful of metrics whose exact definition isn't self-evident (Expected Sales, Average Deal Size, Win Rate, Average Closing Time, Active/Non-Active Quotations, Pending Approvals).

**History**: 2026-07-10 (UI/UX redesign) split this into a two-tier hierarchy — `PrimaryKpiCards.tsx` (6 larger "hero" cards, `text-[28px]` values) + `SecondaryKpiSummary.tsx` (9 small dense mini-cards under a collapsible section) — which also silently stopped rendering 7 of the KPI fields the API still computed. **2026-07-13**: reverted back to the single flat grid after the two-tier hero-card treatment read as too "template-like" for an internal ERP (explicit user feedback) — both tier components deleted, all 22 fields restored to one consistent presentation. **Don't reintroduce a tiered/hero-card KPI hierarchy** — if a new KPI is added, add it to `KpiGrid.tsx`'s card list, matching the existing `KpiCard` visual exactly (icon badge size, value font size, card padding) rather than inventing a new size tier for it.
