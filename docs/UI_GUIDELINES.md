...
**Scroll region (`.sidebar-scroll`, added 2026-08-26)**: the nav column scrolls (`overflow-y-auto`), and until now had no scrollbar styling at all — on Windows that meant a ~17px white OS track with arrow buttons running down the navy rail, the brightest element on the sidebar. `.sidebar-scroll` in `src/styles/index.css` fixes it: transparent track, `::-webkit-scrollbar-button { display: none }` to drop the arrow buttons, a 10px track carrying a 6px `rounded-full` thumb (2px transparent border + `background-clip: content-box`, so the hit target stays comfortable while the visible pill keeps navy breathing room on both sides), plus `scrollbar-width: thin` / `scrollbar-color` for Firefox. The thumb is the sidebar's own `--sidebar-foreground` ink `#a8bed8` at `0.28`, brightening to `0.45` when the rail is hovered and `0.62` on thumb hover/active — **not** gold (the Rare Gold Rule keeps gold for the active nav item and primary actions; a scrollbar is chrome) and **not** hidden-until-hover (same reasoning as the `opacity-50` icon-action rule — the affordance stays visible at rest). The class lives on the single `<nav>` element, so the desktop rail and the mobile drawer both get it.

**Every scroll region is styled (2026-09-14).** Originally this was the app's only styled scrollbar; the owner then asked for all pages. A global rule at the top of `src/styles/index.css` (`*` + `::-webkit-scrollbar*`) gives every light surface the same shape inked navy (`rgba(11,29,58,0.18)` → `0.3` on region hover → `0.45` on thumb hover), and `.sidebar-scroll` — declared after it, because the hover selectors tie on specificity — overrides it on the navy rail. No per-region class is needed on light surfaces. If a future dark-surface scroll region appears, reuse `.sidebar-scroll` (or fork it with the same structure), otherwise the navy thumb disappears into the background.
...
### Topbar
`bg-card border-b border-border`, sidebar collapse toggle, breadcrumb (`องค์กร > {activeNav}` — page name in the body font, semibold, gold ink `#866d28`; was Playfair `#c9a84c` until 2026-09-14, which rendered Thai in a fallback font and failed contrast), Global Search (`GlobalSearch.tsx`, see below — was a decorative dead input until 2026-07-14), notification bell (see Notification Bell below), user avatar dropdown (initials circle or uploaded picture, name/role, dropdown with Settings/Log out).
...
- Breadcrumb: `hidden sm:flex` (640px+) with `truncate` on the page-name span, plus `min-w-[8rem] max-w-[18rem] flex-shrink-0` on the container (2026-09-14) — without a minimum the search box squeezed the page name down to one Thai glyph ("องค์กร › ใ") around 1100px.
...
- Scrollbars need nothing per region — the global light-surface scrollbar rule applies (see "Scroll region" above).
...
**Signature blocks: reserve the signing space on EVERY box, and leave an unsigned name blank, not `-`** (both rules added 2026-09-21 after the owner reported each independently).

A signature box that the system can fill (it knows who created or approved the document) renders a `<PrintSignatureLine>` above the label to hold that person's profile signature image. Reserving that space **only on the boxes with a known signer** is the bug: the boxes sitting beside it in the same row stay short, so their labels and dates sit on a different baseline and the whole block prints visibly crooked. It bit `ProductionOrderPrintDocument.tsx` (first row 46px vs 26px for the other four) and `JobOrderPrintDocument.tsx` (ผู้อนุมัติ and ผู้รับเอกสาร 26px apart) — both owner-reported, both invisible in the DOM until the page was rendered and `getBoundingClientRect()` read off every cell. Reserve the same height on every signing box whether or not a signature exists, and bottom-align the cells. The four boxes nobody's account owns then get as much room to sign by hand as the one the system fills, which is what a signature form wants anyway. `ReceivingReportPrintDocument.tsx` had it right from the start (a bare `<div style={{ height: 28 }} />` matching the other column) — copy that.

For the name itself use **`printTextOrBlank()`**, never `printText()`: an empty signature box is deliberately left for a pen, so printing `-` there means whoever signs has to strike it out first. `printDateOrBlank()` is the same rule for the date half. Keep `printText()` (which does print `-`) for real data — product codes, units, vendor, job code — where `-` correctly means "no value" rather than "we forgot to fill this in".
