...
|---|---|---|---|
| Dashboard | ✅ Built (real data), completed against business spec + independent review + 3 UI simplification passes + a data/filter-correctness fix pass | **Executive BI dashboard, rebuilt 2026-07-10, completed against the full business spec later the same day, fixed against an independent Codex review's findings in a third same-day pass, then visually redesigned in a fourth same-day pass** (tiered Primary/Secondary KPI cards replacing the old flat grid, `PipelineSteps.tsx` horizontal step cards replacing the broken funnel chart, a new Quotation Status Summary and Sales Activity Analytics section) — **then, 2026-07-13, four more user/review-driven fix passes**: three simplification passes after the tiered KPI cards repeatedly read as "too template-like" (reverted to a flat `KpiGrid`, then replaced that with a compact top overview — `ExecutiveSummaryCards` 4 cards, `QuotationStatusSummary`, `SalesPerformancePanel`, `SalesActivityAnalytics`, `ActivityFollowUpSummary`, `ActivityTimeline` — in a fully-specified 7-section order, every other 2026-07-10 addition kept unchanged below an "In-Depth Detail" divider), then a sixth pass fixing 4 High Priority data/filter-correctness bugs an independent review found: Sales Activity now tracks all 5 requested event categories (was 2), trend/deliberately-unfiltered widgets now visibly label their filter exceptions instead of staying silent about them, and `QuotationStatusSummary`'s count/value population mismatch is fixed at the source — see CHANGELOG.md for all passes. Date/salesperson/department filters apply server-side to most sections; a few (trend charts' start date, catalog/personal widgets, the forecast win-rate baseline) are deliberately exceptions for documented reasons and now say so in the UI — see UI_GUIDELINES.md "Filter Honesty." All data from real MongoDB aggregation via `GET /api/dashboard`, no hardcoded values, empty states throughout via `EmptyState.tsx`. Not yet browser-verified against live data (sandboxed-session network limitation, reproduced four times, not a defect — see PROJECT_STATUS.md); client bundle confirmed error-free via repeated Playwright checks against the real components with mock data. | [MODULES/Dashboard.md](./MODULES/Dashboard.md) |
