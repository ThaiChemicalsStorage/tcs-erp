...
│   │   ├── dashboard/              # DashboardPage + ~14 subcomponents (KpiGrid, PipelineFunnel, SalesPerformanceTable, JobTypeAnalytics, CustomerAnalytics, ActivityTimeline, FollowUpReminders, ApprovalDashboard, NotificationSummary, DashboardFilterBar, DashboardCharts, ChartCard, format/dateRanges helpers) — real MongoDB-backed Executive Dashboard, rebuilt 2026-07-10 then completed against the full business spec same day, see MODULES/Dashboard.md
...
|---|---|---|---|
| Dashboard | ✅ Built (real data), completed against business spec + independent review | **Executive BI dashboard, rebuilt 2026-07-10, completed against the full business spec later the same day, then fixed against an independent Codex review's findings in a third same-day pass**: KPI cards (incl. Pending Approvals, Non-Active Jobs), date/department/salesperson filters (now genuinely applied to the Customer Interest panel and Activity Timeline too, not just most widgets), sales performance + executive ranking tables (full column set, sortable), job type/customer analytics (Total + Won Value, Last Quotation Date), forecast, follow-up reminders (date-filtered), an actionable approval dashboard (inline Approve/Reject), activity timeline, notification summary, a CSV export, 10 charts incl. Revenue Trend grouping toggle and Job Type Distribution — all from real MongoDB aggregation via `GET /api/dashboard`, no hardcoded values, empty states throughout. Not yet browser-verified against live data (sandboxed-session network limitation, reproduced twice, not a defect — see PROJECT_STATUS.md). | [MODULES/Dashboard.md](./MODULES/Dashboard.md) |
| Quotation | ✅ Built, server-validated | List + create/edit/duplicate quotes, line items with per-item notes and unlimited sub-details, print/PDF export, product-library picker, **9-status approval workflow** (Draft → Pending Approval → Approved → Sent to Customer → Customer Accepted/Rejected → Won/Lost, plus Cancelled) with approval history and signature-image integration. **2026-07-10**: added Job Type classification (13-entry master list, now required + server-validated on create), Potential Opportunity flag, and Follow-up Date — feed the rebuilt Dashboard above. **2026-07-10, Codex review fix**: every quote field is now validated server-side (`api/_lib/quoteValidation.ts`), `amount` is always server-derived from `lines`/`discount` (never client-writable), and quotation numbering is atomic (a `counters` collection, not scan-then-max+1). | [MODULES/Quotation.md](./MODULES/Quotation.md) |
...
| Notifications | ✅ Built | Header bell with unread badge + panel, role-based delivery for quotation events, click deep-links to the specific quotation (2026-07-10) | [MODULES/Notifications.md](./MODULES/Notifications.md) |
