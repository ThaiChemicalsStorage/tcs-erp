...
│   │   └── admin/                 # UserManagementPage, RoleManagementPage, AuditLogPage, companyProfiles/ (CompanyProfilesPage, CompanyProfileList, CompanyProfileForm, CompanyProfileDetail — added 2026-07-13, no longer used by Quotation as of 2026-07-14)
...
| Company Profiles | ✅ Built (admin master-data management **only** — NOT connected to Quotation) | **Added 2026-07-13** — business-identity master data (name/logo/address/tax ID/branch/bank accounts/quotation prefix-terms-footer/stamp), add/edit/view/activate-deactivate/archive/set-default, 6 new permissions (not Super-Admin-locked, unlike `company:manage`), Critical/High Codex findings from that pass fixed the same day (database-level default-profile invariant, server-authoritative audit entries, a boot-sequence sign-in bug). No fake data — starts empty. **A 2026-07-13–2026-07-14 detour briefly wired this module into the Quotation form as an "issuer company" selector — built against a misunderstanding of the requirement (this ERP only ever has one issuer company) — fully reverted 2026-07-14.** Full history of both the module build and the revert: CHANGELOG.md; the reverted feature is kept purely as a marked-historical record in [MODULES/CompanyProfiles.md](./MODULES/CompanyProfiles.md) "Quotation Integration — ARCHIVED." The module itself remains fully functional for admin-managed master data, available for whatever future use the business decides — see the [Customer Management](./MODULES/Customer.md) row above for the module that actually serves the Quotation form today. | [MODULES/CompanyProfiles.md](./MODULES/CompanyProfiles.md) |
