...
| Quotation Templates | ✅ Built (2026-07-14) | New Create Quotation wizard (Job Type → Template → Preview → the normal form, pre-filled) replacing the old "jump straight to a blank document" flow. 5 templates (`SC-WET-SCRUBBER`/`SC-ACTIVATED-CARBON`/`BF-BAG-FILTER`/`TA-FRP-TANK`/`LI-FRP-LINING`) extracted from a real Excel workbook (`public/Scope of work new template for air pollution control_Technic.xlsx`) into a new `quotation_templates` MongoDB collection, idempotent SHA-256-hash-gated import (`POST /api/quotation-templates/import`), new `quotationTemplates:manage` permission (Administrator by default, Sales template-browsing reuses `quotations:create`), new optional `Quote.quotationTemplateId/Name/Version` + `QuoteLine.isSectionHeader` fields (backward-compatible — every pre-existing quotation is unaffected), a "snapshot, never live reference" copy-on-apply semantic (internal-staff notes are never copied into a customer-facing quote), section-header divider rendering in the editor/PDF, and a new "Template ใบเสนอราคา" Global Search result group. No live MongoDB/browser verification performed this pass (same sandboxed-session limitation as other recent passes) — see [MODULES/QuotationTemplates.md](./MODULES/QuotationTemplates.md) "Known Limitations." **2026-07-14, same-day Codex-review fix pass**: fixed all 3 High Priority issues an independent review found (0 Critical) — `POST /api/quotes` now server-rejects a `quotationTemplateId` whose own `jobTypeCode` doesn't match the request's `jobTypeCode` (`validateQuotationTemplate()` gained a required `quoteJobTypeCode` parameter, closing a direct-API-bypass gap the wizard's UI never actually exposed); the `SC-ACTIVATED-CARBON`/`BF-BAG-FILTER` templates' "Main Ducting" item was missing a real source spec line ("Exhaust Duct, Elbow, Flange, Damper and accessories") re-verified against the source workbook and restored as each item's first specification; and 9 real, non-placeholder literal values (Brand/Material/Static Pressure) that `applyTemplateToQuoteDraft()` would have silently discarded from `editableParameters` were reclassified to plain `specifications` text in `api/_lib/templateSeedData.ts`, so `TemplateEditableParameter.value` is now genuinely always blank as its own doc comment claims. `tsc -b`, `tsc --noEmit -p tsconfig.api.json`, `lint`, `build` all pass clean; seed data changes verified via a `tsx` sanity script. See `docs/CODEX_REVIEW_REPORT.md`'s "Claude Fix Status." | [MODULES/QuotationTemplates.md](./MODULES/QuotationTemplates.md) |
