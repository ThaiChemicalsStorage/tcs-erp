...
│   │   ├── quotation/             # QuotationPage, QuoteList, QuoteDocument, LineItemsEditor, InterestButtons, notesFormat, IssuerCompanySelector (added 2026-07-13), PrintDocument
...
| Company Profiles | ✅ Built (master-data management + Quotation integration), Critical/High Codex findings fixed | **Added 2026-07-13** — multi-company quotation-issuer master data (name/logo/address/tax ID/branch/bank accounts/quotation prefix-terms-footer/stamp), add/edit/view/activate-deactivate/archive/set-default, 6 new permissions (not Super-Admin-locked, unlike `company:manage`). No fake data — starts empty. **Tenth same-day pass**: an independent review found zero Critical (RBAC correctly enforced everywhere) and 3 High Priority issues, all fixed — a current default could be deactivated without reassignment (now blocked), the one-default invariant had no database-level guarantee (now a MongoDB partial unique index on `isDefault`), and audit entries were client-authored/forgeable (now written server-side with structured linkage, mirroring the quotation audit-integrity fix). Also fixed a more severe bug the review only hedged as Medium: the app's boot sequence broke sign-in entirely for every role except Super Admin/Administrator. **Eleventh same-day pass (Quotation integration)**: `Quote.issuerCompanyId`/`issuerCompanySnapshot` are now fully wired — a company-profile selector on the Quotation form (preselects the default/sole active profile, empty-state + warning banner otherwise), server-built frozen snapshots at issue time (never client-supplied), Draft-only issuer changes, `PrintDocument.tsx` switched from the old `company: Company` prop to `issuer: IssuerCompanyDisplay` with a live-snapshot-legacy fallback chain, and `GET /api/company-profiles` relaxed to also accept `quotations:create` callers (filtered to active-only) so Sales users can select without needing Company Profile management rights. Old quotations are unaffected (fields simply unset, legacy `company` singleton fallback). This pass also brought the API layer to Vercel Hobby's 12-function cap. **Twelfth same-day pass**: an independent Codex review of the header integration found zero Critical/High issues and one in-scope Medium defect — the issuer display's fallback chain skipped the default active profile before falling back to the legacy `company` singleton for a quote whose referenced profile had since been deactivated with no snapshot; fixed by inserting that step (snapshot → referenced profile → default active profile → legacy singleton). Also corrected stale "prep only, not yet wired" doc-comment/documentation wording left over from before the eleventh pass. | [MODULES/CompanyProfiles.md](./MODULES/CompanyProfiles.md) |
