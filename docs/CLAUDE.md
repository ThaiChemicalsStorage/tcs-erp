...
│   │   ├── dashboard/              # DashboardPage + ~17 subcomponents (PrimaryKpiCards, SecondaryKpiSummary, PipelineSteps, QuotationStatusSummary, SalesActivityAnalytics, SalesPerformanceTable, JobTypeAnalytics, CustomerAnalytics, ActivityTimeline, FollowUpReminders, ApprovalDashboard, NotificationSummary, DashboardFilterBar, DashboardCharts, ChartCard, format/dateRanges helpers) — real MongoDB-backed Executive Dashboard, rebuilt 2026-07-10, completed against the full business spec, fixed against Codex review, then visually redesigned, all same day, see MODULES/Dashboard.md
...
|---|---|---|---|
| Dashboard | ✅ Built (real data), completed against business spec + independent review + UI/UX redesign | **Executive BI dashboard, rebuilt 2026-07-10, completed against the full business spec later the same day, fixed against an independent Codex review's findings in a third same-day pass, then visually redesigned in a fourth same-day pass**: tiered Primary/Secondary KPI cards (replacing the old flat KPI grid), the Sales Pipeline funnel chart replaced with horizontal step cards (`PipelineSteps.tsx`) plus a separate off-ramp row for Rejected/Lost/Cancelled, a new Quotation Status Summary (Win/Lose/Active/Non-Active), a new Sales Activity Analytics section (Created/Edited counts, week/month/quarter/year tabs), date/department/salesperson filters, sales performance + executive ranking tables (full column set, sortable), job type/customer analytics (Total + Won Value, Last Quotation Date), forecast, follow-up reminders (date-filtered), an actionable approval dashboard (inline Approve/Reject), activity timeline, notification summary, a CSV export, 5 remaining charts (Revenue Trend, Revenue by Job Type, Job Type Distribution, Expected Sales forecast, Products by Category) — all from real MongoDB aggregation via `GET /api/dashboard`, no hardcoded values, empty states throughout via a new shared `EmptyState.tsx`. Also added: shared `PageHeader.tsx`/`MetricInfoTooltip.tsx` components, a regrouped sidebar nav, and an optional first-time Driver.js guided tour. Not yet browser-verified against live data (sandboxed-session network limitation, reproduced four times, not a defect — see PROJECT_STATUS.md); client bundle confirmed error-free via a Playwright check. | [MODULES/Dashboard.md](./MODULES/Dashboard.md) |
