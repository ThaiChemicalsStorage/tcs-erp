...
- **MongoDB connection**: a module-scope singleton client (`api/_lib/mongodb.ts`), created on first use and reused by every request in the one long-lived Express process.
- **Self-hosted MongoDB** on the production VPS (MongoDB Atlas before the ~2026-08-07 cutover); `MONGODB_URI` and `JWT_SECRET` live in `.env` (the server's own, or a developer's local one), never committed to the repo.
...
`server/app.ts`'s `API_ROUTES` maps the first path segment after `/api/` to a handler — the one
routing table (a new resource needs an entry; `tests/serverRouteTable.test.ts` fails if one is
missed). Several handler files serve more than one resource: that grouping is a leftover of the
Vercel Hobby 12-function cap the app lived under before the ~2026-08-07 cutover. **No such cap
exists any more** — a new resource may get its own handler file or a dispatch branch in an existing
one. `API_ROUTES` is the authoritative resource → handler list. History of how the grouping formed,
while the cap still applied: (Briefly
...
- **2026-07-09 cleanup**: a duplicate, unreachable `api/{auth,users,roles,products,categories,notifications,quotes}/[[...segments]].ts` layer (plus a few standalone files like `api/auth/login.ts`) existed alongside `api/handlers/`, unreachable under the `vercel.json` rewrites of the time (see the historical note below) — confirmed dead via production traffic logs and deleted. If old commit history references those paths, they no longer exist; `api/handlers/*.ts` was always the real routing.
...
### Historical: why the folder is `api/handlers/` (pre-cutover Vercel routing gotcha)

> Applies only to the retired Vercel deployment; kept because it explains the `api/handlers/` name.
> `vercel.json` was deleted 2026-09-14 — routing is `API_ROUTES` in `server/app.ts` only.
...
The final, tested, working fix: a plain-named `api/handlers/` directory (no underscore, so it's not auto-excluded, but also not directly routable by folder name alone) plus an explicit **`vercel.json`** with `rewrites` mapping every `/api/<resource>` and `/api/<resource>/:path*` pattern to its one handler file.

### Standalone Express server (2026-08-06) — the only runtime
...
handlers did not change** — their req/res types were type-only imports (erased at runtime; since
2026-09-14 the local `ApiRequest`/`ApiResponse` in `api/_lib/httpTypes.ts`, same shape, with
`@vercel/node` uninstalled), and everything the handlers actually touch (`req.url`/`headers`/`body`/`query`,
...
- **`server/app.ts`** — `createApp()`: `express.json({ limit: "25mb" })` (sized for Service photo
  saves), query parser forced to `"simple"` (string
  | string[] values, matching what handlers were written against), then a single plain middleware
  that maps the first path segment after `/api/` to a handler via the `API_ROUTES`
  table. Deliberately **not** Express path mounts
...
- **`server/index.ts` + `server/env.ts`** — entry point; dotenv loads `.env` — the only env
  file, local dev included (the old `.vercel/.env.development.local` fallback was removed 2026-09-14). Env must load before `api/_lib/mongodb.ts` evaluates (it reads
...
- **No function-count cap applies** — the consolidated handler layout is a leftover of the retired
  Vercel Hobby plan, kept as-is since it costs nothing to leave alone.
...
Every relative import in `api/` files, and in any `src/lib/*.ts` file that gets **value**-imported (not just type-imported) from `api/`, must include an explicit `.js` extension — e.g. `from "../_lib/http.js"`, `from "../../src/lib/roles.js"` — even though the actual source files are `.ts`/`.tsx`. `package.json` has `"type": "module"`, and native Node ESM module resolution requires explicit extensions on relative imports. TypeScript's `moduleResolution: "bundler"` permits omitting them for the Vite/browser build, but the server code is **not** bundled — it's executed per-file (the rule dates from the pre-cutover Vercel runtime, where a missing extension caused `ERR_MODULE_NOT_FOUND` in production, hit and fixed multiple times during the migration). Keep the convention so `api/` stays loadable under plain Node ESM, not just under `tsx`. Type-only imports (`import type { X }`) are exempt since they're fully erased at compile time and never produce a runtime import statement.
...
**This plan is superseded and was never built.** The migration that actually shipped (2026-07-09) used Vite + a Node.js REST API + MongoDB instead — a deliberately simpler, faster path to a real backend that didn't require replacing the frontend framework. Do not assume any Next.js/Prisma/Postgres/Auth.js code exists anywhere in this repo. This section is kept only as a historical record of the design that was considered and abandoned in favor of the real implementation described above. Where the two designs achieve conceptually the same goal via different means (e.g. session revocation), see [RBAC.md](./RBAC.md) for an explicit comparison.

## Database Architecture

Real MongoDB database (self-hosted on the production VPS since ~2026-08-07). Full collection-by-collection breakdown: [DATABASE.md](./DATABASE.md).

## API Architecture

Real REST API served by the standalone Express server (`server/`). Full route-by-route breakdown (method, auth, permission, request/response shape): [API.md](./API.md).
