...
- **Outbound email (added 2026-07-23)**: `api/_lib/email.ts` sends via Resend's plain REST API (a single `fetch` POST, no SDK dependency) — used only by Scope of Work's "ส่งอีเมลแจ้งผู้รับเอกสาร" feature so far (see [MODULES/ScopeOfWork.md](./MODULES/ScopeOfWork.md) "Document Recipients"). Requires `RESEND_API_KEY` in Vercel env vars — **not set by this codebase, a human must sign up at resend.com and add it**; `isEmailConfigured()` lets a caller fail fast with a clear Thai error instead of a raw fetch failure when it's missing. Optional `EMAIL_FROM` (defaults to Resend's zero-setup sandbox address, which only delivers to the account owner's own confirmed email — a real deployment should verify a sending domain and set this once one exists). See [TODO.md](./TODO.md) for the required manual setup step.
