...
- **State management: plain React state**, lifted to `App.tsx` for anything shared across modules (`quotes`, `products`, `categories`, `company`, `users`, `roles`, `notifications`, `auditEntries`, `sessionUserId`). No Redux/Zustand/Context — not needed at this scale. `currentUser` is not its own state — it's derived every render as `users.find(u => u.id === sessionUserId)`, so it's never possible for the "current user" object to go stale relative to the `users` list.
...
**None exists yet.** All data operations are synchronous JavaScript function calls within the browser — see [API.md](./API.md) for the full current-state breakdown and the proposed (not-started) future design. As of 2026-07-08 this includes a full client-side RBAC/user-management/approval-workflow/notification/audit-log simulation (see [RBAC.md](./RBAC.md)) — it is still, by definition, not a backend: every check runs in the browser and every record lives in `localStorage`, with no server as the source of truth.
