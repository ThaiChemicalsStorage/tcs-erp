...
- **Outbound email (added 2026-07-23, rewritten 2026-08-07 to person-to-person Gmail)**: `api/_lib/email.ts` sends via **nodemailer + Gmail SMTP from the acting user's own Gmail account** — each user stores a Gmail App Password (Settings → ความปลอดภัย), kept AES-256-GCM-encrypted in `users.emailAppPasswordEnc` (`api/_lib/emailCredentials.ts`, keyed by the `EMAIL_CRED_SECRET` env var), so recipients see the real sender and can reply directly. Used by Scope of Work's "ส่งอีเมลแจ้งผู้รับเอกสาร" feature and the Settings test-email route (see [MODULES/ScopeOfWork.md](./MODULES/ScopeOfWork.md) "Document Recipients"). The original central-Resend sender (`RESEND_API_KEY`/`EMAIL_FROM`, 2026-07-23 → 2026-08-07) is fully removed — no provider account or sending-domain verification needed anymore; the trade-offs are Gmail's ~500 recipients/day per account limit and each user doing a one-time App Password setup (requires Google 2-Step Verification).
