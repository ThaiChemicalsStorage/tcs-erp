...
- Notification clicks still only navigate to the quotation list, not a specific quote's detail view. **Partially addressed 2026-07-10**: a lightweight `quotationListFilter` (status/client) is now lifted to `App.tsx` and consumed once on mount by `QuotationPage`/`QuoteList`, so the Dashboard's pipeline-stage and follow-up-reminder clicks open a *pre-filtered list* — but true deep-linking to one specific quote's detail view still needs the full `view`/`selectedId` state lifted, which this pass didn't do.
