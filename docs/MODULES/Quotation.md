...
5. **Per-item notes, sub-details, tags & specifications**: sub-details live directly in the table now (restyled 2026-07-21, see below) — click the Pin icon on a line row to append a new "pinned" row directly beneath it (gold-tinted highlight, Pin icon, auto-focused input), addable/editable/deletable/drag-reorderable via a grip handle, no card to open. Notes, specifications, and tags remain behind the separate sticky-note icon, which still expands a panel with (a) a notes textarea + bullet/numbered-list toolbar buttons, (b) a specifications textarea (distinct from notes — auto-copied from `Product.specifications` when the line came from the product picker), (c) a tags chip input. Each of the two icons carries its own gold dot/fill when its own category of content is non-empty (Pin for sub-details, sticky-note for notes/specifications/tags) — independent of each other, and independent of the other icon's expand/collapsed state.
