...
## หน้าประวัติสต๊อก — แยกจากหน้าสต๊อกสินค้า (2026-09-23)

เจ้าของสั่ง: *"แยกประวัติปรับสต๊อกออกมาเป็นหน้าใหม่และสามารถค้นหาดูได้ว่าของชิ้นนี้ตัดไปกับงานไหนบ้างเข้ายังไงบ้าง"*

- เมนูใหม่ **ประวัติสต๊อก** (`stockHistory`) ในกลุ่มคลังสินค้า ถัดจากสต๊อกสินค้า · สิทธิ์ `stock:view` เดิม
  ไม่มีสิทธิ์ใหม่ · หน้าสต๊อกสินค้าเอาการ์ด "ประวัติการปรับสต๊อกล่าสุด" ออก ปุ่มประวัติของแต่ละแถวพามาหน้านี้
  ที่แท็บติดตามสินค้าตัวนั้น (ปุ่มพิมพ์การ์ดสต๊อกยังอยู่ที่เดิม)
- **แท็บทุกความเคลื่อนไหว** — ค้นหาคำเดียวได้ทั้งรหัส/ชื่อสินค้า เลขที่เอกสาร **เลขงาน ใบสั่งงาน ใบสั่งซื้อ ผู้ขาย**
  + ตัวกรองประเภท/ที่มา/ช่วงวันที่ · แบ่งหน้าจากเซิร์ฟเวอร์ (50 แถว) · การ์ดสรุปนับจากทั้งชุดที่กรอง ·
  คอลัมน์ "ไปใช้กับงาน / มาจาก" และ "ผู้ทำรายการ" ใหม่
- **แท็บติดตามรายสินค้า** — เลือกสินค้า (+ช่วงวันที่) แล้วได้: แถบสมการ ยกมา + รับเข้า + คืน − ตัดออก ± ปรับยอด
  = คงเหลือ, **ตัดไปใช้กับงานไหนบ้าง** (รวมตามเลขงาน → ใบสั่งงาน → ใบสั่งผลิต, ไม่มีงาน = แผนก/ทีม, ใบขาย = ขายตรง
  · คลิกเพื่อดูเอกสารทีละใบ), **รับเข้ามาจากไหน** (ใบรับ/ใบสั่งซื้อ/ผู้ขาย/ราคา), คืนของและปรับยอด, สต๊อกการ์ดไล่ยอด ·
  ดึงไม่เกิน 500 แถวต่อครั้ง เกินแล้วบอกให้ย่อช่วงวันที่
- เซิร์ฟเวอร์: `api/_lib/stockHistory.ts` (`GET /api/stock-movements/history`) — งาน/ผู้ขาย **อ่านจากเอกสารต้นทางตอนขอ**
  ไม่คัดลอกลงแถวสต๊อก เพื่อให้แถวเก่าหลายพันแถวได้คำตอบเดียวกับแถวใหม่โดยไม่ต้อง migrate ·
  `GET /api/stock-movements` ตัวเดิมยังอยู่ (การ์ดสต๊อกและหน้าอื่นใช้)
- `StockMovementSourceType` เพิ่ม `stock_import` และ `store_receipt` รอไว้สำหรับงานนำเข้า Excel และใบรับคืนของสโตร์
- เทสต์: `tests/api/stockHistory.test.ts`


## นำเข้ายอดสต๊อกจาก Excel (2026-09-23)

เจ้าของสั่ง: *"สต๊อกสินค้าทำให้สามารถรับข้อมูล stock สินค้าเป็น exel"*

- ปุ่ม **นำเข้าจาก Excel** ในหน้าสต๊อกสินค้า (เฉพาะ `stock:adjust`) → `StockImportDialog`
- ไฟล์หนึ่งแถวต่อสินค้า: **รหัสสินค้า + ยอดคงเหลือ** (บังคับ) และ **ต้นทุนต่อหน่วย** (ไม่บังคับ) · หัวคอลัมน์ไทย/อังกฤษ
  หัวไม่ต้องอยู่บรรทัดแรก (`parseStockRows()` ใน `src/lib/stockImport.ts` แนวเดียวกับนำเข้าสินค้า)
- **ยอดในไฟล์คือยอดที่ควรเป็น** ไม่ใช่ยอดที่บวกเพิ่ม — ระบบลงส่วนต่างเป็นความเคลื่อนไหวสต๊อก ไม่เขียนทับ `stockQty`
  ตรง ๆ (กติกาบัญชีสต๊อก: ยอดเปลี่ยนผ่าน `applyStockMovement()` ทางเดียว) · ยอดเพิ่ม + มีต้นทุน = `receive` ถัวต้นทุนใหม่ ·
  ที่เหลือ `adjust` · ยอดเท่าเดิมข้าม
- ดูตัวอย่างก่อนนำเข้าเสมอ: จะเปลี่ยน / เท่าเดิม / ไม่พบรหัส (มีรหัสที่ไม่พบ = ปุ่มนำเข้าล็อก) + แถวที่ถูกข้ามพร้อมเหตุผล
- **ไฟล์ตั้งต้น** = สินค้าทุกตัวพร้อมยอดปัจจุบัน แก้แค่คอลัมน์ยอดแล้วโยนกลับมาได้ (เหมาะกับตรวจนับประจำเดือน)
- ทุกแถวของการนำเข้าหนึ่งครั้งผูกเลขชุด `SI-YYYYMM-NNNN` ค้นย้อนหลังได้ในหน้าประวัติสต๊อก (ที่มา "นำเข้าจาก Excel")
- **ไฟล์ไม่ถูกอัปโหลด** — แกะในเบราว์เซอร์แล้วส่งแค่ตัวเลข จึงไม่ต้องผ่าน `storeUpload()` (ไม่มีไฟล์ถูกเก็บ)
- เทสต์: `tests/stockImport.test.ts` (ตัวแกะ) + `tests/api/stockImport.test.ts` (ทั้งไฟล์ผิดต้องไม่เขียน, ถัวต้นทุน, เลขชุด)
