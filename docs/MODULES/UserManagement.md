...
3. **Create**: modal-free full-page form — Full Name, Employee ID, Username, Email, Phone, Department, Team, Position (free text + suggestions), Role (dropdown of assignable roles — the Super Admin role option is hidden unless the current user is themselves Super Admin), initial password + confirm. Employee ID/Username/Email uniqueness enforced (`isEmployeeIdTaken`/`isUsernameTaken`/`isEmailTaken`). **Department (changed 2026-07-23, source swapped 2026-08-14)**: a real `<select>`, sourced from the now-wired `departments` collection (`GET /api/departments`, manageable via the new Departments admin page — see [Department.md](./Department.md)) rather than the hardcoded `DOCUMENT_RECIPIENT_DEPARTMENTS` constant it used from 2026-07-23 to 2026-08-14 (that constant is untouched and still drives a *different* concern, Scope of Work's "เอกสารส่งถึง" recipient routing — see [ScopeOfWork.md](./ScopeOfWork.md) "Document Recipients"). A user whose stored value doesn't match any current department name is kept as a selectable "ค่าเดิม"/legacy-value fallback option, never silently overwritten on save. **Team (added 2026-08-14)**: a second `<select>`, populated from `GET /api/teams?departmentId=` for whichever department is currently selected — hidden when that department has no teams (true for every department except Sales today, but not hardcoded that way). Changing the selected department clears the team selection (a team only ever belongs to one department). Powers the `viewTeam` visibility tier on Quotations/Scope of Work/Delivery Order — see [RBAC.md](../RBAC.md) "Departments + Teams + Tiered Visibility".
