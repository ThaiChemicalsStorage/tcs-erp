...
2. Tabs, each independently save-able:
   ...
   - **ความปลอดภัย (Security)**: current/new/confirm password fields. Current password is now **actually verified** against the signed-in user's stored hash (`verifyPassword`) before the new one is accepted — no longer a pure client-side no-op. Writes a `Password Reset` audit entry on success. **2026-08-07**: a second card, **การส่งอีเมล (Gmail App Password)** — self-service setup for person-to-person Scope of Work document emails (sent from the user's own Gmail, see [ScopeOfWork.md](./ScopeOfWork.md) "Document Recipients"): a status pill (`hasEmailAppPassword`), Thai help steps (requires Google 2-Step Verification → create at myaccount.google.com/apppasswords → stored encrypted, never shown again → Gmail's ~500/day limit), a password input (save = `PATCH /api/users/:id { emailAppPassword }`, strictly self-only), a ล้างค่า button (ConfirmDialog → sends `""`), and — once configured — a "ส่งอีเมลทดสอบถึงตัวเอง" button (`POST /api/users/:id/email-test`). Writes an `Email App Password Updated` audit entry on save/clear.
