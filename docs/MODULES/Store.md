...
- มีบันทึกอัตโนมัติ + กู้ร่าง (กฎของทุกหน้าแก้เอกสาร) · ใบพิมพ์ชั่วคราว (`StoreReceiptPrintDocument`) รอฟอร์มจริง
- อยู่ในกล่อง "เอกสารรออนุมัติ" (kind `storeReceipt`) · หน้าประวัติสต๊อกอ่านที่มาของแถว `store_receipt` ได้ (งาน/ใบเบิกต้นทาง)
...
· ลิงก์จากกล่องรออนุมัติ ผลค้นหา (รหัสจ่ายทั้ง 15 ตัวเป็น prefix ของใบเบิกใน Global Search) และคิวตัดของ พามาหน้านี้

**ยังไม่ได้ทำ**: ใบรับคืนยังไม่อยู่ใน Global Search (ค้นเลข JD-/FG-… ไม่เจอ ต้องเข้าผ่านเมนู) · ยังไม่แจ้งเตือนผู้อนุมัติตอนส่งขออนุมัติ
(ขึ้นในกล่องรออนุมัติอย่างเดียว) · ยังไม่ได้กดทดสอบบนเบราว์เซอร์ (เบราว์เซอร์อัตโนมัติยังไม่ได้ล็อกอิน) — ดู TODO.md
