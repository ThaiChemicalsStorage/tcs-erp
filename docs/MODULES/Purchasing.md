...
## รหัสฝ่ายของใบขอซื้อ PR / FD / ED / SD (2026-09-23)

เจ้าของสั่ง: *"ใบขออนุมัติซื้อจากสโตร์ จะมี 4 ฝ่ายเป็นฝ่าย Support(PR), ผลิต(FD), โครงการ(ED), งานเหล็ก(SD)
... ถ้าฝ่ายผลิตสร้างใบขอซื้อก็ให้ขึ้น แต่ละรหัสรันเลขแยกกัน"* — ตรงกับใบกระดาษจริงที่ขึ้นต้นด้วย `ED` อยู่แล้ว
(`ED6908027.pdf`)

- `PurchaseRequest.requestCode: "PR" | "FD" | "ED" | "SD"` — รหัสคือตัวอักษรหน้าเลขที่ใบ
  (`FD-202609-0001`) และ **แต่ละรหัสมีตัวนับของตัวเอง** (`purchase_request_fd_YYYYMM` ฯลฯ)
- `PR` ใช้กุญแจตัวนับเดิม `purchase_request_YYYYMM` เลขของใบ Support จึงต่อจากใบเก่าทุกใบ ไม่เริ่ม 0001 ใหม่กลางเดือน
- **รหัสตั้งต้นมาจากทางที่สร้าง**: จากใบสั่งผลิต → `FD`, จากโครงการ → `ED`, ใบเปล่า → เลือกเองในหน้าต่าง
  `PurchaseRequestCodeDialog` (ตั้งต้นที่ `PR`) ซึ่งเลือก `SD` ได้ · `POST /api/purchase-requests` รับ
  `requestCode` ได้ทุกทาง และตอบ 400 กับรหัสที่ไม่รู้จัก แทนที่จะตกไปใช้ค่าตั้งต้นเงียบ ๆ
- **เปลี่ยนรหัสหลังสร้างไม่ได้** — รหัสฝังอยู่ในเลขที่ใบซึ่งเป็น `_id` · PATCH ไม่รับฟิลด์นี้
- ใบก่อนวันนี้ไม่มีฟิลด์ `requestCode` — `purchaseRequestCodeOf()` อ่านจากตัวอักษรหน้าเลขแทน (ทุกใบเก่าเป็น `PR-`)
  ไม่ได้ทำ migration
- ใบพิมพ์ FM-PU-05 เขียนชื่อฝ่ายในวงเล็บใต้หัวเรื่องจากรหัส ("(ฝ่าย Support)" / "(ฝ่ายผลิต)" / "(ฝ่ายโครงการ)" /
  "(งานเหล็ก)") — เดิมอ่านจาก `ownerDepartment` และใบเปล่าไม่มีวงเล็บเลย
- Global Search รู้จัก `FD-` `ED-` `SD-` เป็นใบขอซื้อ (`DOC_NUMBER_PREFIXES` ใน `api/_lib/searchShared.ts`)
- เทสต์: `tests/api/purchaseRequestCodes.test.ts`

**เมนูสโตร์สลับลำดับ (วันเดียวกัน)**: ในกลุ่มคลังสินค้า "เปิดใบขอซื้อ (สโตร์)" ขึ้นก่อน "ใบขอซื้อ (รอสโตร์เช็คของ)"
แล้ว ตามที่เจ้าของแจ้งว่าสองเมนูนี้ "สลับกันอยู่" · ตรวจแล้วว่าแต่ละเมนูเปิดหน้าที่ถูกต้องอยู่แล้ว ต่างกันแค่ลำดับ
