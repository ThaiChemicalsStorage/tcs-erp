...
/**
 * เลือกใบสั่งซื้อต้นทาง — ใช้ร่วมกันโดยใบตรวจรับสินค้าและใบรับวางบิล
 *
 * อยู่ในโฟลเดอร์ของใบสั่งซื้อเพราะเป็นตัวเลือก "ใบสั่งซื้อ" ไม่ใช่ของโมดูลที่เรียกใช้ — สองหน้าที่
 * เรียกจึง import ข้ามมา แทนที่จะก๊อปกล่องเดียวกันไว้สองที่แล้วรอให้มันเพี้ยนกันเอง
 *
 * แสดงเฉพาะใบที่อนุมัติแล้ว (Final) เพราะเซิร์ฟเวอร์ปฏิเสธการออกเอกสารต่อจากใบที่ยังไม่อนุมัติอยู่แล้ว
 */
export function PurchaseOrderPickerDialog({
  ...
  useEffect(() => {
    ...
    fetchAllPurchaseOrders()
      .then((r) => { if (!cancelled) { setRows(r.filter((x) => x.status === "Final")); setLoading(false); } })
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div ref={panelRef} role="dialog" aria-modal="true" aria-label={t("purchaseOrder.orderPicker.title")}
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-border">
          <div className="flex-1">
            <h2 className="text-base font-semibold text-foreground" style={{ fontFamily: "'Playfair Display', 'Noto Sans Thai', serif" }}>{t("purchaseOrder.orderPicker.title")}</h2>
            <p className="text-xs text-muted-foreground mt-0.5">{t("purchaseOrder.orderPicker.subtitle")}</p>
          </div>
        ...
        <div className="px-5 py-3 border-b border-border">
          <div className="relative h-9">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t("purchaseOrder.orderPicker.searchPlaceholder")}
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {loading ? (
            ...
            <p className="text-center text-sm text-muted-foreground py-12 px-5">{t("purchaseOrder.orderPicker.empty")}</p>
          ) : filtered.map((r) => (
            <button key={r.id} onClick={() => onPick(r.id)} className="w-full text-left px-5 py-3 border-b border-border/50 hover:bg-secondary/40 transition-colors">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">{[r.vendorName, r.jobCode].filter(Boolean).join(" · ") || "—"}</p>
            </button>
          ))}
        </div>
