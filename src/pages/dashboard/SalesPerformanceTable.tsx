...
export function SalesPerformanceTable({ title, sub, entries, limit }: { title: string; sub: string; entries: SalesPerformanceEntry[]; limit?: number }) {
  ...
  const columns: { key: SortKey | null; label: string }[] = [
    ...
    { key: null, label: t("dashboard.ranking.col.expectedRevenue") },
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                {columns.map((c) => (
                  <th key={c.label} className="px-3 py-2 text-left text-[10px] font-mono font-semibold text-muted-foreground uppercase tracking-wider">
                    {c.key ? (
