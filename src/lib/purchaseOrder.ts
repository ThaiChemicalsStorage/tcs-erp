...
export interface PurchaseOrder {
  ...
  // ── ผู้ขาย ────────────────────────────────────────────────────────────────
  /**
   * ผู้ขายในทะเบียน (2026-09-21) — `""` = ยังไม่ได้ผูก (ใบเก่า หรือพิมพ์ชื่อเอง)
   *
   * คอมเมนต์หัวไฟล์ `src/lib/vendors.ts` สัญญาฟิลด์นี้ไว้ตั้งแต่ 2026-08-31 แต่ไม่เคยถูกเพิ่มจริง
   * ใบสั่งซื้อจึงเก็บแค่ชื่อผู้ขายเป็นข้อความอิสระมาตลอด — เป็นรูรั่วที่ทำให้ด่าน "บัญชีต้องอนุมัติผู้ขาย
   * ก่อนเปิด PO" (คำสั่งเจ้าของข้อ 5) บังคับไม่ได้เลย เพราะไม่รู้ว่าชื่อที่พิมพ์คือผู้ขายรายไหน
   *
   * **ด่านจริงอยู่ที่ตอนอนุมัติใบ ไม่ใช่ตอนบันทึก** — ร่างยังพิมพ์อะไรก็ได้ แต่ `beforeApprove` จะ
   * ปฏิเสธถ้าไม่มี `vendorId` / ผู้ขายถูกเก็บถาวร-ปิดใช้งาน / บัญชียังไม่อนุมัติ
   */
  vendorId: string;
  /** ชื่อที่พิมพ์บนใบ — snapshot ตอนเลือก ไม่ตามการเปลี่ยนชื่อในทะเบียนทีหลัง */
  vendorName: string;
  ...
  approvedBy?: string;
  /**
   * ผู้อนุมัติที่ "ตั้งใจให้เป็นคนนี้" (2026-09-21) — เจ้าของสั่งข้อ 2: *"ใบ PO สามารถเลือกคนอนุมัติได้"*
   *
   * ⚠️ **ไม่ใช่การล็อกสิทธิ์** — เจ้าของเลือกไว้ตรง ๆ ว่า *แจ้งเตือนเฉพาะคนนั้น แต่คนอื่นที่มีสิทธิ์
   * ยังกดอนุมัติได้* · ค่านี้มีผลที่เดียวคือ "ตอนส่งขออนุมัติ จะแจ้งใคร" (ดู `recipients` hook ใน
   * `api/_lib/documentApproval.ts`) ด่านของการกดอนุมัติไม่ถูกแตะเลย
   *
   * ต่างจาก `approvedBy` ตรงที่อันนั้นคือ**ลายเซ็นหลังเกิดเหตุ** ส่วนอันนี้คือ**ความตั้งใจก่อนเกิดเหตุ**
   * ฉบับแก้ไข (Rewrite) จึงพาค่านี้ไปด้วย แต่ล้าง `approvedBy` ทิ้ง
   *
   * `intendedApproverName` เป็น snapshot ไว้แสดงผลเมื่อผู้ใช้คนนั้นถูกปิดบัญชีไปแล้ว
   */
  intendedApproverUserId?: string;
  intendedApproverName?: string;
 ...
 */
/**
 * บรรทัดของใบสั่งซื้อนี้ "ซื้อ" บรรทัดไหนของใบขอซื้ออยู่ — `null` = ไม่ได้ซื้ออะไรของใบขอซื้อเลย
 *
 * **นิยามเดียวที่ทั้งระบบใช้ร่วมกัน** — ก่อนหน้านี้ `purchasedPrLineIds()` (ด่านตอนสร้างใบสั่งซื้อ)
 * กับ `purchaseStateOf()` (ป้ายในหน้ารายการ) ต่างคนต่างถือกติกาเดียวกันคนละชุด แล้ว**เพี้ยนจากกันจริง ๆ
 * มาแล้วหนึ่งรอบ** (รีวิว 2026-09-21 ต้องไปแก้ทั้งสองที่พร้อมกันเพราะทั้งคู่ลืมกรองบรรทัดที่ยกเลิก)
 *
 * บรรทัดที่ยกเลิกไม่นับว่าซื้อแล้ว — ของถูกถอนไปแล้ว ไม่ถูกคิดเงิน ไม่ถูกลอกไปใบรับสินค้า
 * ถ้านับอยู่ บรรทัดของใบขอซื้อจะค้างเป็น "ซื้อไม่ได้ตลอดกาล" ทั้งที่ยังไม่ได้ของ
 */
export function orderedPrLineIdOf(line: PurchaseOrderLine): string | null {
  if (line.cancelled) return null;
  return line.sourcePrLineId || null;
}
