...
- Real backend: Vercel Serverless Functions (Node.js) + MongoDB Atlas; RBAC is enforced server-side, not just in the UI.
...
- The current Vercel deployment is a demo/trial only — real production hosting will be a self-managed server (not yet started, deferred until development finishes). No new Vercel-locked services should be added.
