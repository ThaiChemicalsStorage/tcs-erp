...
const RBAC_MIGRATIONS: RbacMigration[] = [
  {
    ...
    //
    // **ต้องแจก `vendor:view` ให้บัญชีด้วย ไม่ใช่แค่ `vendor:approve`** — วันนี้ `accounting_user`
    // ยิง `GET /api/vendors` ได้ 403 เพราะด่านของ `handleList` ผ่านได้สามทางเท่านั้น:
    // `vendor:view` / `purchaseOrder:view` / `purchaseRequest:view` และบัญชีไม่มีสักตัว
    //
    // **ห้ามใช้ `ap:manage` แทน** — มันแปลว่า "บันทึกการจ่ายเงิน" คนละอำนาจกัน
    //
    // บทบาท*จัดซื้อ*ของลูกค้าสร้างเอง ไม่มี `roleKey` คงที่ migration จึงแตะไม่ได้ — ปุ่ม "ส่งให้บัญชี
    // อนุมัติ" ใช้สิทธิ์เดิม `vendor:create`/`vendor:edit` ที่จัดซื้อมีอยู่แล้ว จึงไม่ต้องติ๊กอะไรเพิ่ม
    id: "vendor-approval-2026-09-21",
    grants: {
  ...
  },
  {
    ...
    //
    // แจกได้จริงแค่ตัวเดียว: `purchaseRequest:editApproved` ให้ `administrator` ซึ่งต้องเข้าได้ทุกเมนู
    // อยู่แล้ว — สิทธิ์ใหม่ของฝ่ายจัดซื้อ แก้ชื่อ/ยี่ห้อบนใบที่อนุมัติแล้วได้
    //
    // **สองอย่างที่ migration นี้แจกให้ไม่ได้ และต้องติ๊กเองบนเครื่องจริง** (บันทึกไว้ใน docs/TODO.md):
    //   1. `purchaseRequest:editApproved` ให้บทบาทของ**ฝ่ายจัดซื้อ**
    //   2. `purchaseRequest:view` ให้บทบาทของ**สโตร์** — ไม่งั้นขั้น "สโตร์เช็คของ" ที่เพิ่งสร้างจะไม่มี
    //      ใครเข้าถึงเลย (การจ่ายของเองคุมด้วย `stock:adjust` ที่สโตร์มีอยู่แล้ว)
    // ทั้งสองบทบาทเป็นบทบาทที่ลูกค้าสร้างเองในหน้า "บทบาทและสิทธิ์" ระบบไม่มี roleKey คงที่ให้อ้าง
    // (`defaultRoles` มี 8 บทบาท ไม่มีสโตร์และไม่มีจัดซื้อ) การเดา roleKey แล้วแจกผิดตัวแย่กว่าการ
    // บอกให้ไปติ๊ก เพราะสิทธิ์ที่แจกผิดจะไม่มีใครสังเกตเห็น
    id: "purchase-request-store-stage-2026-09-09",
    grants: {
      administrator: ["purchaseRequest:editApproved"],
    },
  },
  {
